...
| 버전 | 날짜 | 요약 |
|---|---|---|
| **v1.9.5** | 2026-06-03 | **언어팩 번역 적정화 + 저수준 가드 영문 통일 (v1.9.x 언어팩 라인).** 한국 개발 관례대로 `ko` 정본 팩의 정착 기술용어 표기를 영문으로 적정화 — 같은 용어를 한 곳은 영문·다른 곳은 음차로 쓰던 불일치를 영문으로 통일(캐시 `hit/miss` ↔ `히트/미스`: `build.console.cache`·`build.report.md_cache_hitmiss`; `raster` ↔ `래스터`: admin 홈), 글 목록 Actions 컬럼 헤더 `동작`→`작업`. Pond 보안 가드도 영문 일관 — 403 SAPI/루프백 가드(부트스트랩 이전 실행이라 `t()` 불가)의 한·영 혼용 한 줄을 전부 영문으로(405/CSRF 는 이미 영문). issue/warning 본문 `보완 필요`/`살펴볼 사항` 은 작성자 대면 UX 라 의도적 유지. user/ 스키마 무변경 → 새 마이그레이션 스텝 없음(스탬프 1.9.4 → 1.9.5); MANIFEST 86(개수 불변); dist byte-동일; 단위 519 + 진단 6/6. 도구=`ko` 콘솔 캐시 카운트 표기만 `히트/미스`→`hit/miss` 로 바뀜(그 외 ko 출력 불변). |
...
*Heron v1.9.5 — 빌드 Python + Pillow, 런타임 PHP (OPcache 권장). 누적 릴리스 내역은 [§ 16](#16-업데이트-로그).*
