...
| 버전 | 날짜 | 요약 |
|---|---|---|
| **v1.14.11** | 2026-06-13 | **저작 시점(author-time) 릴리스 정합성 자가 점검 추가 — v1.14.8·v1.14.9 식 사고(매니페스트 재생성·버전 참조 갱신 없이 코드만 커밋)가, 모든 사용자의 self-update 가 조용히 거부되기 *전에* 릴리스 단계에서 큰 소리로 실패하게 했다.** 두 파일이 새로 들어간다 — 점검 본체 `system/scripts/check_release.py`(1급 CLI 액션 `python Heron.py --release-check` 로 노출)와, 기존 `unittest discover` 스위트에 올라타 드리프트한 트리는 테스트조차 통과 못하게 막는 `system/tests/test_manifest_sync.py`. 점검은 두 부분이다. (1) 디스크에서 매니페스트를 재계산해 커밋된 `system/MANIFEST.json` 과 *통째로* 대조, 파일 추가·삭제·수정과 version 불일치를 전부 잡는다 — v1.14.8 에서 옛 `make_manifest.verify()` 가 놓쳤던 것(미등록 `pagemeta.php`·stale 해시·stale version 이 좁은 `ok` 를 그대로 통과)이 정확히 이 경로다. (2) `README.md`·`README.ko.md` 의 제목·푸터·changelog 최상단 행(굵게여야 함)이 `__version__` 과 일치하는지 확인한다. 도구 한정 — 빌더·dist 산출물 byte-동일. 스키마 무변경(스탬프 1.14.10 → 1.14.11); MANIFEST 90 → 92(재생성, +`check_release.py` +`test_manifest_sync.py`); 단위 607 → 617. |
...
*Heron v1.14.11 — 빌드 Python + Pillow, 런타임 PHP (OPcache 권장). 누적 릴리스 내역은 [§ 16](#16-업데이트-로그).*
