...
| 버전 | 날짜 | 요약 |
|---|---|---|
| **v1.11.0** | 2026-06-04 | **보안 감사 라운드 — 멀티에이전트 점검에서 확인된 결함만 수정(공개 검색 표시 버그 + 로컬 강건성).** 4개 도메인(공개 런타임 PHP·빌드 파이프라인·로컬 admin·배포/업데이트) 병렬 감사 후 직접 재현된 건만 반영. **(A1)** `search.py` 의 `html_to_plain` 이 태그만 벗기고 HTML 엔티티를 남겨, 본문의 escape 된 코드(예: 코드펜스의 `&lt;html&gt;`)가 스니펫에 `&lt;` 로 보존되고 런타임 `highlight_html()` 의 `htmlspecialchars` 가 이를 `&amp;lt;` 로 **이중 인코딩** → 공개 검색 결과에 `&lt;` 가 그대로 노출됐다. 평문화 끝에 `html.unescape` 를 넣어 스니펫은 진짜 `<` 를 담고 출력 이스케이프는 PHP 측에서 1회만 일어나게 — 색인 무영향(본문은 스니펫 전용). 회귀 가드 `HtmlToPlainTests` 신설. **(A2)** Pond 공개/비공개 토글이 `ltrim($name,'_')` 로 선행 언더스코어를 **전부** 떼어 `__draft`→`draft`(의도는 `_draft`); `substr($name, 1)` 로 정확히 한 개만 제거. **(A3)** `search_bm25.php` 의 `highlight_html` 에서 `preg_replace` 가 PCRE 한계 초과 시 `null` 을 돌려주면 `: string` 반환형이 strict_types 하에서 `TypeError`→500 이 된다 — 기본 설정에선 도달 불가하나 서버의 비표준 `pcre.*` 설정 대비 `?? $escaped` 폴백. **(A4)** `admin_safe_rel` 이 NUL 바이트를 통과시켜(비-Windows 경로 절단 여지) `str_contains($rel, "\x00")` 거부 추가. 감사에서 by-design(단일작성자 신뢰모델) 항목과 공급망 심층방어 항목은 비차단으로 보고 별도 백로그로 분리. user/ 스키마 무변경 → 새 마이그레이션 스텝 없음(스탬프 1.10.0 → 1.11.0); MANIFEST 88; 단위 532 → 536. |
...
*Heron v1.11.0 — 빌드 Python + Pillow, 런타임 PHP (OPcache 권장). 누적 릴리스 내역은 [§ 16](#16-업데이트-로그).*
