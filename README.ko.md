...
| 버전 | 날짜 | 요약 |
|---|---|---|
| **v1.7.2** | 2026-06-01 | **자가 업데이트 후 "업데이트 가능" 배너 잔존 핫픽스.** `update.py::self_update` 가 함수 시작의 `check_update` 로 `user/.heron/update.json` 캐시에 `update_available=true` 를 적어 두고도, 다운로드→오버레이→마이그레이션을 **마친 뒤 그 캐시를 갱신하지 않아** — 업데이트를 끝내고 Pond 를 재시작해도 목록 상단 "새 버전 있음" 배너가 사라지지 않았다(브라우저 캐시가 아니라 디스크 `update.json` 의 stale 값). 이제 오버레이 성공 직후 캐시를 새 버전 상태(`current`=새 버전·`update_available=false`)로 갱신한다. 더불어 배너의 최신 태그가 `vv1.7.2` 처럼 'v' 가 겹쳐 찍히던 표시 결함도 `list.php` 에서 선행 'v' 제거(`ltrim`)로 수정. 회귀 가드: 유일 통합 테스트 `test_full_flow` 가 이제 업데이트 후 캐시(`update_available=false`·`current`=새 버전)를 검증한다. user/ 스키마 무변경 → 새 마이그레이션 스텝 없음; 스탬프 1.7.1 → 1.7.2. dist byte-동일 to v1.7.0 (57파일); 단위 490 + 진단 6/6. |
...
*Heron v1.7.2 — 빌드 Python + Pillow, 런타임 PHP (OPcache 권장). 누적 릴리스 내역은 [§ 16](#16-업데이트-로그).*
