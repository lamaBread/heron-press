...
| 버전 | 날짜 | 요약 |
|---|---|---|
| **v1.14.2** | 2026-06-09 | **imgBox 캡션 스타일 복원 — 빌더가 캡션을 `<p><small><b>…</b></small></p>` 로 감싸, 캡션이 원본 사이트처럼 다시 작고 굵게 렌더된다.** 정본 `imgBox()` 는 캡션을 작고 굵게(브라우저 기본 `<small>` + `<b>`) 냈지만, Heron 빌더는 밋밋한 `<p class="caption">…</p>` 로 내보내 캡션이 큰 기본 글씨로 렌더됐고 기존 `.imgBox small` 스타일 훅도 아무것도 못 잡았다. `markdown.py` 의 두 출력부(마크다운형 `![[alt]](url) {캡션}` · 시뮬레이션 PHP `imgBox()`)가 이제 `<p><small><b>{캡션}</b></small></p>` 로 내며 서로 byte 동일하고, 기존 `.imgBox small` 규칙이 다시 원래의 좌측 여백을 준다. 수정이 user CSS 가 아니라 (self-update 가 오버레이하는) 빌더 `system/` 에 있으므로 기존 사이트도 다음 빌드에서 **마이그레이션·user CSS 수정 없이** 적용된다 — 캡션 있는 모든 imgBox 의 dist HTML 이 그에 맞춰 바뀐다(의도된 변경이라 byte 동일이 아님). 스키마 무변경(스탬프 1.14.1 → 1.14.2); MANIFEST 88; 단위 599. |
...
*Heron v1.14.2 — 빌드 Python + Pillow, 런타임 PHP (OPcache 권장). 누적 릴리스 내역은 [§ 16](#16-업데이트-로그).*
