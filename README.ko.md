...
| 버전 | 날짜 | 요약 |
|---|---|---|
| **v1.11.1** | 2026-06-04 | **자가업데이트 공급망 강건화 (보안 백로그 B1·B2)** — `update.py` 자가업데이트 경로의 심층 방어 2건(둘 다 고정 `REPO` + TLS 로 이미 1차 방어되나 구조적으로 강제). **(B1)** 다운로드 릴리스 트리에 `system/MANIFEST.json` 이 없을 때 `self_update` 가 예전엔 *주의만 찍고 그대로 오버레이* 해 sha256 무결성 게이트를 조용히 건너뛰었다 → 이제 **오버레이 전에 즉시 중단**(`cli.update.no_manifest` 문구 경고→거부로 변경). **(B2)** GitHub tags API 의 `zipball_url` 을 그대로 받던 것을, 신설 `_trusted_zipball()` 이 `https://api.github.com/repos/lamaBread/heron-press/` 로 시작할 때만 사용하고 아니면 구성된 `ZIPBALL` 로 폴백 — `check_update`·`self_update` 양쪽에 적용(리다이렉트·위조 출처 차단 이중 방어). `test_update.py` 회귀 가드 신설(MANIFEST 없는 다운로드는 설치본 불변; `_trusted_zipball` 통과/외부호스트/타저장소/None). 스키마 무변경 → 새 마이그레이션 스텝 없음(스탬프 1.11.0 → 1.11.1); MANIFEST 88; 단위 536 → 542. |
...
*Heron v1.11.1 — 빌드 Python + Pillow, 런타임 PHP (OPcache 권장). 누적 릴리스 내역은 [§ 16](#16-업데이트-로그).*
