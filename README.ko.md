...
| 버전 | 날짜 | 요약 |
|---|---|---|
| **v1.13.1** | 2026-06-06 | **배포 미리보기: 다섯 번째 *변경 없음* 카드 — 그대로 두는(전송 안 하는) 파일을 보여줘, 빌드·배포 판단 시 추가/수정/삭제로부터 "안 바뀐 것"을 머리로 역산하지 않아도 된다.** dry-run 은 *바뀌는* 파일만 한 줄씩 내고 건너뛰는 파일은 한 줄도 안 내므로, '변경 없음'은 rclone 출력에서 파싱할 수 없다 — **로컬 `dist/` 전체 워크에서 (추가 ∪ 수정) 집합을 뺀 나머지**로 계산한다(삭제는 원격 전용이라 워크에 안 잡힌다). 순수 로컬, 추가 네트워크 없음. 새 카드는 **삭제**와 **경고** 사이에 놓이고, 패널은 변경 없는 파일을 경로+크기로 나열하며 *"서버와 동일하여 전송하지 않습니다"* 주석을 단다(비교 대상이 없어 diff 버튼 없음). 표시 목록은 다른 패널과 같은 500파일 상한 + `+N개 더` 롤업이지만, 뺄셈용 변경 경로 집합은 **상한 없이** 추적한다. 카드 그리드는 `auto-fit` 으로 바뀌어 5장을 한 줄에 담고 좁으면 자연 줄바꿈한다. 기존 "미변경" 문구는 **변경 없음 / Unchanged** 로 다듬어 EN↔KO 정렬. 요약 JSON 은 **가산적**(`unchanged`/`unchanged_files`/`unchanged_more` 추가)이라 CLI 한 줄과 기존 테스트가 전부 그대로다. dry-run 전용; 적용 경로는 byte 불변. 스키마 무변경(스탬프 1.13.0 → 1.13.1); MANIFEST 88; 단위 583 → 589. |
| v1.13.0 | 2026-06-06 | 배포 미리보기: 추가 / 수정 / 삭제 / 경고 카드 — 클릭하면 펼쳐지고, 수정 파일은 파일별 코드 diff. v1.12.0 은 copy+update 를 "업로드" 한 카드로 합쳤지만, 이제 미리보기는 이를 **클릭형 아코디언 카드 4장**으로 나눈다 — **추가**(rclone `copy`), **수정**(`update`), **삭제**(`delete`, 0이 아니면 빨강), **경고** — 카드를 클릭하면 그 파일 목록이 아래 펼쳐진다. **삭제** 패널은 각 항목을 서버 실제 경로(`remote_path` + 상대경로)로 접두해 *어느 경로의 무엇*이 지워지는지 명확히 한다. **수정** 패널은 파일마다 *diff 보기* 버튼을 둔다: dry-run 은 파일 내용을 주지 않고 "이전"은 서버에만 있으므로, 클릭하면 **그 파일 하나만** 새 `Heron.py --deploy-diff`(stdin=상대경로 → 같은 연결 플래그로 `rclone cat`)로 라이브로 받아 VSC식 **unified 줄 diff**(Python `difflib`, `deploy.py` 의 `compute_diff`/`cat_remote` 단일 출처)를 렌더한다. 안전장치: 텍스트 전용(널바이트로 바이너리 판별 → "바이너리, diff 없음"), 512 KiB 상한(초과 → "너무 큼"), modtime 만 바뀐 경우 "내용 동일"로 표시, TOCTOU·연결 실패는 `gone`/`error` kind, 한 줄 2000자·한 diff 4000줄 클램프, 최초 배포용 500파일 목록 상한 + `+N개 더` 롤업(전체 조망용 디렉터리별 표는 유지). 새 `?a=deploy_diff` AJAX 엔드포인트는 POST+CSRF, 경로를 `admin_safe_rel` + `dist/` 하위 realpath 경계(`asset` 프록시와 동형)로 검증하고 argv 리스트(셸 미경유)로 실행한다 — 인증된 로컬 admin 은 *로컬 `dist/` 에도 존재하는* 원격 파일만 읽을 수 있어, 기존 적용-sync 가 이미 허용하는 범위의 부분집합이다. 요약 JSON 은 **가산적**(`upload` 유지; `added`/`modified`/`*_files`/`*_more`/`remote_path` 추가)이라 v1.12.0 CLI 한 줄과 기존 테스트가 전부 그대로다. `build_argv` 의 연결 플래그는 공용 `_connection_flags` 로 추출(기존 argv 테스트가 byte 불변을 보증). dry-run 전용; 적용 경로는 byte 불변. 스키마 무변경(스탬프 1.12.0 → 1.13.0); MANIFEST 88; 단위 568 → 583. |
| v1.12.0 | 2026-06-06 | 배포 미리보기: 접이식 10행 라이브 로그 + dry-run 결과 파싱·시각화. 관리자 배포 미리보기가 rclone 의 객체별 `--dry-run` NOTICE 전체(수백 줄)를 한 `<pre>` 에 쏟아 가독성이 죽었다. 이제 라이브 로그는 스트리밍 중 꼬리를 자동 추적하는 ~10행 peek(`.log-peek`, ≈212px)에 펼치기/접기 토글이 붙고, 그 아래 Pond 가 파싱 요약을 그린다 — 업로드 / 삭제 / 디렉터리 작업 / 경고 카드(삭제가 0이 아니면 빨강), 상위 디렉터리별 표(용량순, `(루트)` 버킷 + 30행 초과분 `그 외 N개` 롤업), 경고 목록(호스트키 미검증 NOTICE + `.DS_Store`/`Thumbs.db` 잡파일 업로드 플래그). 파싱은 `deploy.py` 단일 출처(`parse_size`/`human_size`/`DryRunSummary`/`build_dry_run_summary`): rclone 스트림을 집계해 **항상** 한 줄 사람용 요약을 출력하고(CLI 도 이득), Pond 가 `HERON_DEPLOY_SUMMARY=json` 을 줄 때만 제어문자(RS) sentinel 뒤에 기계용 JSON 한 줄을 내보낸다 — `deploy_run.php` 가 이를 가로채(로그에선 숨김) 카드·표로 렌더한다. `admin_proc_stream` 에 선택적 `$env` 인자 추가(`getenv()` 병합으로 `proc_open` 이 PATH 보존). dry-run 전용; 적용 경로는 byte 불변. 스키마 무변경(스탬프 1.11.4 → 1.12.0); MANIFEST 88; 단위 555 → 568. |
...
*Heron v1.13.1 — 빌드 Python + Pillow, 런타임 PHP (OPcache 권장). 누적 릴리스 내역은 [§ 16](#16-업데이트-로그).*
