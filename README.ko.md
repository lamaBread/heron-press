...
│   └── tests/                   ← 단위 테스트 (458) + run_diagnostics.py (6 항목)
│
...
12. **(v1.6.0) 마이그레이션·자가 업데이트 — 빌드는 읽기 전용 유지** — 업그레이드는 프로그램 표면(`system/` + 진입점)만 교체하고 `user/` 는 절대 건드리지 않는다. 기록된 스키마 스탬프(`user/.heron/version`)가 멱등·`user/`-한정 마이그레이션 체인을 구동한다. 빌드 자체는 `user/` 에 쓰지 않으며(원칙 5 무손상), `--migrate`/`--update` 만 writer 이고 그것도 백업 후 쓴다.
...
| 버전 | 날짜 | 요약 |
|---|---|---|
| **v1.6.1** | 2026-06-01 | **마이그레이션 충실도 수정 — 실제 v1.2.2 `site.yaml` 재현으로 드러난 2건.** LF 전용 실설정을 `--migrate` 로 돌려보다 v1.6.0 엔진의 빈틈 둘을 잡았다. ① **개행 보존** — 편집이 universal-newline 으로 읽고 텍스트 모드로 써서, Windows 에서 파일 *전체* 가 LF↔CRLF 로 뒤집혔다(LF 원본이 전부 CRLF 로 출력): "건드린 줄만 바뀐다"는 보장을 깨는 전(全)라인 VCS diff. 이제 bytes 로 읽고(`_yamledit.read_preserving`) bytes 로 써(`atomic_write`) LF·CRLF 원본을 **그대로** 보존하고, 스키마 스탬프도 LF 고정으로 쓴다. ② **변형 전 백업** — `--migrate` 가 `user/site.yaml` 을 백업 없이 in-place 로 고쳐 문서의 "writer 는 백업 후 쓴다" 와 어긋났다. 이제 엔진이 적용 직전 바뀔 모든 파일을 `user/.heron/backups/migrate-<UTC>/` 에 스냅샷한다(`--migrate` + 자가 업데이트 경로 모두 적용); 깨끗한 트리나 `--dry-run` 은 아무것도 안 쓴다. 스키마 변경 없음 → 새 마이그레이션 스텝 없음; 스탬프만 1.6.0 → 1.6.1 전진. dist 무변경(결정적, 57파일; 산출 경로 코드 미접촉). 단위 **464**(+신규 6 개행/백업) 통과; 진단 6/6. |
| v1.6.0 | 2026-05-31 | **마이그레이션 · 원클릭 업데이트 시스템** (§ 17-6), v1.5.0 user/system 분리 위에: 스키마 버전 스탬프(`user/.heron/version`), 순서·멱등 `user/`-한정 마이그레이션 엔진(1.5.3→1.6.0 스텝이 v1.4.0 폐기 `site.yaml` 5키를 주석 보존 줄단위 편집으로 정리), `MANIFEST.json` sha256 무결성 + allowlist 오버레이, Pond 원클릭 GitHub 확인/업데이트(`update.py`, stdlib 전용; 저장소 `lamaBread/heron-press`), CLI 탈출구(`--check`/`--migrate`/`--check-update`/`--update`). 빌드는 `user/` 읽기 전용 유지. dist byte-완전 동일 to v1.5.3 (57파일). |
...
*Heron v1.6.1 — 빌드 Python + Pillow, 런타임 PHP (OPcache 권장). 누적 릴리스 내역은 [§ 16](#16-업데이트-로그).*
