...
| 버전 | 날짜 | 요약 |
|---|---|---|
| **v1.9.7** | 2026-06-03 | **도구·사이트 기본 언어 한국어→영어 전환 + 로케일 도구화 (v1.9.x 언어팩 라인).** i18n 정본/기본/폴백 로케일을 `ko`→`en` 으로 바꿨다(`system/scripts/i18n.py` `CANONICAL`·`system/admin/lib/i18n.php` `I18N_CANONICAL`) — `user/.heron/locale` 이 없으면 도구 UI(Pond admin + 빌드/CLI)가 이제 **영어** 기본, 어떤 팩의 누락 키도 만국 공통으로 읽히는 **영어로 폴백**(이전엔 한국어). 이미 언어를 고른 기존 설치는 유지(1.8.0→1.9.0 마이그레이션의 `ko` 시드는 불변 → 1.9.0 이전 업그레이더는 계속 한국어), 한 번도 안 고른 설치는 영어 기본 — **Pond ▸ 설정**(`?a=settings`) 에서 변경. 방문자 사이트 `lang` 기본도 `ko`→`en`(코드 기본값 + 동봉 데모 `user/site.yaml: lang: en`) → `<html lang>`·검색·푸터·페이지네이션·404·홈/카테고리 라벨이 영어 기본; 단 이는 *방문자 콘텐츠 설정*(`site.yaml`)이라 자가 업데이트가 덮지 않음 → 신규 클론/배포 데모에만 적용(데모 i18n 예시 글은 사이트 기본=영어, 그 글만 `lang: ko` 로 글 단위 오버라이드를 계속 시연). 신설 도구 둘: `python Heron.py --check-locale [코드]`(읽기 전용 진단 — 영어 정본 대비 누락[영어 폴백]·잉여·stray 백슬래시·미번역 보고, 결함 시 비-0 종료로 CI 게이트), `python Heron.py --new-locale 코드`(영어 정본 verbatim 복사 + 엔도님 키 주입으로 즉시 패리티·드롭다운 등재, 코드 정규식 검증으로 경로 탐색 차단). user/ 스키마 무변경 → 새 마이그레이션 스텝 없음(스탬프 1.9.6 → 1.9.7). |
...
*Heron v1.9.7 — 빌드 Python + Pillow, 런타임 PHP (OPcache 권장). 누적 릴리스 내역은 [§ 16](#16-업데이트-로그).*
