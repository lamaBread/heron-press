...
| 버전 | 날짜 | 요약 |
|---|---|---|
| **v1.7.1** | 2026-06-01 | **자가 업데이트 백업 크래시 핫픽스.** 업데이트가 백업 단계에서 스키마 스탬프(`user/.heron/version`)를 백업 폴더로 복사할 때 **목적지 부모 디렉터리를 안 만들어** `FileNotFoundError` 로 업데이트 전체가 죽던 버그 수정 — `update.py::_backup_program` 의 스탬프 복사 한 곳만 선행 `parent.mkdir(parents=True, exist_ok=True)` 를 빠뜨렸다(같은 함수의 다른 복사 3곳은 모두 mkdir 후 복사; 다운로드·MANIFEST sha256 검증은 통과한 뒤 백업에서 터짐 — OS 무관). 회귀 원인: 유일한 통합 테스트 `test_full_flow` 가 스탬프를 일부러 안 만들어 `stamp.is_file()` 분기를 한 번도 안 탔음 → 이제 베이스라인 스탬프(1.5.3)를 실제 생성해 그 분기(부모 디렉터리 부재)를 커버한다. user/ 스키마 무변경 → 새 마이그레이션 스텝 없음; 스탬프 1.7.0 → 1.7.1. dist byte-동일 to v1.7.0 (57파일); 단위 490 + 진단 6/6. |
...
*Heron v1.7.1 — 빌드 Python + Pillow, 런타임 PHP (OPcache 권장). 누적 릴리스 내역은 [§ 16](#16-업데이트-로그).*
