...
| 버전 | 날짜 | 요약 |
|---|---|---|
| **v1.9.3** | 2026-06-02 | **argparse `--help` + 잔여 abort 현지화 (v1.9.x 언어팩 라인).** 마지막까지 한국어 하드코딩이던 *상시* 운영자 표면을 마감 — (1) `Heron.py --help` 의 description·epilog·11개 옵션 `help=` 를 신설 `cli.help.*` 로(argparse 는 파서 생성 시점에 텍스트를 고정하므로 `main()` 이 `parse_args` 보다 **먼저** `i18n.init_from_base` 적재 — 순서만 재배열, 신규 함수 없음); (2) `images.{widths,max_width,quality}` 검증 abort 6줄을 `build.abort.images_*` 로(빌더 `tool_tr`; `{received!r}` 로 원래 repr 보존); (3) 토크나이저 패리티-실패 헤더 + v1.9.2 가 놓친 형제 줄(미설치/타임아웃 시 영문 전용이던 warn)을 `build.parity.fail_header` / `php_unavailable` 로. 이제 admin·CLI·배포·업데이트·마이그레이션·빌드 콘솔/리포트·`--help`·전(全) abort 까지 **도구 언어로 완전 일관**(조건부 노출되는 글 단위 `_issue`/`_warning` 본문만 후속 범위). `ko` 값 byte-동일 → 한국어 출력 불변. user/ 스키마 무변경 → 새 마이그레이션 스텝 없음(스탬프 1.9.2 → 1.9.3); MANIFEST 해시 갱신(86파일, 개수 불변); dist byte-동일(빌드 콘솔/`--help`/리포트는 `dist/` 밖); 단위 514 → 517 + 진단 6/6. |
...
**세 표면.** (1) **방문자 사이트 chrome** — *사이트 언어*(`site.yaml: lang`), 빌드 시점에 `dist/` 에 구움; (2) **Pond admin UI** — *도구 언어*, PHP 전역 `t()`; (3) **빌드/CLI 운영자 메시지** — *도구 언어*, Python 전역 `i18n.t()`(및 빌더의 `tool_tr`). 표면 3 은 deploy/update/migration/CLI 출력, v1.9.2 부터 빌더 자신의 **16단계 진행·마일스톤·요약·`build-report.md` 전체**, v1.9.3 부터 argparse `--help`(description·epilog·옵션 `help=`)와 `images.*` 검증·토크나이저 패리티-실패 abort 까지 포함한다 — 그래서 도구=`en` 설치는 **상시 운영자 표면이 전부 영문**으로 나온다(`--help` 는 파서가 텍스트를 생성 시점에 고정하므로 `Heron.py main()` 이 인자 파싱보다 먼저 도구 언어를 적재한다). 남은 한국어 잔재는 *콘텐츠 결함이 있을 때만* 노출되는 글 단위 `_issue`/`_warning` 본문 일부로, 다음 범위 후보다.
...
*Heron v1.9.0 — 빌드 Python + Pillow, 런타임 PHP (OPcache 권장). 누적 릴리스 내역은 [§ 16](#16-업데이트-로그).*
