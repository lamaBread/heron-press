...
| 버전 | 날짜 | 요약 |
|---|---|---|
| **v1.8.0** | 2026-06-01 | **Pond 설정 화면 + 시스템 개요 홈.** Pond 에 **설정**(`?a=settings`) 추가 — ① `user/.heron/deploy.json` 구조화 폼(없으면 `deploy.example.json` 값으로 프리필; `ssh_key_path` 존재는 비검증 = 다른 머신·미생성일 수 있어 차단하면 footgun, 정말 없으면 배포 때 rclone 이 명확히 실패), ② **`user/site.yaml` 원문 편집기**(주석 보존) — 저장 시 신설 `Heron.py --check-config` 로 **빌드와 동일한 검증**을 통과해야만 기록되고, 직전본은 `user/.heron/backups/settings/` 에 백업. `--check-config` 는 후보를 stdin 으로 받아 빌드가 쓰는 그 `Builder._apply_site_config`(abort 검증)로 돌려 '검증 통과 → 빌드 실패' 불일치를 구조적으로 차단(`_load_config` 를 동작 보존 3분할, 부수효과인 토크나이저 패리티·Pillow 점검은 `_post_config_checks` 로 분리). 더불어 좌상단 **Pond admin** 브랜드가 이제 **시스템 개요 홈**(`?a=home`, 기본 진입)으로 링크 — 작성→빌드→`dist/`→배포→서빙 전체 흐름을 시각화하고, 글 목록은 nav 의 `?a=list` 로 이동(활성 nav 강조 + 버전 배지) (§ 17-3 · § 17-8). user/ 스키마 무변경 → 새 마이그레이션 스텝 없음; 스탬프 1.7.2 → 1.8.0. dist byte-동일 to v1.7.0 (57파일, 워크트리 재빌드로 직접 검증); 단위 490 + 진단 6/6. |
...
좌상단 **Pond admin** 브랜드는 v1.8.0 부터 **시스템 개요 홈**(`?a=home`)으로 링크된다 — Heron+Pond 전체 흐름을 시각화한 기본 진입 화면([§ 17-3](#17-3-기능)). 글 목록은 nav 의 **목록**(`?a=list`) 으로 분리됐다.
...
*Heron v1.8.0 — 빌드 Python + Pillow, 런타임 PHP (OPcache 권장). 누적 릴리스 내역은 [§ 16](#16-업데이트-로그).*
