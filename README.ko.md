...
| 버전 | 날짜 | 요약 |
|---|---|---|
| **v1.11.4** | 2026-06-06 | **rclone 배포 `ssh_alias` 위임 모드 (opt-in) — 패스프레이즈 키·호스트키 미스매치 배포 실패 정정** — `deploy.py` 의 키파일 경로가 rclone 에 `--sftp-key-file ~/.ssh/id_ed25519` 만 줘서 **패스프레이즈로 잠긴** 키가 `ssh: this private key is passphrase protected` 로 즉사했고, 그걸 우회해도 rclone 자체 호스트키 검증이 `knownhosts: key mismatch` 로 막혔다(Go `x/crypto` 가 비-ed25519 호스트키 알고리즘을 선호하는데 `known_hosts` 의 그 호스트명엔 ed25519 만 등록돼 있음). 신설 **선택** 필드 `ssh_alias`: 값이 있으면 `build_argv` 가 host/user/port/key/known_hosts 플래그를 전부 버리고 `--sftp-ssh "ssh <alias>"` + `--sftp-disable-hashcheck` 만 내보내, 인증과 호스트키 검증을 시스템 ssh(`~/.ssh/config`·Keychain·ssh-agent·`known_hosts`)에 위임한다 — `ssh <alias>` 가 이미 잘 되는 바로 그 경로이며, `--sftp-disable-hashcheck` 는 파일별 `ssh md5sum` 연결 폭주를 막는다. `load_config` 는 alias 모드에서 `remote_path` 만 필수로 보고 키·known_hosts 파일 존재 검사를 건너뛴다(그 경로 미사용); 별칭은 `^[A-Za-z0-9._-]+$` 로 제한(argv 단일 토큰 — 공백·셸 메타문자 차단). Pond ▸ 설정에 `ssh_alias` 입력이 추가돼 host/user/key 가 선택이 된다. **완전 하위호환** — `ssh_alias` 가 없으면 키파일 경로는 byte 불변(Windows 기본 무변경; alias 모드는 macOS/Linux opt-in). `EXAMPLE_CONFIG`/`deploy.example.json` 는 미변경(opt-in 이라 기본값처럼 오해 방지). 재발 가드: `load_config` alias 수락/거부·`remote_path` 만 필수, `build_argv` alias 플래그 형태·dry-run 토글, 실제 rclone 플래그 가드(v1.10.0)를 alias 경로로 확장. 또 (릴리스 도구): `make_manifest` 가 OS 파일탐색기 부산물(`.DS_Store`/`Thumbs.db`)을 어느 깊이서든 제외 — macOS/Windows 에서 매니페스트를 재생성해도 타 플랫폼 `--check` 가 매번 `missing` 으로 보던 유령 항목이 안 끼게 했다(가드 `test_update`). 스키마 무변경(스탬프 1.11.3 → 1.11.4); MANIFEST 88; 단위 546 → 555. |
| v1.11.3 | 2026-06-04 | 글 끝 발행/수정 메타 줄 사이트-언어 현지화 (v1.9.x 현지화 드리프트 정정) — v1.4.0 이 도입한 글 끝 발행/수정 메타 줄의 라벨("발행"/"수정")이 한국어로 하드코딩돼 v1.9.x 사이트-언어 스윕에서 누락됐다 — `lang=en`(v1.9.7 부터 기본)으로 빌드해도 **다른 chrome 은 영문인데 이 푸터만 한국어로 새던** 드리프트(데모 dist 의 모든 글 footer 가 "2026-… 발행"). `_render_article_end_meta` 를 나머지 방문자 chrome 과 동일하게 `self.site_tr`(사이트 언어, 빌드 시점 치환)로 라우팅하고, 신설 `site.article.published`/`updated` 키(`{date}` 자리표시자)를 en·ko 팩에 추가. 재발 가드: 로케일별 라벨 단언 2건(`test_v140`)·키 치환 1건(`test_i18n`), 키 패리티는 기존 `test_every_locale_matches_canonical_key_set` 가 강제. **열거 diff = `lang=en`(비-ko) 빌드의 글 페이지 footer `발행/수정`→`Published/Updated` 뿐 — `lang=ko` 는 byte 불변**(로케일 값이 옛 하드코딩과 동일); `__version__` 이 캐시 `global_hash` 입력이라 버전 범프가 글 캐시를 무효화 → 증분 재빌드도 옛 푸터 미잔존. 스키마 무변경 → 새 마이그레이션 스텝 없음(스탬프 1.11.2 → 1.11.3); MANIFEST 88; 단위 543 → 546. |
| v1.11.2 | 2026-06-04 | rclone PATH 폴백 무결성 경고 (보안 백로그 B3 — v1.11.x 보안 아크 마감) — 핀 바이너리 다운로드 실패로 `rclone_bin.ensure()` 가 PATH 의 rclone 으로 폴백하면 그 바이너리는 **sha256 미검증**으로 실행되므로(다운로드 sha256 *불일치* 는 폴백 없이 즉시 중단 — 불변), `cli.rclone.fallback_path` 를 양성 문안 대신 **미검증/무결성 미확인** 사실과 해결책을 명시하도록 강화(메시지 전용·호출부 무변경; 가드 `test_fallback_warns_binary_is_unverified`). 스키마 무변경(스탬프 1.11.1 → 1.11.2); MANIFEST 88; 단위 542 → 543. |
| v1.11.1 | 2026-06-04 | 자가업데이트 공급망 강건화 (보안 백로그 B1·B2) — 다운로드 릴리스 트리에 `system/MANIFEST.json` 이 없으면 `self_update` 가 즉시 중단(이전엔 주의만 찍고 오버레이), 신설 `_trusted_zipball()` 이 GitHub `zipball_url` 을 `repos/lamaBread/heron-press/` 출처로 고정(아니면 구성된 `ZIPBALL` 폴백)해 `check_update`·`self_update` 양쪽 적용. 스키마 무변경(스탬프 1.11.0 → 1.11.1); MANIFEST 88; 단위 536 → 542. |
| v1.11.0 | 2026-06-04 | 보안 감사 라운드 — 4개 도메인 멀티에이전트 병렬 점검에서 확인된 결함만: `search.py` 의 `html_to_plain` 에 `html.unescape` 추가로 공개 검색 스니펫의 escape 코드 이중 인코딩 해소(`&lt;`→`<`; 가드 `HtmlToPlainTests`); Pond 공개/비공개 토글 `ltrim($name,'_')`→`substr($name,1)`(선행 언더스코어 전부→한 개); `search_bm25.php` 의 `highlight_html` 에 `?? $escaped` 로 PCRE-null `TypeError`→500 방어(기본 설정 도달 불가); `admin_safe_rel` NUL 바이트 거부. by-design(단일작성자 신뢰모델)·공급망 심층방어 항목은 별도 백로그로 분리. 스키마 무변경(스탬프 1.10.0 → 1.11.0); MANIFEST 88; 단위 532 → 536. |
| v1.10.0 | 2026-06-04 | 배포 핫픽스 — `deploy.py` 의 `build_argv` 가 rclone 에 **없는** 플래그 `--sftp-known-hosts` 를 넘겨 모든 배포(미리보기·적용)가 플래그 파싱 단계에서 즉사하던 것을 `--sftp-known-hosts-file`(핀 v1.74.2 바이너리로 확인)로 교정. 단위 테스트가 **같은 오타를 기준값으로 박아** 못 잡았던 것 → `build_argv` 가 내보내는 플래그를 실제 rclone `help flags` 와 대조하는 재발 가드(`TestArgvFlagsAgainstRealRclone`) 신설. 스키마 무변경(스탬프 1.9.7 → 1.10.0); MANIFEST 88; 단위 531 → 532. |
| v1.9.7 | 2026-06-03 | 도구·사이트 기본 언어 한국어→영어 전환 + 로케일 도구화 (v1.9.x 언어팩 라인) — i18n 정본/기본/폴백을 `ko`→`en` 으로(`i18n.py` `CANONICAL`·`i18n.php` `I18N_CANONICAL`); 미선택 설치는 영어 기본·누락 키 영어 폴백, 기존 선택 설치는 유지(**Pond ▸ 설정**에서 변경). 방문자 사이트 `lang` 기본도 `ko`→`en`(자가 업데이트 미침범, 신규 클론/데모만). 신설 `--check-locale`(패리티/stray/미번역 진단, 결함 시 비-0 종료=CI 게이트)·`--new-locale`(영어 정본 복사+엔도님 주입 스캐폴딩). 스키마 무변경(스탬프 1.9.6 → 1.9.7); MANIFEST 88; 단위 531. |
| v1.9.6 | 2026-06-03 | 데모 설정 주석 영문 통일 (언어팩 라인) — 유일하게 한국어 주석이 남아 있던 `user/site.yaml` 주석만 영문으로(설정값·구조·키 순서·CRLF·UTF-8 no-BOM 불변); 로케일 팩은 의도적 미변경. 스키마 무변경(스탬프 1.9.5 → 1.9.6); MANIFEST 86; 단위 519. |
| v1.9.5 | 2026-06-03 | 언어팩 번역 적정화 + 저수준 가드 영문 통일 (언어팩 라인) — `ko` 정본 팩의 정착 기술용어 표기 불일치를 영문으로 통일(캐시 `hit/miss` ↔ `히트/미스`, `raster` ↔ `래스터`; 글 목록 Actions 컬럼 `동작`→`작업`) + Pond 403 SAPI/루프백 가드의 한·영 혼용을 영문화(405/CSRF 기존 영문). issue/warning 본문 `보완 필요`/`살펴볼 사항` 은 작성자 대면 UX 라 유지. 스키마 무변경(스탬프 1.9.4 → 1.9.5); MANIFEST 86; 단위 519. |
| v1.9.4 | 2026-06-02 | 글 단위 콘텐츠-결함 `_issue`/`_warning` 본문 현지화 (언어팩 라인) — 빌드 중 글/카테고리 결함 시에만 노출되던 한국어 하드코딩 진단 본문(builder 29곳) + 이미지 인코딩-실패를 `build.issue.*`(28키)·`build.label.*`(2키)·`build.warn.image_optimize_failed` 로(`{received!r}` repr 보존). 스키마 무변경(스탬프 1.9.3 → 1.9.4); MANIFEST 86; 단위 517 → 519. |
| v1.9.3 | 2026-06-02 | argparse `--help` + 잔여 abort 현지화 (언어팩 라인) — `Heron.py --help` 의 description·epilog·옵션 `help=` 를 신설 `cli.help.*` 로(`main()` 이 `parse_args` 보다 먼저 `i18n.init_from_base` 적재), `images.*` 검증 abort 6줄 + 토크나이저 패리티-실패/`php_unavailable` 을 `build.abort.*`/`build.parity.*` 로(`{received!r}` repr 보존). 스키마 무변경(스탬프 1.9.2 → 1.9.3); MANIFEST 86; 단위 514 → 517. |
| v1.9.2 | 2026-06-02 | 빌드 콘솔 + `build-report.md` 전체 현지화 (언어팩 라인) — 16단계 진행·시작/완료/캐시 마일스톤·라이브 카운터·`[search]` 패리티 라인·리포트 헤딩/요약/단계별 시간 표를 `build.step.*` / `build.console.*` / `build.parity.*` / `build.report.*` 로(빌더 `tool_tr`, report/search 는 전역 `i18n.t()`). 도구=`en` 빌드 패널 완전 영문; `ko` byte-동일(`[search]` 한 줄만 영문→한국어). 스키마 무변경(스탬프 1.9.1 → 1.9.2); MANIFEST 86; 단위 512 → 514. |
...
| v1.7.1 | 2026-06-01 | 자가 업데이트 백업 크래시 핫픽스 — 백업 단계가 스키마 스탬프(`user/.heron/version`)를 복사할 때 목적지 부모 디렉터리를 안 만들어 `FileNotFoundError` 로 업데이트가 죽던 버그 수정(`update.py::_backup_program`); `test_full_flow` 가 베이스라인 스탬프를 실제 생성해 그 분기를 커버. 스키마 무변경; 스탬프 1.7.0 → 1.7.1. dist byte-동일 to v1.7.0 (57파일). |
| v1.7.0 | 2026-06-01 | rclone 원클릭 dist 배포(§ 13-1 · § 17-7) — Pond 상단 **[배포]** 가 빌드된 `dist/` 를 rclone(SFTP)로 서버에 **증분 동기화**(바뀐 파일만 전송 + 서버 고아 삭제, **2단계 dry-run 게이트**로 `sync` 의 원격 삭제 방어). 핀 버전 rclone(**v1.74.2**)을 온디맨드로 받아 아카이브 SHA256 을 플랫폼 소스 핀과 대조 후 `system/runtime/bin/<os>-<arch>/` 에 두고(머신 종속물이라 `make_manifest` 가 프로그램 표면에서 제외 — .gitignore 와 짝, 커밋·MANIFEST·dist 무누수), 인증은 **SSH 키파일**(경로만 저장) + `known_hosts` 검증 강제. 설정은 gitignore 된 `user/.heron/deploy.json`(견본 `deploy.example.json` 커밋 + `m_1_7_0` 마이그레이션이 기존 사용자에게 시드). 빌더 무변경이라 dist byte-동일 to v1.6.2 (57파일); 스탬프 1.6.2 → 1.7.0; 단위 464 → 490 + 진단 6/6. |
...
*Heron v1.11.4 — 빌드 Python + Pillow, 런타임 PHP (OPcache 권장). 누적 릴리스 내역은 [§ 16](#16-업데이트-로그).*
