...
| 버전 | 날짜 | 요약 |
|---|---|---|
| **v1.6.0** | 2026-05-31 | **마이그레이션 · 원클릭 업데이트 시스템.** 사이트가 앞으로의 버전업을 편안하게 따라오게 하는 인프라 — v1.5.0 user/system 분리 위에 세움 (§ 17-6). ① **스키마 버전 스탬프** — `user/.heron/version` 이 `user/` 트리가 따르는 스키마 버전을 기록(프로그램 `__version__` 과 별개). `system/` 교체에도 생존·콘텐츠와 동행하고 `.` 접두라 빌드에서 자동 제외. 스탬프 부재 = pre-1.6.0 베이스라인. ② **마이그레이션 엔진**(`system/scripts/migrations/`) — 순서·**멱등** 스텝이 **오직 `user/` 만** 변형(프로그램 코드 미접촉); 1.5.3→1.6.0 스텝은 v1.4.0 폐기 `site.yaml` 5키를 줄단위 편집으로 정리(주석·순서 보존 — `yaml_parser` 는 읽기 전용). ③ **Pond 원클릭 업데이트** — GitHub 최신 태그 확인 → 다운로드 + `MANIFEST.json`(sha256) 검증 + 프로그램 표면(`system/` + `Heron.py` + `Pond.php`)만 오버레이 + 마이그레이션 → 재시작 안내. 헤더 **업데이트 확인**, 목록 배너. 로직은 Python(`update.py`, stdlib `urllib`+`zipfile`); Pond 는 원클릭 빌드처럼 얇은 트리거; 저장소 고정 `lamaBread/heron-press`. ④ **CLI 탈출구**(전문 사용자/CI): `Heron.py --check / --migrate [--dry-run] / --check-update / --update`. ⑤ **MANIFEST.json** — 프로그램 표면 목록 + sha256 으로 `--check` 무결성 + allowlist 오버레이(절대 `user/` 미접촉). 빌드는 `user/` 읽기 전용 유지(원칙 5); `--migrate`/`--update` 만 writer 이고 그것도 `user/.heron/backups/` 백업 후. **dist byte-완전 동일 to v1.5.3** (57파일, sha256 `fa131350…`; `__version__` 는 dist 미도달, 산출 경로 코드 무변경). 단위 **458**(429+신규 29) 통과; 진단 6/6; 예시 세트 보완 0 / 살펴볼 사항 0 / 의도된 PHP 빌드 1건. |
...
*Heron v1.6.0 — 빌드 Python + Pillow, 런타임 PHP (OPcache 권장). 누적 릴리스 내역은 [§ 16](#16-업데이트-로그).*
