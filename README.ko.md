...
| 버전 | 날짜 | 요약 |
|---|---|---|
| **v1.14.0** | 2026-06-07 | **Pond 관리자 목록: 행마다 있던 카테고리 드롭다운이 이제 글의 *실제* 카테고리를 보여주고(현재 부모가 `selected`), 값을 바꾸면 즉시 이동한다 — 별도 *이동* 버튼은 사라졌다.** 이전엔 드롭다운이 글의 현재 부모를 *제외*해, 하위 글은 첫 옵션이 항상 "(톱레벨)"로 떠 마치 "모든 글이 톱레벨"인 듯한 거짓 상태로 읽혔다(톱레벨 글은 거꾸로 첫 실제 카테고리가 기본값이 됐다). 이제 아무것도 제외하지 않고 현재 부모를 `selected` 로 표시 → 컨트롤이 곧 정직한 현재 카테고리 표시를 겸한다. 다른 카테고리를 고르면 `onchange` → confirm 1회 → 제출(폴더 rename; URL slug 불변)이고, **취소하면 `data-cur` 로 직전 선택을 복원**하며, 휠 스크롤 오작동은 `blur` 가드로 막고, 서버 **no-op 가드**로 "같은 카테고리 재선택"은 무동작이다. 이동 성공 후 목록은 해당 행의 **경로 문자열을 1회 강조**(~1.8s 초록 가라앉기)하고 `#moved`(스티키 헤더 오프셋)로 스크롤하며, `prefers-reduced-motion` 대체를 둔다. 관리자 UI 한정 — 빌더·dist 산출물은 byte 불변. 스키마 무변경(스탬프 1.13.1 → 1.14.0); MANIFEST 88; 단위 589(불변 — PHP 관리자 표면이라 Python 스위트 비대상). |
...
*Heron v1.14.0 — 빌드 Python + Pillow, 런타임 PHP (OPcache 권장). 누적 릴리스 내역은 [§ 16](#16-업데이트-로그).*
