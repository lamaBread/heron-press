...
| 버전 | 날짜 | 요약 |
|---|---|---|
| **v1.9.6** | 2026-06-03 | **데모 설정 주석 영문 통일 (v1.9.x 언어팩 라인).** 배포되는 데모 설정의 한국어 주석을 영문으로 통일 — i18n 팩과 달리 `user/*.yaml` 주석은 정적 파일이라 런타임 도구 언어(`user/.heron/locale`)를 따라갈 수 없으므로, `en`/`ko` 어느 도구 언어 사용자가 열어도 읽히도록 영문 단일화. `user/articles/**/meta.yaml` 18개는 이미 영문이라, 유일하게 한국어 주석이 남아 있던 `user/site.yaml` 한 파일만 번역(주석 텍스트만 — 설정값·구조·키 순서·CRLF·UTF-8 no-BOM 불변). 로케일 팩(`system/locales/`)은 **의도적 미변경** — 각 팩이 자기 언어로 자기를 서술(`ko`↔`ko`·`en`↔`en`)하는 것이 언어 내부 일관성. user/ 스키마 무변경 → 새 마이그레이션 스텝 없음(스탬프 1.9.5 → 1.9.6); MANIFEST 86(개수 불변, `__init__` 해시만 갱신); dist byte-동일; 단위 519. 자가 업데이트는 user/ 를 덮지 않으므로 신규 클론에 적용된다(기존 설치의 `site.yaml` 은 사용자 콘텐츠라 보존). |
...
*Heron v1.9.6 — 빌드 Python + Pillow, 런타임 PHP (OPcache 권장). 누적 릴리스 내역은 [§ 16](#16-업데이트-로그).*
