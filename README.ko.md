...
| 버전 | 날짜 | 요약 |
|---|---|---|
| **v1.10.0** | 2026-06-04 | **배포 핫픽스 — rclone 에 없는 플래그명 교정.** `deploy.py` 의 `build_argv` 가 호스트키 검증에 **존재하지 않는** 플래그 `--sftp-known-hosts` 를 넘겨 rclone 이 **플래그 파싱 단계**에서 거부 → 서버 접속 전에 즉사. dry-run 미리보기와 실제 적용이 동일 argv 라 **배포 기능 전체가 동작 불능**이었다. 올바른 이름은 `--sftp-known-hosts-file`(rclone 백엔드 옵션 `known_hosts_file` 의 CLI 규칙명; 핀 v1.74.2 바이너리로 확인) — v1.7.0 도입 이래 처음부터 잘못 박힌 상수(버전 드리프트 아님). 단위가 못 잡은 건 테스트가 **같은 오타를 기준값으로 박아** 버그와 자기일관적이었기 때문. 교정 = `deploy.py` 플래그·docstring + `test_deploy.py` 단언 둘 + README 보안 노트. **재발 가드 신설**(`TestArgvFlagsAgainstRealRclone`): `build_argv` 가 내보내는 모든 `--flag` 를 실제 rclone `help flags` 표면과 대조해 "코드가 만든 문자열 ↔ rclone 이 받는 플래그"의 불일치를 실제로 검출(바이너리 부재 시 skip). user/ 스키마 무변경 → 새 마이그레이션 스텝 없음(스탬프 1.9.7 → 1.10.0); MANIFEST 88; 단위 531 → 532. |
...
*Heron v1.10.0 — 빌드 Python + Pillow, 런타임 PHP (OPcache 권장). 누적 릴리스 내역은 [§ 16](#16-업데이트-로그).*
