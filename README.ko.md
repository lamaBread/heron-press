...
<?php imgBox("./imgs/p.jpg", "캡션", "alt") ?>
  → <div class="imgBox"><img src="/my-slug/imgs/p.jpg" alt="alt"><p><small><b>캡션</b></small></p></div>
...
![[이미지 설명]](./imgs/p.jpg) {캡션}   → <div class="imgBox"><img …><p><small><b>캡션</b></small></p></div>
```
...
| 버전 | 날짜 | 요약 |
|---|---|---|
| **v1.14.3** | 2026-06-09 | **문서 한 편만 고쳐도 사이트 전체가 재업로드되던 문제 정정 — 빌더가 이제 `dist/` 텍스트 산출물을 바이트가 실제로 바뀔 때만 쓰고, 같으면 파일 mtime 을 보존한다.** 빌드마다 모든 텍스트 산출물(글·카테고리·홈·404 HTML, `feed.rss`/`feed.atom`, `sitemap.xml`, `robots.txt`, `ads.txt`, `search.php`)을 무조건 다시 써서 내용이 byte 동일해도 mtime 이 갱신됐고, `deploy.py` 가 `rclone sync` 를 기본 modtime+size 비교(`--checksum` 없음)로 돌리므로 재빌드된 파일이 전부 *수정됨* 으로 잡혀 — 문서 한 편 편집이 배포 미리보기에 ~70개 변경으로 떠 사이트 전체를 재업로드했다. 신설 `_write_text_if_changed()` 헬퍼(기존 바이트를 읽어 같으면 쓰기·mtime 갱신을 건너뛰고, 다르면 원자적 `tmp→replace`)가 11개 `dist/` 쓰기 지점을 모두 받치며, 이미지·CSS 가 이미 쓰던 `_copy_if_newer` mtime 가드와 같은 결이다. 무변경 재빌드가 이제 출력 mtime 을 **0개** 갱신(이전엔 전부)하고 한 글 편집은 그 글 페이지만 건드린다; 배포 argv 는 무수정이라 `--sftp-disable-hashcheck` alias 최적화도 그대로다. 스키마 무변경(스탬프 1.14.2 → 1.14.3); MANIFEST 89; 단위 599 → 606. |
...
*Heron v1.14.3 — 빌드 Python + Pillow, 런타임 PHP (OPcache 권장). 누적 릴리스 내역은 [§ 16](#16-업데이트-로그).*
