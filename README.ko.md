...
| 버전 | 날짜 | 요약 |
|---|---|---|
| **v1.14.1** | 2026-06-09 | **imgBox 파싱 안정화 — 마크다운형 `![[alt]](url) {캡션}` 이 자기 구분자를 본문에 품어도 깨지지 않고, PHP형이 `<?=` 짧은 echo 태그와 대소문자 무시 `<?PHP` 를 인식한다.** 한 줄 인식 정규식을 다시 써서 URL 속 `)`(예: Windows 중복 다운로드 `dog(1).png`)·alt 속 `[`/`]`·캡션 속 `{`/`}` 가 더는 매치를 끊지 않게 했고, 탭·공백으로 들여쓴 imgBox 도 `<pre><code>` 로 떨어지지 않고 인식돼 컬럼0 div 로 나온다. 마크다운 캡션은 더 이상 HTML 이스케이프하지 않아 캡션 속 raw `<br>`·`<a>` 가 PHP형과 똑같이 렌더된다(두 형식 산출물 일치). `simulate_php_in_html`/`has_live_php` 는 `<?= imgBox(…) ?>` 와 대문자 `<?PHP …` 를 라이브 PHP 로 본다 — 후자는 이전엔 `.html` 페이지에 평문으로 샜다. Pond 실시간 미리보기와 빌드가 이 파서를 공유하므로 둘은 byte 동일하고, 이미 유효한 콘텐츠는 그대로 렌더된다. 스키마 무변경(스탬프 1.14.0 → 1.14.1); MANIFEST 88; 단위 589 → 599. |
...
*Heron v1.14.1 — 빌드 Python + Pillow, 런타임 PHP (OPcache 권장). 누적 릴리스 내역은 [§ 16](#16-업데이트-로그).*
