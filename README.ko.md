...
| 버전 | 날짜 | 요약 |
|---|---|---|
| **v1.11.2** | 2026-06-04 | **rclone PATH 폴백 무결성 경고 (보안 백로그 B3 — v1.11.x 보안 아크 마감)** — 핀 바이너리 다운로드가 실패해 `rclone_bin.ensure()` 가 `PATH` 의 `rclone` 으로 폴백할 때, 그 바이너리는 다운로드본이 받는 **sha256 검증 없이** 실행된다(다운로드의 sha256 *불일치* 는 폴백 없이 즉시 중단 — 불변). 폴백을 "PATH 의 rclone 사용" 이라는 양성 문안으로만 찍던 것을, `cli.rclone.fallback_path` 가 **미검증/무결성 미확인** 사실과 해결책(네트워크 복구, 또는 `system/runtime/bin/<os>-<arch>/` 에 검증된 바이너리 배치)을 명시하도록 강화. 메시지 전용 강건화(호출부 무변경); 가드 `test_fallback_warns_binary_is_unverified` 신설. 심층 방어·단일작성자 로컬 신뢰모델(Low). 스키마 무변경 → 새 마이그레이션 스텝 없음(스탬프 1.11.1 → 1.11.2); MANIFEST 88; 단위 542 → 543. |
...
*Heron v1.11.2 — 빌드 Python + Pillow, 런타임 PHP (OPcache 권장). 누적 릴리스 내역은 [§ 16](#16-업데이트-로그).*
