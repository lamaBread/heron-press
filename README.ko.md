...
| 버전 | 날짜 | 요약 |
|---|---|---|
| **v1.14.5** | 2026-06-10 | **빌드 버튼 로그가 실시간으로 흐른다 — 종전에는 빌드가 끝날 때까지 화면이 로딩 중에 멈춰 있다가 로그가 한꺼번에 떴다.** `?a=build` 가 블로킹 `admin_proc`(Heron.py 종료까지 대기 후 일괄 렌더)를 쓰던 것을 배포와 같은 스트리밍 패턴으로 전환 — 출력 버퍼를 끄고 빌더의 `[ n/16]` 단계 헤더(파이썬 쪽은 이미 `flush=True`)를 접이식 자동추적 `<pre id="log">` 에 한 줄씩 흘려보내며, 결과 배너는 종료 코드를 알 수 있는 로그 아래로 옮겼다. logbar+추적 스크립트는 `deploy_run.php` 인라인을 `layout.php` 의 공유 `admin_log_widget_head()` 로 추출하고, 문자열은 `admin.deploy_run.log.*`/`banner.tail` 을 `admin.log.*` 로 일반화했다. 빌드 자식의 stderr(빌드 말미 이슈 리포트·`[ABORT]`·traceback)는 `admin_proc_stream` 의 신규 opt-in `2 => ['redirect', 1]` 디스크립터로 stdout 에 병합 — 종료-후-드레인 설계의 파이프 버퍼 데드락 위험을 제거하고 출력 순서를 보존 — 하고, 드문 무flush print 안전벨트로 `PYTHONUNBUFFERED=1` 을 설정한다(deploy 의 sentinel 경로는 병합 기본 off 라 무변경). Admin-UI 한정; 빌더·dist 산출물 byte-동일. 스키마 무변경(스탬프 1.14.4 → 1.14.5); MANIFEST 89; 단위 606. |
...
*Heron v1.14.5 — 빌드 Python + Pillow, 런타임 PHP (OPcache 권장). 누적 릴리스 내역은 [§ 16](#16-업데이트-로그).*
