...
| 버전 | 날짜 | 요약 |
|---|---|---|
| **v1.14.10** | 2026-06-13 | **v1.14.8·v1.14.9 가 빠뜨린 릴리스 마무리 복구 — `system/MANIFEST.json` 재생성으로 깨졌던 self-update 를 되살리고, 누락된 변경 로그를 백필했다.** 두 릴리스가 코드만 커밋하고 매니페스트 재생성을 건너뛰어, 무결성 매니페스트가 v1.14.7(89개 파일·해시 stale·신규 `pagemeta.php` 미등록) 상태로 남았다. self-update 는 다운로드 트리를 자기 MANIFEST 와 sha256 대조해 `ok` 일 때만 오버레이하므로([update.py](system/scripts/update.py) 무결성 게이트), 두 태그 모두 검증 실패로 자가 업데이트가 *거부*됐다 — self-update 가 항상 최신 태그만 받는 덕에, 올바른 MANIFEST 를 담은 이 릴리스 하나로 모두 복구된다(깨진 1.14.8·1.14.9 태그는 건너뛰고 직행). 이번 릴리스는 `make_manifest` 로 전 파일을 재해시해 MANIFEST 를 89 → 90(version 1.14.10, `pagemeta.php` 등록)으로 갱신하고, 빠졌던 v1.14.8·v1.14.9 변경 로그(ko/en)와 제목·푸터 버전을 소급했다. 문서·매니페스트 전용 — 코드 무변경, 빌더·dist 산출물 byte-동일. 스키마 무변경(스탬프 1.14.9 → 1.14.10); MANIFEST 89 → 90(재생성); 단위 607. |
...
*Heron v1.14.10 — 빌드 Python + Pillow, 런타임 PHP (OPcache 권장). 누적 릴리스 내역은 [§ 16](#16-업데이트-로그).*
