...
> **v1.5.0 한 줄 요약** — 루트를 **`user/` (네가 편집: `articles`·`site.yaml`·`templates`·`styles`·`branding`) 와 `system/` (프로그램: `scripts`·`runtime`·`admin`·`tests`) 로 분리**하고 진입점을 `Heron.py`(빌드)·`Pond.php`(로컬 저작)로 명명했다. 표현 표면(페이지 템플릿·CSS·정체성 자산)이 빌더 내부에서 빠져나와 *"내가 만지는 것 vs 시스템"* 이 한눈에 갈린다. 순수 *소스 레이아웃* 변경이라 **dist 는 v1.4.2 와 byte-완전 동일** (787=787, `diff -r` 빈 출력 — v0.8.1 "빌더를 `src/` 로, 산출물 불변" 의 확장판). 단위 **429** 승계 · 진단 6/6. 누적 기능은 [§ 16](#16-업데이트-로그) 표.
...
│   └── tests/                   ← 단위 테스트 (429) + run_diagnostics.py (6 항목)
│
...
| 버전 | 날짜 | 요약 |
|---|---|---|
| **v1.5.0** | 2026-05-29 | **루트 `user/`·`system/` 분리 + Heron/Pond 명명** (구조 릴리스, dist byte-불변) — 표현 표면(페이지 템플릿·전역 CSS·정체성 자산)을 빌더 내부에서 빼내 루트를 `user/` (네가 편집: `articles`·`site.yaml`·`templates`·`styles`·`branding`) 와 `system/` (프로그램: `scripts`·`runtime`·`admin`·`tests`) 로 분리. 진입점 `build.py`→`Heron.py`·`admin.php`→`Pond.php`. 옛 `src/templates`(=.html+.php 혼재)·`src/assets`(=css+png+js 혼재) 두 혼합 폴더를 해소 — .html→`user/templates`, css→`user/styles`, og→`user/branding`, `search*.php`·`*.js`→`system/runtime`(서브타임 코드). `dist/assets/` 는 `user/styles`+`user/branding`+`system/runtime`(.js) 를 빌더가 집계(`/assets/` URL·산출물 불변). 파일명 전부 보존 → **dist 는 v1.4.2 와 byte-완전 동일** (787=787, `diff -r` 빈 출력 — v0.8.1 "빌더를 `src/` 로, 산출물 불변" 의 확장판). 단위 **429** 승계 · 진단 6/6 승계. |
...
*Heron v1.5.0 — 빌드 Python + Pillow, 런타임 PHP (OPcache 권장). 누적 릴리스 내역은 [§ 16](#16-업데이트-로그).*
