...
| 버전 | 날짜 | 요약 |
|---|---|---|
| **v1.9.4** | 2026-06-02 | **글 단위 콘텐츠-결함 `_issue`/`_warning` 본문 현지화 (v1.9.x 언어팩 라인).** 빌드 중 글/카테고리에 결함이 있을 때만 노출되던 한국어 하드코딩 진단 본문(meta.yaml 타입·`template`·`seo`·`tags` 폴백, slug 정규식/예약어/중복/충돌, date·updated 형식, 필수 필드 누락, `seo.description` 누락·빈, content 누락, 내부 링크 깨짐 등 builder 29곳)과 이미지 인코딩-실패 메시지(images.py)를 `build.issue.*`(28키)·`build.label.*`(2키 — `template` 미발견 위치 라벨)·`build.warn.image_optimize_failed` 로(빌더 `self.tool_tr`, images.py 는 전역 `i18n.t()`). `{received!r}` 로 원래 f-string `{x!r}` repr 보존, slug-예약어의 동적 시스템 경로는 파라미터로 선계산. 이제 *조건부* 운영자 출력까지 도구 언어로 일관 — 남은 한국어는 의도적 잔여(부트스트랩 이전이라 `t()` 불가한 Pond 403 가드 + 사용자 결정으로 보존한 사이트 chrome 영문값)뿐. `ko` 값 byte-동일 → 한국어 출력 불변. user/ 스키마 무변경 → 새 마이그레이션 스텝 없음(스탬프 1.9.3 → 1.9.4); MANIFEST 해시 갱신(86파일, 개수 불변); dist byte-동일(진단 본문은 `dist/` 밖); 단위 517 → 519 + 진단 6/6. |
...
**세 표면.** (1) **방문자 사이트 chrome** — *사이트 언어*(`site.yaml: lang`), 빌드 시점에 `dist/` 에 구움; (2) **Pond admin UI** — *도구 언어*, PHP 전역 `t()`; (3) **빌드/CLI 운영자 메시지** — *도구 언어*, Python 전역 `i18n.t()`(및 빌더의 `tool_tr`). 표면 3 은 deploy/update/migration/CLI 출력, v1.9.2 부터 빌더 자신의 **16단계 진행·마일스톤·요약·`build-report.md` 전체**, v1.9.3 부터 argparse `--help`(description·epilog·옵션 `help=`)와 `images.*` 검증·토크나이저 패리티-실패 abort, v1.9.4 부터 글 단위 `_issue`/`_warning` 콘텐츠-결함 본문과 이미지 인코딩-실패 메시지까지 포함한다 — 그래서 도구=`en` 설치는 **상시는 물론 조건부(콘텐츠 결함) 운영자 표면까지 전부 영문**으로 나온다(`--help` 는 파서가 텍스트를 생성 시점에 고정하므로 `Heron.py main()` 이 인자 파싱보다 먼저 도구 언어를 적재한다). 남은 한국어는 의도적 잔여뿐 — 부트스트랩(i18n 적재) 이전에 실행돼 `t()` 를 못 쓰는 Pond 403 가드와, 사용자 결정으로 보존한 일부 사이트 chrome 영문값이다.
...
*Heron v1.9.4 — 빌드 Python + Pillow, 런타임 PHP (OPcache 권장). 누적 릴리스 내역은 [§ 16](#16-업데이트-로그).*
