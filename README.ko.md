...
| 버전 | 날짜 | 요약 |
|---|---|---|
| **v1.14.12** | 2026-06-13 | **저장소 루트에 MIT `LICENSE` 파일 추가 — 그동안 공개 저장소에 라이선스가 없어, 법적으로는 All Rights Reserved(누구도 사용·수정·재배포 불가)였고, 공유를 전제로 한 도구로선 의도치 않은 상태였다.** 저작권자 Copyright (c) 2026 Siheon Lee. 관대형 라이선스이며 번들된 3rd-party 코드(포팅된 Parsedown © Emanuil Rusev·온디맨드 rclone 바이너리, 둘 다 MIT)와 충돌이 없다. `LICENSE` 는 (`.gitignore` 처럼) 프로그램 표면이 아니라 저장소 인프라라 `system/MANIFEST.json` 과 self-update 오버레이에서 제외되며, 신설 [§ 19](#19-라이선스) 가 이를 3rd-party 출처 표기와 함께 문서화한다. 문서 전용 — 빌더·dist 산출물 byte-동일. 스키마 무변경(스탬프 1.14.11 → 1.14.12); MANIFEST 92(불변); 단위 617. |
| v1.14.11 | 2026-06-13 | 저작 시점(author-time) 릴리스 정합성 자가 점검 추가(`python Heron.py --release-check` + 기존 `unittest discover` 스위트에 올라타는 `system/tests/test_manifest_sync.py`) — v1.14.8·v1.14.9 식 사고(매니페스트 재생성·버전 참조 갱신 없이 코드만 커밋)가 모든 사용자의 self-update 가 조용히 거부되기 전에 릴리스 단계에서 큰 소리로 실패하게 했다. 디스크에서 매니페스트를 재계산해 커밋된 `system/MANIFEST.json` 과 *통째로* 대조하고(파일 추가·삭제·수정·version 불일치를 전부 잡음), `README.md`·`README.ko.md` 의 제목·푸터·changelog 최상단 행이 `__version__` 과 일치하는지 확인한다. 도구 한정 — 빌더·dist byte-동일. 스키마 무변경(스탬프 1.14.10 → 1.14.11); MANIFEST 90 → 92(재생성, +`check_release.py` +`test_manifest_sync.py`); 단위 607 → 617. |
| v1.14.10 | 2026-06-13 | v1.14.8·v1.14.9 가 빠뜨린 릴리스 마무리 복구 — 둘 다 코드만 커밋하고 `system/MANIFEST.json` 재생성을 건너뛰어 무결성 매니페스트가 stale(v1.14.7·89파일·신규 `pagemeta.php` 미등록)했고, self-update 의 sha256 게이트가 두 태그를 *거부*했다. 매니페스트 재생성(89 → 90·version 1.14.10)으로 모두 한 번에 복구되고(self-update 는 항상 최신 태그만 받음), 빠졌던 v1.14.8·v1.14.9 변경 로그(ko/en)·제목·푸터 버전을 소급했다. 문서·매니페스트 전용, 코드 무변경. 스키마 무변경(스탬프 1.14.9 → 1.14.10); MANIFEST 89 → 90(재생성); 단위 607. |
| v1.14.9 | 2026-06-13 | 글 푸터 이전/다음 글 내비게이션 라벨 i18n — `lang:en` 사이트에서도 한국어로 렌더되던 마지막 방문자 문자열을 고쳤다. 다른 Surface 1 문자열은 전부 `self.site_tr.t('site.…')`(빌드 시 `lang` 로케일 팩 조회)를 거치는데, 이 nav 만 `다른 글`(aria-label)·`‹ 이전 글`·`다음 글 ›` 가 `builder.py` 에 하드코딩돼 있었다. 라벨 3개를 ko/en `site.yaml`(`site.prev_next.aria`/`prev`/`next`)로 옮기고 `_render_prev_next_nav` 가 `site_tr` 로 조회하게 했다(ko 값은 옛 하드코딩과 byte-동일, en 은 More posts / ‹ Previous / Next ›). ko 빌드 출력 불변; en dist 의 방문자 노출 한국어 0건. 빌더 한정. 스키마 무변경(스탬프 1.14.8 → 1.14.9); **MANIFEST·변경 로그 갱신 누락(v1.14.10 에서 소급)**; 단위 607. |
| v1.14.8 | 2026-06-13 | Pond 에 페이지 설정 편집기 추가 — 그동안 UI 가 없던 홈(`user/articles/meta.yaml`)·카테고리(`user/articles/<카테고리>/meta.yaml`) 의 `meta.yaml` 을 site.yaml 과 똑같은 원문(raw YAML) 방식으로 편집한다. 빌더가 홈·카테고리를 한 파서(`_parse_category_meta_file(scope, text=…)`)로 처리하므로 편집기도 하나로 통합했고, 저장 시 빌드와 동일한 검증 게이트(신설 `Heron.py --check-page-meta <REL>` — stdin 후보를 디스크 변경 없이 검사, 어떤 issue 든 있으면 거부)를 통과해야 기록된다. 빈 본문은 (선택 파일인) meta.yaml 을 삭제해 기본값으로 되돌린다(삭제 전 백업). 진입점은 설정 ④ 섹션과 목록 헤더의 '페이지 설정' 버튼; 신규 뷰 `system/admin/views/pagemeta.php`. 관리자 UI + CLI 게이트; 빌더·dist 산출물 byte-동일. 스키마 무변경(스탬프 1.14.7 → 1.14.8); **MANIFEST·변경 로그 갱신 누락 — self-update 무결성 깨짐(v1.14.10 에서 소급·복구)**; 단위 607. |
| v1.14.7 | 2026-06-11 | **파일별 배포 diff(`?a=deploy_diff`)가 연결 스톨로 Pond 가 동결되는 시간을 제한한다 — `cat_remote` 에 짧은 `--contimeout`/`--timeout`/`--retries 1`/`--low-level-retries 2` 를 붙여, 끊기지 않고 멈춘 호스트에서 단일 워커 `php -S` 가 수 분 매달리는 대신 빨리 실패하게 했다.** 블로킹 `rclone cat` 에 타임아웃이 없어 노트북 절전 복귀·VPN 단절 시 rclone 기본값(contimeout 1m·IO 5m)에 맡겨져 diff 박스뿐 아니라 *모든* Pond 페이지가 포기할 때까지 잠겼다; 장시간 정상 동작인 sync(`build_argv`)는 손대지 않았다. v1.10.0 식 회귀 가드로 cat 플래그도 실제 rclone 바이너리와 대조한다. 함께 뷰 한 줄 수정 — 배포 결과 배너에 빌드·업데이트 배너엔 이미 있던 `margin-top:14px` 를 넣어 apply·실패 시 로그 박스에 밀착하지 않게 했다. 관리자 UI + 배포 경로 4플래그 변경; 빌더·dist 산출물 byte-동일. 스키마 무변경(스탬프 1.14.6 → 1.14.7); MANIFEST 89; 단위 607. |
| v1.14.6 | 2026-06-10 | 업데이트 버튼(`?a=update`) 로그도 실시간으로 흐른다 — v1.14.5 가 빌드 버튼에 한 수정과 동일하게, 블로킹 `admin_run_heron` 에서 스트리밍 경로(출력 버퍼 끔, 신설 `admin_run_update_stream`, 공유 `admin_log_widget_head()` 로그 위젯, 로그 아래 결과 배너)로 전환하고 `_action_update` 에 없던 줄 단위 `print(..., flush=True)`(없으면 파이프로 아무것도 안 흐름)를 넣었다. 부가 효과로 뷰·layout·proc·i18n 이 자가 오버레이 전에 로드돼 페이지가 구버전 코드의 일관 상태로 렌더된다. 스키마 무변경(스탬프 1.14.5 → 1.14.6); MANIFEST 89; 단위 606. |
| v1.14.5 | 2026-06-10 | 빌드 버튼 로그가 실시간으로 흐른다 — `?a=build` 를 블로킹 `admin_proc` 에서 배포형 줄 단위 스트리밍으로 전환하고, logbar+추적 스크립트를 공유 `admin_log_widget_head()` 로 추출(문자열은 `admin.log.*` 로 일반화)했으며, 빌드 자식의 stderr 는 신설 opt-in `2 => ['redirect', 1]` 디스크립터로 stdout 에 병합했다(데드락 제거·순서 보존). Admin-UI 한정; 빌더·dist 산출물 byte-동일. 스키마 무변경(스탬프 1.14.4 → 1.14.5); MANIFEST 89; 단위 606. |
| v1.14.4 | 2026-06-09 | imgBox 캡션을 HTML 표준 시맨틱 마크업으로 — `markdown.py` 의 두 출력부(마크다운형 · 시뮬레이션 PHP `imgBox()`)가 비표준 `<div>` + `<p><small><b>` 조합을 `<figure class="imgBox">` + `<figcaption><small>` 로 교체하고, 표준 유효성을 위해 컨테이너도 `<figure>` 로 올렸다. `.imgBox` 규칙이 전부 클래스·자손 선택자라 마이그레이션·user CSS 수정이 없고(캡션은 의도적으로 더는 굵지 않음), 두 출력부는 byte 동일을 유지하며 캡션 있는 imgBox 의 dist HTML 만 의도대로 바뀐다. 스키마 무변경(스탬프 1.14.3 → 1.14.4); MANIFEST 89; 단위 606. |
...
Heron 은 **MIT 라이선스** 로 배포된다 — 저장소 루트의 [`LICENSE`](LICENSE) 참조. 요약하면: 저작권 고지와 라이선스 전문을 보존하는 한 누구나 사용·복사·수정·재배포(폐쇄 소스·상업적 이용 포함)할 수 있고, 소프트웨어는 무보증(as-is)으로 제공된다.

Copyright (c) 2026 Siheon Lee.

**3rd-party 구성요소** (전부 관대형, MIT 호환):

| 구성요소 | 위치 | 라이선스 |
|---|---|---|
| **Parsedown** 1.7.4 — `system/scripts/parsedown.py` 로 Python 포팅 | 번들(소스) | MIT — © Emanuil Rusev ([parsedown.org](http://parsedown.org)) |
| **rclone** (핀 v1.74.2) | `system/runtime/bin/` 로 온디맨드 다운로드, **커밋 안 함** | MIT |
| **Pillow** | 빌드 시 의존성(`pip install Pillow`), 번들 안 함 | HPND (MIT 계열) |

`LICENSE` 는 (`.gitignore` 처럼) 저장소 인프라이며 `system/MANIFEST.json` 이 추적하는 프로그램 표면이 아니라, self-update 오버레이 대상이 아니다.

---

*Heron v1.14.12 — 빌드 Python + Pillow, 런타임 PHP (OPcache 권장). 누적 릴리스 내역은 [§ 16](#16-업데이트-로그).*
