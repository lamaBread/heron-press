...
| 버전 | 날짜 | 요약 |
|---|---|---|
| **v1.14.6** | 2026-06-10 | **업데이트 버튼 로그도 실시간으로 흐른다 — v1.14.5 가 빌드 버튼에 한 수정과 동일; 종전에는 다운로드 → 무결성 → 백업 → 오버레이 → 마이그레이션 전 과정이 끝날 때까지 화면이 로딩 중에 멈춰 있다가 로그가 한꺼번에 떴다.** `?a=update` 가 블로킹 `admin_run_heron` 에서 빌드·배포와 같은 스트리밍 경로로 전환 — 출력 버퍼를 끄고 `proc.php` 신설 `admin_run_update_stream`(opt-in `2 => ['redirect', 1]` 로 stderr 병합, `PYTHONUNBUFFERED=1`)과 공유 `admin_log_widget_head()` 로그 위젯을 쓰며, 결과 배너는 종료 코드를 알 수 있는 로그 아래로 옮겼다(재시작 안내는 그대로). 파이썬 쪽 `_action_update` 는 이제 `print(..., flush=True)` 로 로그를 낸다 — 빌더와 달리 줄 단위 flush 가 없어 이것 없이는 파이프에서 아무것도 흐르지 않는다. 부가 효과: 뷰·layout·proc·i18n 테이블이 오버레이 *전에* 전부 로드되어 페이지가 구버전 코드의 일관 상태로 렌더된다 — 종전 구조는 오버레이 *후* `update.php` 를 require 해 새 버전 뷰를 구버전 Pond 가 파싱했다. 관리자 UI + CLI flush 한 줄; dist 산출물 byte-동일. 스키마 무변경(스탬프 1.14.5 → 1.14.6); MANIFEST 89; 단위 606. |
...
*Heron v1.14.6 — 빌드 Python + Pillow, 런타임 PHP (OPcache 권장). 누적 릴리스 내역은 [§ 16](#16-업데이트-로그).*
