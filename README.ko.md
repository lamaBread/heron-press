...
| 버전 | 날짜 | 요약 |
|---|---|---|
| **v1.9.0** | 2026-06-01 | **도구 다국어 — 언어팩(§ 17-9).** 한국어 기반이던 도구를 로케일 팩으로 현지화한다. **세 표면**을 같은 포맷으로 분리: ① 방문자 사이트 chrome(검색 박스·푸터·페이지네이션 aria·404·홈/카테고리 라벨), ② Pond admin UI 전체(258키), ③ 빌드/CLI 메시지(경고·abort). **로케일 선택자는 둘로 분리** — *사이트 언어*는 기존 `site.yaml: lang` 으로 **빌드 시점에 `dist/` 에 구워지고**(결정적 빌드 유지; `search.php` 는 BM25 인덱스처럼 빌드 때 문자열 테이블을 주입), *도구 언어*는 신설 `user/.heron/locale`(없으면 `ko`) 로 **런타임 lookup** 하며 Pond **설정**의 드롭다운에서 고른다(§ 17-8). 공통 포맷은 `system/locales/<locale>/*.yaml` 플랫 닷키이고 폴더 안 조각(`admin`/`site`/`build`)을 머지한다 — `ko` 가 정본 겸 폴백, 키 부재 시 키 문자열을 그대로 돌려 절대 빈칸이 안 된다. 로더는 외부 의존성 0(Python `system/scripts/i18n.py`, PHP `system/admin/lib/i18n.php` 가 같은 포맷 공유). 영어 팩(`en`) 동봉 — 새 언어는 `*.yaml` 한 벌만 추가하면 된다. `m_1_9_0` 마이그레이션이 `user/.heron/locale=ko` 를 시드(부재 시·멱등 — 기존 사용자 UI 그대로 유지). **`ko` 값은 v1.8.0 하드코딩과 byte-동일**이라 `lang=ko`·도구=`ko` 의 `dist/` 렌더 출력은 불변(`search.php` 소스만 빌드 시점 주입으로 재생성). 스탬프 1.8.0 → 1.9.0; MANIFEST 74 → 84 파일; 단위 490 → 499 + 진단 6/6. |
