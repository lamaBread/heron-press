...
|---|---|---|
| **v1.6.1** | 2026-06-01 | **Migration fidelity fixes — surfaced by replaying a real v1.2.2 `site.yaml`.** Running an actual LF-only config through `--migrate` exposed two gaps in the v1.6.0 engine. ① **Newline preservation** — edits were read with universal-newline and written in text mode, so on Windows the *whole* file flipped LF↔CRLF (an LF original came out all-CRLF): a full-file VCS diff that broke the "only edited lines change" guarantee. The migration now reads bytes (`_yamledit.read_preserving`) and writes bytes (`atomic_write`), preserving LF **and** CRLF originals exactly; the schema stamp is likewise written LF-fixed. ② **Backup before mutation** — `--migrate` rewrote `user/site.yaml` in place with no backup, contradicting the documented "writers back up first". The engine now snapshots every at-risk file to `user/.heron/backups/migrate-<UTC>/` before applying (covers `--migrate` and the self-update path); a clean tree or `--dry-run` writes nothing. No schema change → no new migration step; the stamp just advances 1.6.0 → 1.6.1. dist unchanged (deterministic, 57 files; no output-path code touched). 464 tests (+6 newline/backup) pass; 6/6 diagnostics. |
| v1.6.0 | 2026-05-31 | **Migration & one-click update system** (§ 17-5), on the v1.5.0 user/system split: a schema-version stamp (`user/.heron/version`), an ordered idempotent `user/`-only migration engine (the 1.5.3→1.6.0 step tidies the five v1.4.0-retired `site.yaml` keys via comment-preserving line edits), `MANIFEST.json` sha256 integrity + allowlist overlay, and Pond one-click check/update from GitHub (`update.py`, stdlib only; repo `lamaBread/heron-press`) plus a CLI escape hatch (`--check`/`--migrate`/`--check-update`/`--update`). Build stays read-only on `user/`. dist byte-identical to v1.5.3 (57 files). |
...
*Heron v1.6.1 — build with Python + Pillow, runtime PHP (OPcache recommended). Full release history in [README.ko.md § 16](README.ko.md).*
