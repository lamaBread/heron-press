...
│   │   ├── i18n.py                 ← (v1.9.0) locale string lookup (Surface 1/3; ko canonical + parser)
...
│   ├── locales/                 ← (v1.9.0) i18n string packs (ko canonical + en; loaders = i18n.py / i18n.php)
...
|---|---|---|
| **v1.9.2** | 2026-06-02 | **Build console + build-report localization (v1.9.x language-pack line).** The builder's own operator output now resolves through the language pack: the 16-step progress labels (`[ n/16] …`), the start/done/cache milestones, the in-place per-article counters, the `[search]` tokenizer-parity lines, **and the entire `build-report.md`** (title, meta, progress, the per-step timing table, the to-fix/to-review/PHP-built headings + summary, and the system-fault abort suffix) — via `build.step.*` / `build.console.*` / `build.parity.*` / `build.report.*` in `build.yaml` (the builder uses its `tool_tr`; `report.py` / `search.py` use the global `i18n.t()`). So a tool=`en` build is **fully English**, closing the last always-visible Surface-3 gap (the build panel used to mix English warnings with Korean progress labels). `ko` values are byte-identical to v1.9.1, so the Korean console is unchanged except the `[search]` line (now Korean, matching the rest). Docs: the § 3 tree gains `i18n.py` / `locales/` / `i18n.php`; § 17-9 documents the parser escape rules and the full Surface-3 scope. Deferred by design (recorded in the roadmap): six `images.*` config-validation aborts + the tokenizer-parity-failure detail stay Korean (the rest of the abort *detail* surface is already English), and the per-article `_issue`/`_warning` *message bodies* are a separate, larger surface; the argparse `--help` text stays canonical Korean. No `user/` schema change → no new migration step (stamp 1.9.1 → 1.9.2); MANIFEST hashes refresh (86 files, count unchanged); dist byte-identical (the build console/report live outside `dist/`); 512 → 514 unit tests + 6/6 diagnostics. |
...
**Pack format.** `system/locales/<locale>/*.yaml` — one `key: "value"` per line, flat dotted keys. The fragment files in a folder (`admin.yaml`, `site.yaml`, `build.yaml`, `cli.yaml`) are merged. `ko` is canonical and the fallback for every locale; a missing key returns the **key string itself**, so the UI is never blank. `{name}` placeholders are substituted (e.g. `{n} results`). Inside a double-quoted value the parser interprets `\"` `\\` `\n` `\t` (any other `\x` keeps the backslash); single-quoted values are literal — so the PHP and Python loaders parse **byte-for-byte identically** (`system/admin/lib/i18n.php` / `system/scripts/i18n.py`, both dependency-free; a `test_i18n` parity test enforces it).

**The three surfaces.** (1) **visitor site chrome** — the *site language* (`site.yaml: lang`), baked into `dist/` at build; (2) the **Pond admin UI** — the *tool language*, via the PHP global `t()`; (3) **build/CLI operator messages** — the *tool language*, via the Python global `i18n.t()` (and the builder's `tool_tr`). Surface 3 covers the deploy/update/migration/CLI output and, since v1.9.2, the builder's own **16-step progress, milestones, summary, and the whole `build-report.md`** — so a tool=`en` install runs every operator surface in English. The one deliberate exception is the argparse `--help` text (a dev/CI reference, never shown in Pond), kept in canonical Korean.

**Adding a language.** Drop `admin.yaml`/`site.yaml`/`build.yaml`/`cli.yaml` into `system/locales/<code>/` and translate the `ko` keys (missing keys fall back to `ko` automatically). An English pack (`en`) ships in the box. Key parity between the `ko` source and other packs is enforced by `system/tests/test_i18n.py`.
