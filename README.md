# siheonlee.com v0.6.0 — 사용설명서 & 시스템 문서
...
> **v0.6.0 한 줄 요약:** v0.5.5 시점 한계 표의 두 항목을 일괄 해소. (1) **검색 인덱스 메타데이터 3-필드 색인 + 정적 PHP 인라인** — 본문 색인 폐기. `title` / `description` / `tags` 만 BM25 역색인. 인덱스가 `search-index.json` 대신 `dist/search.php` 안에 PHP 정적 배열 리터럴로 인라인되어 OPcache 가 바이트코드와 함께 캐시 (= 매 요청 시 JSON 파싱 / 디스크 IO 0). 스니펫 추출용 본문 평문은 글마다 앞 1500 자만 보존. 토크나이저·BM25 함수도 같은 파일에 인라인되어 dist 검색 산출물이 `search.php` 한 파일로 축약. 결정적 직렬화로 두 번 빌드해도 산출물 동일. (2) **단위 테스트 본격 확장** — `tests/test_bm25.py` 31 개 (v0.5.0 의 25 → BM25 v4 + 신규 가드 6 개) + `test_builder.py` / `test_markdown.py` / `test_seo.py` / `test_sitemap.py` / `test_feed.py` / `test_yaml_parser.py` / `test_slugs.py` / `test_report.py` / `test_parsedown.py` / `test_php_literal.py` 합 **179 개**. 부가적으로 `tests/run_diagnostics.py` (진단 스크립트) 가 단위 테스트 + 빌드 결정성 + PHP 구문 + Python↔PHP BM25 점수 패리티 + 인덱스 형식을 한 번에 검사하고 리포트 파일을 생성합니다. 자세한 내역과 이전 버전 (v0.5.5 ~ v0.1) 의 변경 사항은 [§ 18 업데이트 로그](#18-업데이트-로그) 참조.
...
이 폴더(`siheonlee.com_v0.6.0/`) 에서 터미널을 열고:
...
사이트 내 글 검색을 제공합니다. 클라이언트 JS 0줄, 외부 검색엔진 의존도 0. 한국어/영어 혼용 본문에서 부분 일치까지 잡아내는 자체 역색인 + 서버측 PHP **BM25 검색** 엔드포인트로 동작합니다.
...
> 아래 표는 v0.6.0 시점에 여전히 유효한 한계만 모았습니다. v0.6.0 에서 해소된 항목 (테스트 부분적 / 정적 검색 인덱스 본문 포함) 은 표에서 제거되었습니다 — 단위 테스트 179 개로 확장 + 진단 스크립트 신설, 그리고 본문 색인 폐기 + 정적 PHP 인라인 인덱스 도입으로 두 한계를 동시에 해소.
...
| 본문 검색 폐기 (v0.6.0) | 검색 색인이 글 본문 토큰을 더 이상 잡지 않습니다 — 본문에만 등장하는 단어로는 그 글을 찾을 수 없습니다. author 가 `title` / `seo.description` / `tags` 에 핵심 단어를 명시적으로 적어야 검색 가능. 본문 평문은 검색 결과 미리보기 스니펫 추출용으로 앞 1500 자만 보존. |
| Parsedown PHP↔Python 동등성 비교 트리 미동봉 (§ 12) | scripts/parsedown.py 는 PHP Parsedown 1.7.4 의 포트이며, 포팅 시점에 사용자가 별도로 동등성 검증을 거쳤습니다. v0.6.0 의 `tests/test_parsedown.py` 는 Python 포트의 기본 마크다운 요소만 회귀 가드하며 PHP 측 출력과의 대조는 포함하지 않습니다. PHP 원본을 끌고 다닐 의도가 없으므로 의도된 한계. |
...
| **v0.4.6** | (동일) | (동일) | + **페이지네이션 nav 여백 축소 + SSR 시점의 첫 페이지 정적 생성 (FOUC 제거)** | (동일) | (동일) | (동일) | (동일) | (동일) | + **`Articles/meta.yaml` (홈 = 루트의 페이지 설정 — per_page / excludes_categories / ...)** / + **카테고리·홈 meta.yaml 의 `priority` (정수, 큰 값 먼저)** / **site.yaml 의 home_* 류 키 폐기 (전역 ↔ 페이지 설정 분리)** | (동일) | (동일) | (동일) | (동일) | (동일) | (동일) | (동일) |
| **v0.4.5** | (동일) | (동일) | + **JS DOM 페이지네이션 컨트롤 (홈·카테고리 인덱스·서브카테고리 section). `<html lang>` 동적화** | (동일) | (동일) | (동일) | + **서브카테고리 URL 포함** | (동일) | + **글 `lang:`** / + **카테고리 meta.yaml (per_page / preview_per_page / layout / styles / lang)** | (동일) | (동일) | (동일) | (동일) | + **워닝 메시지에 슬러그 변환 결과 표기 + ASCII rename 권장** | + **assets/pagination.js** | (동일) |
| **v0.4.4** | (동일) | (동일) | (동일) | (동일) | (동일) | (동일) | **자동 생성 (글·톱레벨 카테고리·홈, noindex 제외, lastmod=updated\|date). robots.txt Sitemap 디렉티브 활성화** | (동일) | (동일) | (동일) | (동일) | (동일) | (동일) | (동일) | + **scripts/sitemap.py** | (동일) |
| **v0.4.3** | (동일) | (동일) | (동일) | **`{seo.title_prefix}{title}{seo.title_suffix}` 로 정상화** | + **섹션 마커 `===제목===` / `======`** | (동일) | 없음 (robots.txt 에 주석으로만 자리표시) | (동일) | **seo_* 평면 필드 → `seo:` 블록 그룹화** | (동일) | (동일) | (동일) | (동일) | (동일) | + **models.SeoMeta** | (동일) |
| **v0.4.2** | (동일) | (동일) | (동일) | (동일) | (동일) | + **검색 결과 페이지에 noindex,follow** | 없음 | (동일) | (동일) | (동일) | + **slug ↔ 카테고리 slug 충돌 차단** | **site.yaml 의 base_url 사용** | **balanced parser — nested parens + quoted `)` 정상 처리** | (동일) | (동일) | (동일) |
| **v0.4.1** | (동일 — 빌드 PHP 의존만 사라짐) | (동일) | (동일) | (동일) | (동일) | (동일) | 없음 | **scripts/parsedown.py — Parsedown 1.7.4 Python 포팅 (단일)** | (동일) | (동일 — PHP 없으면 워닝 후 건너뜀) | (동일) | (동일) | (동일) — 단순 정규식 (`[^)]*`) | (동일) | + scripts/parsedown.py | **Python 3 만 (빌드). PHP runtime 은 검색·리다이렉트용으로 여전 필요.** |
| **v0.4.0** | **"PHP 기반 경량 웹 사이트 생성기"** | (동일) | (동일) — nav-search 의 italic placeholder 제거 | (동일) | (동일) | **전역 noindex 제거. 글마다 `noindex: true` 로 개별 차단** | 없음 | (동일) | + **`noindex:`** / − **`seo_keywords:` 폐기** | **1글자 한국어 제외 (bigram 만). 본문 길이 절단 폐지. Py↔PHP 패리티 자동 검증** | + 토크나이저 패리티 | (동일) | (동일) | **_meta.yaml 오버라이드 폐기. hex 코드포인트 자동 변환 + 워닝** | **build.py + scripts/ 패키지** | (동일, 다만 캐치프레이즈로 솔직 표기) |
| **v0.3.2** | (동일) | (동일 — 점수 공식 동일, 카테고리 스코프만 추가) | (동일) — nav-search 만, 카테고리별 스코프 검색 | (동일) | (동일) | (동일) | 없음 | (동일) | (동일) | (동일) | (동일) | (동일) | (동일) | (동일) | 단일 build.py (2085줄) | (동일) |
| **v0.3.1** | "SSG (런타임 PHP 검색 포함)" | **신설 — 단순 TF 합산 + 매직 ×5 제목 부스트. 인덱스 v2** | (동일) + 모든 페이지 nav 우측 검색창 + Recent posts 위 inline 폼 | (동일) | (동일) | (동일) | 없음 | (동일) | (동일) | 1글자 한국어 그대로 인덱싱 | (동일) | (동일) | (동일) | (동일) | 단일 build.py | Python 3 + PHP CLI (빌드) + PHP runtime (검색) |
...
- 빌드 6 글 / 2 카테고리 / 0 abort. 두 번째 빌드도 동일 (dist 의 모든 파일 sha256 일치).
- 단위 테스트 179 / 179 통과 (BM25 31 + 비-BM25 148).
- 진단 스크립트 5/5 PASS (PHP 8.3 환경).
- Python↔PHP BM25 점수 8 쿼리 × 12 매치 = 모두 1e-6 이하로 일치.
- dist 산출물 크기: v0.5.5 의 4 파일 합계 ~33 KB → v0.6.0 의 search.php 단일 ~28 KB (-15%).
- BM25 시맨틱 회귀 0 — 6 개 회귀 가드 케이스 (긴 글이 짧은 글을 압도하지 않음 / 흔한 한글 bigram 이 희귀 영문 토큰과 동등 가중되지 않음 / tags substring 이 정확매치 부스트를 받지 않음 / 본문에만 있는 단어는 v4 에서 점수 0 / 결정적 키 순서 / phrase boost 가 0 점수에 적용되지 않음) 통과.

**적용 메모:**

- **글 측 작업 의무**: 옛 글의 meta.yaml 변경 의무 없음. **단**, v0.6.0 의 검색은 본문이 아니라 title/description/tags 만 보므로, 어떤 글이 본문에만 핵심 단어를 가지고 있다면 author 가 `seo.description` 이나 `tags` 에 그 단어를 명시해야 검색됨. v0.5.5 의 `seo.description` 필수화 (= 누락 시 리포트) 와 자연스럽게 결합.
- **점진 빌드**: v0.5.5 dist 위에 그대로 빌드 가능 — `_build_search` 가 옛 `search-index.json` / `search_tokenize.php` / `search_bm25.php` 를 자동 정리.
- **PHP OPcache 활성화 권장**: OPcache 가 꺼져 있어도 정상 동작하지만, 매 요청마다 search.php (인덱스 포함) 가 재파싱된다. `opcache.enable=1` 만 두면 인덱스가 바이트코드 캐시에 상주.
- **외부 의존성**: v0.5.5 와 동일 (Pillow + `images.enabled=false` 옵션 그대로).
- **참조 도구**: 사용자는 터미널에서 언제든 `python tests/run_diagnostics.py` 로 현재 상태 리포트 생성 가능. 빌드 / 단위 테스트 / 결정성 / Python↔PHP 점수 패리티 / 인덱스 형식이 한 번에 점검됨.

**미수용 (차기 의제):**
- 홈/카테고리 페이지의 SEO 메타 태그 출력 (§ 17 한계 표의 잔존 항목).
- 태그별 색인 페이지 / 검색 결과의 카테고리·태그 필터 UI (v0.5.3 미수용 그대로 이월).
...
- About 의 face_img.png → `dist/about/face_img-400.webp` (8,948 bytes) + `dist/about/face_img-778.webp` (33,414 bytes). 글의 `dist/about/index.html` 과 같은 폴더에 동거.
- About index.html 의 `<img>` src 가 `/about/face_img-778.webp` 로, srcset 도 `/about/face_img-{400,778}.webp` 로 출력.
- 옛 `dist/src/` 트리가 존재하지 않음 (`_prune_orphans` 정리 또는 `--clean` 으로 시작).
- BM25 단위 테스트 25개 그대로 통과 (회귀 0 — 검색·인덱스 모듈은 자산 경로와 무관).
- 비-이미지 산출물 (검색, sitemap.xml, robots.txt, redirect.php, 404.html, 페이지 HTML 구조) 은 v0.5.1 과 동등.

**적용 메모:**

- 글마다 별도 작업 없음. 본문의 `![alt](./imgs/photo.jpg)` 나 `<img src="./photo.png">` 그대로 둬도 빌드가 새 URL 스킴으로 출력.
- 글의 meta.yaml / Articles/meta.yaml / 카테고리 meta.yaml / site.yaml 옛 키 모두 변경 의무 없음.
- v0.5.1 dist 위에 그대로 빌드를 돌려도 `_prune_orphans` 가 옛 `dist/src/` 트리를 정리합니다. 재현성 확보가 필요하면 `python build.py --clean` 권장.
- 외부 의존성은 v0.5.1 과 동일 (Pillow). `images.enabled=false` 로 끄면 stdlib 만으로 빌드.
...
- About 페이지의 face_img.png (1,090,494 bytes / 778×W) → dist/src/about/face_img-400.webp (8,948 bytes) + face_img-778.webp (33,414 bytes). 원본 PNG 대비 약 30× ~ 120× 절감.
- dist/src/about/index.html 의 `<img src="./face_img.png" alt=''>` 가 후처리 후 `<img src="/src/about/face_img-778.webp" alt='' srcset="/src/about/face_img-400.webp 400w, /src/about/face_img-778.webp 778w" sizes="(max-width: 800px) 100vw, 800px" loading="lazy">` 로 변환됨.
- 두 번째 빌드 (캐시) — 변종 mtime 이 첫 빌드와 동일 (재인코딩 건너뜀 정상 동작).
- 외부 URL `<img src="https://...">` — src 그대로 + loading="lazy" 만 추가.
- SVG `<img src="/assets/icon.svg">` — src 그대로 + loading="lazy" 만 추가.
- 기존 BM25 단위 테스트 25개 그대로 통과 (회귀 0).

**호환성 노트:**

- v0.5.0 까지의 dist 와 v0.5.1 의 dist 는 **검색 / sitemap.xml / robots.txt / redirect.php / 404.html / 페이지 HTML 구조 모든 측면에서 동일.** 차이는 ① 본문의 `<img>` 마크업이 webp + srcset + lazy 로 확장 (외부 URL / SVG 는 loading 한 줄만), ② raster 이미지 자리에 원본 대신 webp 변종 파일.
- meta.yaml / Articles/meta.yaml / 카테고리 meta.yaml 변경 불필요.
- site.yaml 의 옛 키 변경/추가 의무 없음. `images:` 블록을 명시하지 않으면 v0.5.1 기본값으로 동작.
- 외부 의존성: 빌드 머신에 `pip install Pillow` 가 새로 필요. 회피하려면 site.yaml 에 `images: { enabled: false }` 한 줄을 추가.

#### v0.5.0 → v0.5.1 마이그레이션 시 주의

- `pip install Pillow` (또는 `pip install -r requirements.txt` — 다만 이 프로젝트는 의존성 1개라 requirements.txt 를 도입하지 않음. 다음 의존성 추가 시 함께 도입 검토).
- 첫 `python build.py --clean` 후 dist 의 raster 이미지 자리가 webp 변종으로 바뀜. 옛 dist 를 그대로 두고 partial 빌드를 돌리면 dist/src 안에 옛 jpg/png 와 새 webp 가 공존할 수 있으니, 첫 빌드는 `--clean` 권장.
- Apache `mod_mime` 가 `.webp` 의 MIME 타입을 정상 인식하는지 확인 (대부분의 Apache 2.4 디폴트 mime.types 에 포함). 미인식 시 `AddType image/webp .webp` 한 줄 추가.
- 이미지 변종 너비를 조정하고 싶다면 site.yaml 의 `images.widths` 수정 후 재빌드 — `_prune_article_assets` 가 옛 너비 변종을 정리한다.
- v0.4.2 로드맵의 미수용 B-5 (description 폴백 추출 범위) / B-6 (description_truncate 단어 경계) 는 v0.5.1 범위 밖. 별도 의제로 이월.
...
**v0.4.x → v0.5.0 결함 해소표** ([§13 의 결함 해소표](#13-검색-기능--searchphp) 참조).

**호환성 노트:**

- 검색 인덱스 포맷이 v2 → v3 으로 비호환 변경. **반드시 v0.5.0 의 build.py 로 새 dist/ 를 생성** 해야 함. 기존 dist/ 의 v2 search-index.json 으로 v0.5.0 search.php 를 띄우면 500 응답.
- meta.yaml / site.yaml / Articles/meta.yaml / 카테고리 meta.yaml 변경 불필요 — 콘텐츠 측 스키마는 v0.4.7 과 동일.
- 글 페이지 HTML 산출물, 홈 / 카테고리 / 404 / sitemap.xml / robots.txt / redirect.php 는 v0.4.7 과 출력이 동일 (검색 외 변경 없음).
- 클라이언트 페이지/스타일 변동 없음. 검색 결과 페이지의 마크업도 동일 — 변하는 것은 결과 순서와 스니펫 구간 뿐.

#### v0.4.7 → v0.5.0 마이그레이션 시 주의

- 검색이 살아나려면 `python build.py` 로 새 인덱스를 만들어 dist/ 를 다시 배포해야 합니다 (v2 인덱스를 그대로 두면 search.php 가 500 을 반환).
- v0.4.2 로드맵의 미수용 B-5 (description 폴백 추출 범위), B-6 (description_truncate 단어 경계) 는 여전히 미수용 — 검색과 무관한 SEO 폴백 항목이므로 별도 의제로 이월.
- BM25 하이퍼파라미터 (k1/b/w_title/phrase_boost_*) 는 인덱스 안의 `params` 블록으로 박혀 있어 빌드마다 결정적. 튜닝이 필요하면 `scripts/search.py` 의 `BM25_PARAMS` 를 수정 후 재빌드 — 점수 함수에 직접 인자로 전달하는 것이 아니라 인덱스 안에 박히므로 dist/ 만 재배포해도 새 점수가 반영됨.
...
*이 문서는 siheonlee.com v0.6.0 (PHP 기반 경량 웹 사이트 생성기 — 빌드는 Python + Pillow, 런타임은 PHP; 검색은 Okapi BM25 메타데이터 3-필드 색인 + PHP 정적 배열 인라인; 이미지는 WebP 다중 해상도 자동 변환) 기준으로 작성되었습니다. (2026-05-15)*
