...
|---|---|---|
| **v1.9.7** | 2026-06-03 | **Default tool + site language flipped Korean→English + locale tooling (v1.9.x language-pack line).** The i18n canonical/default/fallback locale changed `ko`→`en` (`system/scripts/i18n.py` `CANONICAL`, `system/admin/lib/i18n.php` `I18N_CANONICAL`) — when `user/.heron/locale` is absent the tool UI (Pond admin + build/CLI) now defaults to **English**, and a missing key in any pack falls back to the universally readable **English** value (was Korean). An existing install that already chose a language keeps it (the 1.8.0→1.9.0 migration's `ko` seed is unchanged, so pre-1.9.0 upgraders stay Korean); an install that never chose one now defaults to English — pick a language in **Pond ▸ Settings** (`?a=settings`). The visitor-site `lang` default also flipped `ko`→`en` (the code default + the bundled demo `user/site.yaml: lang: en`), so `<html lang>`, search, footer, pagination, 404, and home/category labels default to English; but this is *visitor-site content config* (`site.yaml`) that self-update never overwrites, so it only affects fresh clones / the published demo (the demo's per-article i18n example now has the site default English and that one article overriding to `lang: ko` to keep demonstrating the per-article override). Two new commands: `python Heron.py --check-locale [code]` (read-only diagnostic — reports, versus the English canonical, missing [English fallback] / extra / stray-backslash / untranslated, with a non-zero exit on gaps so it gates CI) and `python Heron.py --new-locale code` (scaffolds by copying the English canonical verbatim + injecting the endonym key, so it passes parity and lists in the dropdown immediately; the code is regex-validated against path traversal). No `user/` schema change → no new migration step (stamp 1.9.6 → 1.9.7). |
...
*Heron v1.9.7 — build with Python + Pillow, runtime PHP (OPcache recommended). Full release history in [§ 16](#16-changelog).*
