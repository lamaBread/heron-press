...
|---|---|---|
| **v1.10.0** | 2026-06-04 | **Deploy hotfix — corrected a nonexistent rclone flag name.** `deploy.py`'s `build_argv` passed `--sftp-known-hosts` (a flag rclone **never had**) for host-key verification, so rclone rejected it at the **flag-parsing stage** — dying before any server connection. Since the dry-run preview and the real apply build the same argv, **the entire deploy feature was inoperable**. The correct name is `--sftp-known-hosts-file` (the CLI form of the rclone backend option `known_hosts_file`; verified against the pinned v1.74.2 binary) — a constant wrong from the start (since v1.7.0), not version drift. The unit tests missed it because they **asserted the same typo**, staying self-consistent with the bug. The fix touches the `deploy.py` flag + docstring, the two `test_deploy.py` assertions, and the README security notes. **A new recurrence guard** (`TestArgvFlagsAgainstRealRclone`) checks every `--flag` that `build_argv` emits against the real rclone `help flags` surface, actually detecting a "string the code builds ↔ flag rclone accepts" mismatch (skips when no binary is present). No `user/` schema change → no new migration step (stamp 1.9.7 → 1.10.0); MANIFEST 88; 531 → 532 unit tests. |
...
*Heron v1.10.0 — build with Python + Pillow, runtime PHP (OPcache recommended). Full release history in [§ 16](#16-changelog).*
