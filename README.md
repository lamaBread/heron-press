...
|---|---|---|
| **v1.14.10** | 2026-06-13 | **Recovers the release finalization that v1.14.8 and v1.14.9 skipped — regenerating `system/MANIFEST.json` to restore the self-update it had broken, and backfilling the missing changelog.** Both releases committed code but skipped regenerating the manifest, leaving the integrity manifest at v1.14.7 (89 files, stale hashes, the new `pagemeta.php` unregistered). Self-update sha256-checks the downloaded tree against its own MANIFEST and overlays only when `ok` ([update.py](system/scripts/update.py) integrity gate), so both tags failed verification and self-update was *refused* — and because self-update always targets the latest tag, this single release with a correct MANIFEST recovers everyone (jumping straight past the broken 1.14.8/1.14.9 tags). This release rehashes every file via `make_manifest`, bumping MANIFEST 89 → 90 (version 1.14.10, `pagemeta.php` registered), and backfills the missing v1.14.8/v1.14.9 changelog (ko/en) plus the title/footer version. Docs and manifest only — no code change, builder and dist output byte-identical. No schema change (stamp 1.14.9 → 1.14.10); MANIFEST 89 → 90 (regenerated); 607 unit tests. |
...
*Heron v1.14.10 — build with Python + Pillow, runtime PHP (OPcache recommended). Full release history in [§ 16](#16-changelog).*
