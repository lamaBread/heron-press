...
|---|---|---|
| **v1.9.6** | 2026-06-03 | **Demo-config comments unified to English (v1.9.x language-pack line).** The Korean comments in the shipped demo config are unified to English — unlike the i18n packs, `user/*.yaml` comments are static files that cannot follow the runtime tool language (`user/.heron/locale`), so they are settled on English so they read for an `en` or `ko` tool-language user alike. The 18 `user/articles/**/meta.yaml` files were already English, leaving only `user/site.yaml` — the single user yaml with Korean comments — to translate (comment text only; config values, structure, key order, CRLF, and UTF-8 no-BOM unchanged). The locale packs (`system/locales/`) are **left untouched on purpose** — each pack self-documenting in its own language (`ko`↔`ko`, `en`↔`en`) is the within-language consistency. No `user/` schema change → no new migration step (stamp 1.9.5 → 1.9.6); MANIFEST 86 (count unchanged, only the `__init__` hash refreshes); dist byte-identical; 519 unit tests. Self-update never overwrites `user/`, so this lands for fresh clones (an existing install's `site.yaml` is its own content and is preserved). |
...
*Heron v1.9.6 — build with Python + Pillow, runtime PHP (OPcache recommended). Full release history in [§ 16](#16-changelog).*
