# siheonlee.com v0.3.1 — 사용설명서 & 시스템 문서
...
| **사이트 내 검색** — 클라이언트 JS 없이 한국어 친화 부분검색 (v0.3.1) | 빌드 시 `dist/search-index.json` (한글 bigram + 영문 토큰 역색인) + `dist/search.php` 동시 생성. 모든 페이지 nav 우측에 작은 검색창. |
...
13. [검색 기능 — search.php (v0.3.1)](#13-검색-기능--searchphp-v031)
...
- **모든 페이지의 nav 우측 상단** — 작은 input 1개. focus 시 가로폭이 살짝 늘어나며 #0172d5 (lama 의 링크 색) 로 테두리가 바뀜.
- **메인 페이지 section 상단** — `[input] [검색]` 의 inline 폼. nav 의 작은 입력보다 발견하기 쉬운 위치.
- **검색 결과 페이지 (`/search.php?q=...`)** — 기존 글 목록 (`listup_module_div`) 과 동일 마크업으로 결과를 표시. 각 결과 아래에 매치 위치 ±40 글자 스니펫. 매치된 부분은 `<mark>` (배경 #fff3cd) 로 강조.
- **모바일 (≤600px)** — nav 의 검색창은 숨김. 사용자는 Home 으로 가서 메인 페이지의 inline 폼 사용.
...
```json
{
  ...
  "docs": [
    {"slug":"hello-world","title":"Hello, World!","date":"2026-05-07",
     "category":"Blog","body":"평문 본문 (최대 5000자)..."}
  ],
  "index":       {"마스": [[0, 3]], "프린": [[0, 1]], ...},
...
- `docs[doc_id]` — 표시용 메타데이터 + 스니펫 추출용 평문.
...
`templates/search.php` 파일을 삭제하면 build.py 가 경고만 출력하고 인덱스 파일은 만들지만 search.php 는 만들지 않습니다. 또한 템플릿의 nav-search/home-search 를 제거하면 UI 도 사라집니다.
...
| 영역 | v0.1 | v0.2 | v0.3 | v0.3.1 (현재) |
|---|---|---|---|---|
| 출력 UI/UX | v0.1 자체 디자인 | 원본 `lama_website-main` 와 동일 | 원본 + 글마다 스타일 오버라이드 가능 | (동일) + 모든 페이지 nav 우측 검색창 |
| 마크다운 파서 | Python stdlib 자체 파서 | (동일) | **Parsedown.php (PHP CLI)** — 자체 파서는 fallback | (동일) |
| meta.yaml 필드 | slug, title, date, seo_* | (동일) | + **`styles:` (글 단위 CSS 오버라이드)** | (동일) |
| YAML 파서 | flat 매핑만 | (동일) | + **nested mapping (styles 트리용)** | (동일) |
| 검색 | — | — | — | **dist/search.php + search-index.json (한글 bigram 역색인)** |
| 외부 의존성 | Python 3 | (동일) | Python 3 + (parsedown 사용 시) PHP CLI | Python 3 + PHP CLI (빌드) + PHP runtime (검색) |
...
*이 문서는 siheonlee.com SSG v0.3 기준으로 작성되었습니다. (2026-05-09)*
