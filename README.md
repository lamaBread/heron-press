...
|---|---|---|
| **v1.9.4** | 2026-06-02 | **Per-article content-defect `_issue`/`_warning` localization (v1.9.x language-pack line).** The Korean-hardcoded diagnostic bodies shown only when an article/category is defective during a build (meta.yaml type / `template` / `seo` / `tags` fallbacks, slug regex/reserved/duplicate/conflict, date·updated format, missing required fields, missing/empty `seo.description`, missing content, broken internal links — 29 sites in builder) plus the image-encoding-failure message (images.py) move to `build.issue.*` (28 keys), `build.label.*` (2 keys — the `template`-not-found location label), and `build.warn.image_optimize_failed` (the builder's `self.tool_tr`; images.py uses the global `i18n.t()`). `{received!r}` preserves the original f-string `{x!r}` repr, and the reserved-slug's dynamic system path is precomputed into a parameter. Now even *conditional* operator output is consistent in the tool language — the only Korean left is intentional (the Pond 403 guard, which runs before the i18n bootstrap so `t()` is unavailable, and the site-chrome English values preserved by user decision). `ko` values are byte-identical, so Korean output is unchanged. No `user/` schema change → no new migration step (stamp 1.9.3 → 1.9.4); MANIFEST hashes refresh (86 files, count unchanged); dist byte-identical (the diagnostic bodies live outside `dist/`); 517 → 519 unit tests + 6/6 diagnostics. |
...
*Heron v1.9.4 — build with Python + Pillow, runtime PHP (OPcache recommended). Full release history in [§ 16](#16-changelog).*
