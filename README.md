...
> **v1.0.1 한 줄 요약:** *소분류 헤더 링크화 — 우측 → 화살표 폐지, 분류명 글씨 자체가 그 소분류 페이지로 가는 깔끔한 링크 (기능 릴리스).* 톱레벨 카테고리 페이지(예: `/blog/`)의 자식 소분류 section 헤더가 v1.0.0 까지는 `소분류명 <a class='more-link'>→</a>` (이름은 plain text, 우측에 가는 화살표 링크) 였다. v1.0.1 은 화살표를 없애고 **소분류명 글씨 자체**를 그 소분류의 자기 페이지(`/{top}/{sub}/`)로 가는 a 태그로 — 글씨를 클릭하면 그 소분류 글만 보인다. 링크는 본문 글씨와 똑같이 보이도록 스타일을 비웠다(`.gap .subcat-link`: `color: inherit; text-decoration: none;`, 호버 효과 없음 — 클릭 암시는 브라우저 기본 포인터 커서뿐). `_render_section` `more_url` 분기 + `common_template.css` 의 `.more-link` 3 룰 → `.subcat-link` 1 룰. `more_url` 없는 section(카테고리 직속 글)·홈의 정적 "Recent posts" 갭은 무영향. **코드 릴리스** — 무결성은 정본 Articles 고정 후 v1.0.0 *코드* 클린 재빌드(불변 v1.0.0 손대지 않는 4번째-숫자 검증 복사본 `siheonlee.com_v1.0.0.1`) vs v1.0.1 클린 재빌드의 열거 diff: **5 파일만 변경** = `assets/common_template.css` + `blog`·`project`·`research`·`study`/`index.html` (자식 소분류를 둔 4 톱레벨 카테고리), 0 added/0 removed, 781 byte-동일(786=786). 홈·소분류 말단 페이지·`feed.rss`/`feed.atom`·`sitemap.xml`·`search.php`·글 페이지 전부 byte-불변, 클린 빌드 2회 결정성 동일(combined sha256 `bac1e2c6…`). `__version__` 1.0.0→1.0.1 의 dist 누수 0 (B1). 단위 313 승계 · 진단 6/6. 자세한 내역은 [§ 16](#16-업데이트-로그).
...
```
빌드 시작 - siheonlee.com v1.0.1 (2026-05-19 03:00:00)
...
```
siheonlee.com_v1.0.1/        ← 보이는 것은 아래 6 개뿐
│
...
**현재 한계 (v1.0.1)** — 두 부류. ⓐ 현 능력의 *내재적* 한계, ⓑ *의도적으로 보류한* 확장. **둘 다 현 상황에서는 도입하지 않는다** (사용자 합의 2026-05-18 — 사유가 유효한 동안 유지, 필요해질 때 재검토).
