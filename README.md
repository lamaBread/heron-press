...
> **v1.0.0 한 줄 요약:** *첫 정식 릴리스 — 기본 og:image 실파일 + `About` 검색 비허용 (기능 릴리스).* (1) `site.default_og_image` 가 가리키는 자산은 webp 변환·variant 등록을 건너뛰고 **원본 그대로** dist 에 — og:image 소비자는 SNS 링크 언퍼ler 라 고정 URL 하나만 가져가고(다중 해상도 무의미), KakaoTalk·일부 Facebook 은 WebP og:image 를 못 렌더하며, `resolve_og_image` 가 문자열 그대로 써 변환 시 그 URL 이 404 다. 실제 `default-og.png`(1200×480, 도메인 워드마크) 동반 — v0.8.4 까지는 site.yaml 이 `/assets/default-og.png` 를 가리키되 파일이 없어 본문 이미지 없는 모든 페이지의 og:image 가 죽은 404 였다 (이번에 해소). (2) `About` 글 `noindex: true` — robots noindex + sitemap.xml·검색 인덱스에서 제외 (피드는 최신 20개 윈도우라 date 2025-01-01 인 About 은 애초에 미수록 — noindex 가 피드엔 no-op). **코드 릴리스** — v0.8.4 dist 대비 **실측** `+assets/default-og.png`, Δ `about/index.html`·`sitemap.xml`·`search.php` (786 vs 785 파일); `feed.rss`/`feed.atom` 포함 그 외 전부 byte-불변, 클린 빌드 2회 결정성 동일. `__version__` 0.8.4→1.0.0 의 dist 누수 0 (B1). 단위 313 승계. 자세한 내역은 [§ 16](#16-업데이트-로그).
...
*siheonlee.com v1.0.0 — 빌드 Python + Pillow, 런타임 PHP. 검색 Okapi BM25 메타데이터 3-필드 인라인 인덱스, 이미지 WebP 다중 해상도 자동, 글/홈/카테고리 통일 SEO 메타 + 외부 CSS·`template:` 지원, schema.org JSON-LD (글 페이지 `Article`+`BreadcrumbList`, 기존 메타 태그 보강·off 스위치 동반). 빵부스러기는 정확한 의미로 출시 — 중간 조상=자기 중첩 URL, 글 말단=글 제목 (nav-tracker/JSON-LD 단일 공유 소스); JSON-LD 의미 게이트 [6] 동반. v1.0.0 첫 정식 릴리스 — 기본 og:image 자산 패스스루(원본 그대로, SNS 언퍼ler 용; `default-og.png` 동반) + `About` 검색 비허용(robots noindex + sitemap·search 제외; About 은 feed 최신 20개 윈도우 밖이라 피드 byte-불변); v0.8.4 dist 대비 실측 열거 diff(786 vs 785)·그 외 byte-불변. (2026-05-19)*
