...
| v1.11.3 | 2026-06-04 | Localize the article-end published/updated meta line to the site language (v1.9.x localization-drift fix) — the labels ("발행"/"수정") on the article-end published/updated meta line introduced in v1.4.0 were hardcoded Korean and were missed by the v1.9.x site-language sweep — so building with `lang=en` (the default since v1.9.7) left **this one footer in Korean while the rest of the chrome was English** (every article footer in the demo dist read "2026-… 발행"). `_render_article_end_meta` now routes through `self.site_tr` (site language, substituted at build time) like the rest of the visitor chrome, with new `site.article.published`/`updated` keys (`{date}` placeholder) added to the en·ko packs. Regression guards: per-locale label assertions (2 in `test_v140`) + placeholder substitution (1 in `test_i18n`); key parity is enforced by the existing `test_every_locale_matches_canonical_key_set`. **Enumerated diff = the article-page footer `발행/수정`→`Published/Updated` in `lang=en` (non-ko) builds only — `lang=ko` is byte-identical** (the locale value matches the old hardcoded string); since `__version__` feeds the cache `global_hash`, the version bump invalidates the article cache so no stale footer survives an incremental rebuild. No schema change → no new migration step (stamp 1.11.2 → 1.11.3); MANIFEST 88; 543 → 546 unit tests. |
| v1.11.2 | 2026-06-04 | rclone PATH-fallback integrity warning (security backlog B3 — closes the v1.11.x security arc) — when the pinned-binary download fails and `rclone_bin.ensure()` falls back to an `rclone` on `PATH`, that binary runs **without sha256 verification** (a sha256 *mismatch* on a download still hard-aborts with no fallback — unchanged), so `cli.rclone.fallback_path` now states the binary is **UNVERIFIED / integrity not checked** and points to the remedy instead of a benign note (message-only, call site unchanged; guard `test_fallback_warns_binary_is_unverified`). No schema change (stamp 1.11.1 → 1.11.2); MANIFEST 88; 542 → 543 unit tests. |
