...
|---|---|---|
| **v1.14.11** | 2026-06-13 | **Adds an author-time release-integrity self-check so the v1.14.8/v1.14.9 class of breakage — committing code without regenerating the manifest or refreshing the version references — fails loudly at release time instead of silently refusing every user's self-update.** Two files ship: the check itself, `system/scripts/check_release.py`, exposed as a first-class CLI action `python Heron.py --release-check`; and `system/tests/test_manifest_sync.py`, which rides the existing `unittest discover` suite so a drifted tree can't even pass tests. The check has two halves. (1) It recomputes the manifest from disk and compares it *whole* against the committed `system/MANIFEST.json`, flagging any added, removed, or changed file and any version mismatch — exactly what slipped past the older `make_manifest.verify()` in v1.14.8 (an untracked `pagemeta.php`, stale hashes, and a stale version all passed its narrower `ok`). (2) It confirms `README.md` / `README.ko.md` agree with `__version__` at the title, the footer, and the top changelog row (which must also be bold). Tooling only — the builder and dist output are byte-identical. No schema change (stamp 1.14.10 → 1.14.11); MANIFEST 90 → 92 (regenerated, +`check_release.py` +`test_manifest_sync.py`); 607 → 617 unit tests. |
...
*Heron v1.14.11 — build with Python + Pillow, runtime PHP (OPcache recommended). Full release history in [§ 16](#16-changelog).*
