...
> **v1.3.0 한 줄 요약** — 빌드 속도 향상 묶음 릴리스. 이미지 최적화 멀티프로세스화 (5단계 cold 250s→32s) + 토크나이저 parity 캐시 (1단계 warm 3.3s→0.17s) + 단계별 timing 계측 + 자산 패스 통합. 실측: warm 5.2s→2.7s, cold 270s→33s. dist 산출물은 v1.2.2 와 byte-완전 동일 (코드 릴리스이되 dist byte-불변형). 누적 기능은 [§ 16](#16-업데이트-로그) 표 참조.
...
```
siheonlee.com_v1.2.2/        ← 보이는 것은 아래 7 개뿐 (v1.1.0 부터 admin.php 포함)
│
...
**현재 한계 (v1.2.2)** — 두 부류. ⓐ 현 능력의 *내재적* 한계, ⓑ *의도적으로 보류한* 확장. **둘 다 현 상황에서는 도입하지 않는다** (사용자 합의 2026-05-18 — 사유가 유효한 동안 유지, 필요해질 때 재검토).
...
| 버전 | 날짜 | 요약 |
|---|---|---|
| **v1.3.0** | 2026-05-28 | **빌드 속도 향상** (코드 릴리스이되 dist byte-불변) — 네 항목 묶음 (A·B·D·E). C 는 ROI 미흡 보류. (A) **단계별 timing 계측** — `Builder._step` / `_step_close` 가 단계별 경과시간을 `self._step_times` 에 누적, `_write_build_report` 가 `build-report.md` 에 16 단계 시간 표 직렬화 (리포트는 dist 밖이라 결정성 무관). (B) **이미지 최적화 멀티프로세스** — `_sync_assets` 를 세 패스로 분리(분류 → raster 변환 → 비-raster 복사 + prune). raster 변환을 `ProcessPoolExecutor(workers=min(cpu_count, len(jobs)))` 로 fan-out, 워커는 모듈-레벨 자유 함수 `_image_worker`(Windows spawn 도 OK). 결과 처리(`image_variants` 등록·에러 BuildReport 라우팅)는 메인 `_handle_image_result` 헬퍼로. raster_jobs<4 또는 worker≤1 이면 시리얼 폴백(Windows Pillow import 비용 절약). (D) **자산 패스 통합** — `_prune_article_assets` 의 src `rglob` 중복 패스 제거. 분류 단계에서 `article_expected` set 까지 같이 채우고 prune 은 dst `rglob` 한 번만 비교. raster overrun width 변종은 `_handle_image_result` 가 실제 `variants.widths` 를 expected 에 추가해 보존. (E) **tokenizer parity 캐시** — `search.run_parity_test` 결과를 `.build_cache/parity.json` 에 캐싱. 키 = `sha256(search.py + search_tokenize.php + php -v 첫 줄)`. hit 시 18 fixture PHP subprocess 호출(~3s) 건너뜀, `--no-cache` 시 매 빌드 풀 검증. E 두 번째(`compute_global_hash` mtime+size 캐시)는 추정 절감 50ms 미만이라 보류. 실측 (`siheonlee.com_v1.2.2/dist` baseline, 47 글 · 227 이미지): **cold 270.1s → 33.1s (-88%), warm 5.2s → 2.7s (-48%)**. 무결성 = **코드 릴리스 형이되 dist byte-불변** 형 (정본 Articles + 같은 site.yaml 기준 — v1.2.0/v1.2.1 패턴): v1.2.2 shipped `dist/` 와 v1.3.0 클린 재빌드 `dist/` 가 **787=787 file, 0 added/0 removed/0 changed = byte-완전 동일** (v1.2.2 의 [2] 결정성 통과를 자기 클린 재빌드 동일성의 강한 증거로 사용 — v1.2.2.x 4번째-숫자 검증 복사본을 따로 만들지 않고 shipped dist 동일성으로 갈음). 클린 빌드 2회 결정성 동일 (진단 [2] PASS). `__version__` 1.2.2→1.3.0 dist 누수 0 (B1 유지). 단위 374→**3xx** (`StepTimingTests` + `ImageWorkerTests` + `ParityCacheTests` 신설) · 진단 **6/6** 승계 ([6] ld+json 47/46 그대로). 사용자 결정 보류: C (글 렌더 멀티코어) — 글 렌더 1.47s 의 멀티코어화 vs 광범위 self-메서드 리팩터의 ROI 미흡 ([[feedback_deferred_extensions]] ③ 유지). |
| v1.2.2 | 2026-05-21 | **`yaml_parser` 멀티라인 inline list 지원** (코드 릴리스, dist 의도 변경) — v0.4.0 의 inline-list 분기가 `val.startswith('[') and val.endswith(']')` 단일 조건이라 같은 줄에 `]` 가 없는 multi-line 형이 silently 스칼라 `'['` 로 파싱되고 정규화 단계의 `frozenset(['/['])` 로 떨어져 매칭 0건이었던 버그. v1.1.5 의 `exclude_urls` 를 multi-line 형(`exclude_urls: [\n  '/',\n  ...\n]`)으로 적은 정본 site.yaml 에서 26 URL 차단이 dormant. 헬퍼 `parse_inline_list_multiline(first_val)` 신설 — `[` 로 시작하고 같은 줄에 `]` 가 없으면 후속 줄을 누적해 `]` 가 나오는 줄에서 종료, 줄 단위 trailing comma·단독 주석 줄 허용 (트레일링 inline 주석 비지원). 두 호출 사이트(top-level loop · `parse_block_map`) 를 `startswith('[')` 단일 검사로 통일, `endswith(']')` 여부로 single/multi-line 분기. v0.4.0 의 single-line inline list 케이스는 동치 분기 그대로(모든 기존 yaml 호환). 무결성 = **코드 릴리스 형이되 dist *의도된* 1줄 변화 형** (정본 `siheonlee.com_v1.2.0.1/Articles` 고정, site.yaml 도 v1.2.1 의 service-tuned 본을 그대로 승계 — `exclude_urls` 의 multi-line 형도 v1.2.1 baseline 과 동일). 비교 baseline = `siheonlee.com_v1.2.0.1/dist` 와 *같은 yaml* 의 v1.2.2 빌드. dist 변화: 26 페이지의 자동광고 로더 한 줄이 마침내 사라진다 (v1.2.1 까진 파싱 실패로 dormant). 그 외 자산·feed·sitemap·검색 인덱스 등은 byte-불변. 클린 빌드 2회 결정성 동일. `__version__` 1.2.1→1.2.2 dist 누수 0 (B1 유지). 단위 367→**374** (`MultilineInlineListTests` 7 신설: 각 줄 한 항목·마지막 줄 `]` 동행·주석 줄 허용·빈 리스트·줄당 다중 항목·후행 키 연결·중첩 맵 안 사용) · 진단 **6/6** 승계. |
...
*siheonlee.com v1.2.2 — 빌드 Python + Pillow, 런타임 PHP. 검색 Okapi BM25 메타데이터 3-필드 인라인 인덱스, 이미지 WebP 다중 해상도 자동, 글/홈/카테고리 통일 SEO 메타 + 외부 CSS·`template:` 지원, schema.org JSON-LD (글 페이지 `Article`+`BreadcrumbList`, off 스위치 동반), 로컬 글쓰기 `admin.php` (§ 17), Google AdSense 통합 + URL 기반 광고 차단 (페이지별 canonical URL 정확 일치, 개별 글 차단 가능). `noindex: true` 글은 description 필수 검사 면제 (v1.2.1). `yaml_parser` multi-line inline list 지원 (v1.2.2 — `exclude_urls` multi-line 형 정상 발효). 누적 릴리스 내역은 [§ 16](#16-업데이트-로그). (2026-05-21)*
