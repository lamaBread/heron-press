...
|---|---|---|
| **v1.11.0** | 2026-06-04 | **Security-audit round — only the confirmed findings from a multi-agent review (public search display bug + local hardening).** Reflects only what was reproduced firsthand from a parallel 4-domain audit (public runtime PHP, build pipeline, local admin, deploy/update). **(A1)** `search.py`'s `html_to_plain` stripped tags but left HTML entities, so body content with escaped code (e.g. `&lt;html&gt;` in a code fence) kept `&lt;` in the snippet and the runtime `highlight_html()` `htmlspecialchars` **double-encoded** it to `&amp;lt;` — surfacing a literal `&lt;` on the public search results. Added `html.unescape` at the end of plain-texting so the snippet holds a real `<` and output escaping happens exactly once on the PHP side — no index impact (body is snippet-only). New regression guard `HtmlToPlainTests`. **(A2)** Pond's visibility toggle used `ltrim($name,'_')`, stripping **all** leading underscores (`__draft`→`draft`, intended `_draft`); `substr($name, 1)` now removes exactly one. **(A3)** in `search_bm25.php`, `highlight_html`'s `preg_replace` returns `null` on a PCRE-limit overflow, which a `: string` return type under strict_types turns into a `TypeError`→500 — unreachable on default config but guarded with `?? $escaped` for servers with non-standard `pcre.*`. **(A4)** `admin_safe_rel` passed NUL bytes through (path-truncation room off-Windows); added a `str_contains($rel, "\x00")` reject. The audit's by-design items (single-author trust model) and supply-chain defense-in-depth items were judged non-blocking and split into a separate backlog. No `user/` schema change → no new migration step (stamp 1.10.0 → 1.11.0); MANIFEST 88; 532 → 536 unit tests. |
...
*Heron v1.11.0 — build with Python + Pillow, runtime PHP (OPcache recommended). Full release history in [§ 16](#16-changelog).*
