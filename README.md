# siheonlee.com v0.3 — 사용설명서 & 시스템 문서
...
13. [기존 글 마이그레이션 — migrate.py](#13-기존-글-마이그레이션--migratepy)
...
| 영역 | v0.1 | v0.2 | v0.3 (현재) |
|---|---|---|---|
| 출력 UI/UX | v0.1 자체 디자인 | 원본 `lama_website-main` 와 동일 | 원본 + 글마다 스타일 오버라이드 가능 |
| 마크다운 파서 | Python stdlib 자체 파서 | (동일) | **Parsedown.php (PHP CLI)** — 자체 파서는 fallback |
| meta.yaml 필드 | slug, title, date, seo_* | (동일) | + **`styles:` (글 단위 CSS 오버라이드)** |
| YAML 파서 | flat 매핑만 | (동일) | + **nested mapping (styles 트리용)** |
| 외부 의존성 | Python 3 | (동일) | Python 3 + (parsedown 사용 시) PHP CLI |
