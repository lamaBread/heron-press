...
- **글 단위 자산** — 글 폴더 안에 둡니다. `meta.yaml`/`content.*` 를 제외한 모든 파일·폴더가 `dist/{slug}/` 로 복사 (URL `/{slug}/imgs/…`).
...
- **비공개** — 파일/폴더명 앞에 `_`. 경로의 어느 폴더든 `_` 면 그 아래 전체 제외. 빌드 시 `dist/{slug}/` 자동 삭제.
...
| 위치 | 설정 |
|---|---|
| `site.yaml` | 도메인·name·copyright·lang·default_og_image / `category_per_page`·`category_preview_per_page` / robots.txt / reserved_slugs / warn_on_* / `description_truncate` / `images:` / `error_404_title`·`search_title` |
...
| 한계 | 내용 |
|---|---|
| 이미지 최적화는 정적 단일 프레임 | animated GIF 는 첫 프레임만 WebP. 보존하려면 webp 직접 첨부/외부 URL. 글·이미지 단위 토글 없음 (전역만). |
