...
|---|---|---|
| **v1.9.0** | 2026-06-01 | **Tool internationalization — language packs (§ 17-9).** The previously Korean-only tool is now localizable via locale packs. **Three surfaces** in one shared format: ① visitor-facing site chrome (search box, footer, pagination aria, 404, home/category labels), ② the entire Pond admin UI (258 keys), ③ build/CLI messages (warnings, aborts). **Two separate locale selectors** — the *site language* stays `site.yaml: lang` and is **baked into `dist/` at build time** (determinism preserved; `search.php` gets a localized string table injected at build time just like the BM25 index), while the new *tool language* is read at runtime from `user/.heron/locale` (defaults to `ko`) and chosen from a dropdown in Pond **Settings** (§ 17-8). The shared format is `system/locales/<locale>/*.yaml` flat dotted keys, merging the per-folder fragments (`admin`/`site`/`build`); `ko` is canonical and the fallback, and a missing key returns the key string so output is never blank. Loaders have zero external dependencies (Python `system/scripts/i18n.py`, PHP `system/admin/lib/i18n.php` share the format). An English pack (`en`) ships in the box — a new language is just one more set of `*.yaml`. The `m_1_9_0` migration seeds `user/.heron/locale=ko` (only when absent; idempotent — existing users keep their current UI). **`ko` values are byte-identical to the v1.8.0 hardcoded strings**, so `lang=ko` / tool=`ko` `dist/` rendered output is unchanged (only `search.php` source is regenerated by build-time injection). Stamp 1.8.0 → 1.9.0; MANIFEST 74 → 84 files; 490 → 499 unit tests + 6/6 diagnostics. |
