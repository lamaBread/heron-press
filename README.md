...
|---|---|---|
| **v1.9.1** | 2026-06-02 | **Language-pack stabilization + operator-message localization.** *Stabilization:* the two locale loaders (PHP `i18n.php` / Python `i18n.py`) now share **one byte-identical parser** with escape handling (`\"` `\\` `\n` `\t` inside double-quoted values; single-quoted = literal). This fixes two v1.9.0 bugs the live server exposed — `<code class="k">` rendered with **literal backslashes** (`class=\"k\"`), silently breaking the `code.k`/`code.file` styling across the whole admin UI, and the Pillow-abort message printing a **literal `\n`** instead of a line break. A regression test now enforces PHP↔Python parser parity + that no resolved value carries a stray backslash. *Expansion (the missed surface):* a new `cli.yaml` fragment brings the **operator-facing CLI/deploy/update/migration output into the language pack** — deploy (`deploy.py`/`rclone_bin.py`), self-update (`update.py`), the migration engine + per-step summaries/details, and `Heron.py` action output (`--check`/`--migrate`/`--check-update`/`--deploy`/`--check-config`/`--clean`) — looked up via a global `i18n.t()` set once at `Heron.py` startup from the tool language (mirroring Pond's PHP `t()`). So tool=`en` now yields **English deploy/update/migration panels**, not Korean ones (the i18n doc had listed `deploy.py` as a consumer that was never wired). Two build surfaces stay canonical Korean by design: the argparse `--help` text (a dev/CI reference, never shown in Pond's panels) and the builder's own 16-step progress labels (a follow-up — the build panel's warnings/aborts are already localized). *Also:* the **Settings** language dropdown shows endonyms (`한국어 (ko)` / `English (en)`) instead of raw codes; the orphaned `admin.home.flow.serve.t` key is wired into Home; `proc.php`'s last hardcoded error is localized; `test_build_cli` no longer depends on the Windows console encoding for `--help`. No `user/` schema change → no new migration step (stamp advances 1.9.0 → 1.9.1); MANIFEST 84 → 86 files (`ko/cli.yaml` + `en/cli.yaml`); 503 → 512 unit tests + 6/6 diagnostics. `lang=ko`/tool=`ko` `dist/` rendered output unchanged (visitor site chrome untouched). |
...
- **Site language** — the visitor-facing chrome in `dist/` (search box, footer, pagination, 404, home/category labels). Set by the existing `site.yaml: lang` and **substituted at build time** (determinism preserved — output is byte-identical for the same `lang`; `search.php` gets its string table injected at build time, like the BM25 index). This is unrelated to multilingual *content* (article bodies) — it is the language of the UI chrome the tool generates.
- **Tool language** — the operator-facing Pond admin UI and build/CLI messages (warnings, aborts). Set by a new one-line `user/.heron/locale` (a BCP 47 code; `ko` when absent), chosen from a dropdown on the **Settings** (`?a=settings`) screen. Runtime lookup, independent of the build.

**Pack format.** `system/locales/<locale>/*.yaml` — one `key: "value"` per line, flat dotted keys. The fragment files in a folder (`admin.yaml`, `site.yaml`, `build.yaml`) are merged. `ko` is canonical and the fallback for every locale; a missing key returns the **key string itself**, so the UI is never blank. `{name}` placeholders are substituted (e.g. `{n} results`). The loaders have zero external dependencies — Python `system/scripts/i18n.py` and PHP `system/admin/lib/i18n.php` share the format.

**Adding a language.** Drop `admin.yaml`/`site.yaml`/`build.yaml` into `system/locales/<code>/` and translate the `ko` keys (missing keys fall back to `ko` automatically). An English pack (`en`) ships in the box. Key parity between the `ko` source and other packs is enforced by `system/tests/test_i18n.py`.
...
*Heron v1.9.0 — build with Python + Pillow, runtime PHP (OPcache recommended). Full release history in [§ 16](#16-changelog).*
