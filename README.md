...
|---|---|---|
| **v1.14.12** | 2026-06-13 | **Adds an MIT `LICENSE` file at the repository root — until now the public repo shipped no license, which legally means all-rights-reserved (no one may use, modify, or redistribute it), an unintended state for a tool meant to be shared.** Copyright (c) 2026 Siheon Lee. The license is permissive and compatible with the bundled third-party code (the Parsedown port © Emanuil Rusev and the on-demand rclone binary are both MIT). `LICENSE` is repository infrastructure, not program surface (like `.gitignore`), so it is excluded from `system/MANIFEST.json` and the self-update overlay; a new [§ 19](#19-license) documents it alongside the third-party attributions. Docs only — the builder and dist output are byte-identical. No schema change (stamp 1.14.11 → 1.14.12); MANIFEST 92 (unchanged); 617 unit tests. |
| v1.14.11 | 2026-06-13 | Adds an author-time release-integrity self-check (`python Heron.py --release-check`, plus `system/tests/test_manifest_sync.py` riding the `unittest discover` suite) so the v1.14.8/v1.14.9 class of breakage — committing code without regenerating the manifest or refreshing the version references — fails loudly at release time instead of silently refusing every user's self-update. It recomputes the manifest from disk and compares it *whole* against the committed `system/MANIFEST.json` (flagging any added/removed/changed file or version mismatch), and confirms `README.md`/`README.ko.md` agree with `__version__` at the title, footer, and top changelog row. Tooling only — builder and dist byte-identical. No schema change (stamp 1.14.10 → 1.14.11); MANIFEST 90 → 92 (regenerated, +`check_release.py` +`test_manifest_sync.py`); 607 → 617 unit tests. |
| v1.14.10 | 2026-06-13 | Recovers the release finalization v1.14.8/v1.14.9 skipped — both committed code but never regenerated `system/MANIFEST.json`, leaving the integrity manifest stale (v1.14.7, 89 files, the new `pagemeta.php` unregistered), so self-update's sha256 gate *refused* both tags. Regenerating the manifest (89 → 90, version 1.14.10) recovers everyone at once — self-update always targets the latest tag — and backfilled the missing v1.14.8/v1.14.9 changelog (ko/en) plus the title/footer version. Docs and manifest only, no code change. No schema change (stamp 1.14.9 → 1.14.10); MANIFEST 89 → 90 (regenerated); 607 unit tests. |
| v1.14.9 | 2026-06-13 | The article-footer prev/next navigation labels are now localized — the last visitor-facing string that still rendered in Korean on a `lang:en` site. Every other Surface 1 string goes through `self.site_tr.t('site.…')` (the `lang` locale pack, resolved at build time), but this nav alone had `다른 글` (aria-label), `‹ 이전 글`, and `다음 글 ›` hardcoded in `builder.py`. The three labels move into ko/en `site.yaml` (`site.prev_next.aria`/`prev`/`next`) and `_render_prev_next_nav` now looks them up via `site_tr` (the ko values are byte-identical to the old hardcoding; en is More posts / ‹ Previous / Next ›). The ko build output is unchanged; the en dist has zero visitor-facing Korean. Builder-only. No schema change (stamp 1.14.8 → 1.14.9); **MANIFEST and changelog not regenerated (backfilled in v1.14.10)**; 607 unit tests. |
| v1.14.8 | 2026-06-13 | A page-settings editor lands in Pond — the home (`user/articles/meta.yaml`) and category (`user/articles/<category>/meta.yaml`) `meta.yaml` files, which had no UI, are now edited as raw YAML the same way site.yaml is. The builder handles home and category through one parser (`_parse_category_meta_file(scope, text=…)`), so the editor is unified too, and a save is recorded only after it passes the same validation gate the build uses (new `Heron.py --check-page-meta <REL>` — checks the stdin candidate without touching disk, rejecting on any issue). An empty body deletes the (optional) meta.yaml to restore defaults (backed up first). Entry points: the settings ④ section and a "Page settings" button in the list header; new view `system/admin/views/pagemeta.php`. Admin-UI + a CLI gate; builder and dist output byte-identical. No schema change (stamp 1.14.7 → 1.14.8); **MANIFEST and changelog not regenerated — self-update integrity broken (backfilled/fixed in v1.14.10)**; 607 unit tests. |
| v1.14.7 | 2026-06-11 | **The per-file deploy diff (`?a=deploy_diff`) now bounds how long a stalled connection can freeze Pond — `cat_remote` adds short `--contimeout`/`--timeout`/`--retries 1`/`--low-level-retries 2` flags so an unresponsive (not disconnected) host fails fast instead of hanging the single-worker `php -S` for minutes.** The blocking `rclone cat` had no timeouts, so on a sleep-resumed laptop or dropped VPN it leaned on rclone's defaults (contimeout 1m, IO 5m) and locked up *every* Pond page — not just the diff box — until it gave up; sync (`build_argv`) is long-running by design and is left untouched. A v1.10.0-style regression guard now checks the cat flags against the real rclone binary too. Also a one-line view fix: the deploy result banner gets the `margin-top:14px` the build/update banners already had, so on apply/failure it no longer butts against the log box. Admin-UI + a four-flag deploy-path change; builder and dist output byte-identical. No schema change (stamp 1.14.6 → 1.14.7); MANIFEST 89; 607 unit tests. |
| v1.14.6 | 2026-06-10 | The Update button (`?a=update`) now streams its log live too — the same fix the Build button got in v1.14.5 — switching from the blocking `admin_run_heron` to the streaming path (output buffering off, new `admin_run_update_stream`, shared `admin_log_widget_head()` log widget, result banner below the log) with `_action_update` gaining the per-line `print(..., flush=True)` it lacked, without which nothing streams through a pipe. As a side benefit the view/layout/proc/i18n are loaded *before* the self-overlay replaces them, so the page renders from consistent old-version code. No schema change (stamp 1.14.5 → 1.14.6); MANIFEST 89; 606 unit tests. |
| v1.14.5 | 2026-06-10 | The Build button now streams its log live — `?a=build` switched from the blocking `admin_proc` to the deploy-style line streaming, with the logbar/follow script extracted into the shared `admin_log_widget_head()` (strings generalized to `admin.log.*`) and the build child's stderr merged into stdout via the new opt-in `2 => ['redirect', 1]` descriptor (deadlock-free, output in true order). Admin-UI only; builder and dist output byte-identical. No schema change (stamp 1.14.4 → 1.14.5); MANIFEST 89; 606 unit tests. |
| v1.14.4 | 2026-06-09 | imgBox captions are now standards-compliant semantic markup — both emit sites in `markdown.py` (markdown form and simulated PHP `imgBox()`) replace the non-standard `<div>` + `<p><small><b>` combo with `<figure class="imgBox">` + `<figcaption><small>`, lifting the container to `<figure>` so the markup validates. The `.imgBox` rules are all class/descendant selectors, so no migration or user-CSS edit is needed (the caption is no longer bold, by design); the two emit sites stay byte-identical while the dist HTML of captioned imgBoxes changes intentionally. No schema change (stamp 1.14.3 → 1.14.4); MANIFEST 89; 606 unit tests. |
...
| v1.12.0 | 2026-06-06 | Deploy preview: a collapsible ~10-line live log (auto-follows the tail while streaming) plus a parsed dry-run summary — Upload / Delete / Directory-ops / Warnings cards, a by-top-level-directory table, and a warnings list — replacing the unreadable hundreds-of-lines `<pre>`. Parsing is single-sourced in `deploy.py` (always prints a one-line human summary the CLI benefits from; emits a machine-readable JSON line behind an RS sentinel only when Pond sets `HERON_DEPLOY_SUMMARY=json`, which `deploy_run.php` intercepts and renders into the cards/table). Dry-run only; the apply path is byte-identical. No schema change (stamp 1.11.4 → 1.12.0); MANIFEST 88; 555 → 568 unit tests. |
| v1.11.4 | 2026-06-06 | rclone deploy `ssh_alias` delegation mode (opt-in) — fixes the passphrase-key + host-key-mismatch deploy failures: when set, `build_argv` drops the host/user/port/key/known_hosts flags and emits `--sftp-ssh "ssh <alias>"` + `--sftp-disable-hashcheck`, delegating auth and host-key verification to system ssh (`~/.ssh/config`, Keychain, ssh-agent, `known_hosts`). **Fully backward-compatible** — with no `ssh_alias` the key-file path is byte-identical (Windows default unchanged; alias mode is a macOS/Linux opt-in). Also (release tooling): `make_manifest` now excludes `.DS_Store`/`Thumbs.db` at any depth, so regenerating the manifest no longer injects a phantom entry that makes `--check` report `missing` on other platforms. No schema change (stamp 1.11.3 → 1.11.4); MANIFEST 88; 546 → 555 unit tests. |
| v1.11.3 | 2026-06-04 | Localized the article-end published/updated meta labels ("발행"/"수정") to the site language — they were hardcoded Korean and missed by the v1.9.x sweep, so `lang=en` builds left that one footer in Korean. Now routed through `self.site_tr` with new `site.article.published`/`updated` keys; `lang=ko` output is byte-identical. |
| v1.11.2 | 2026-06-04 | rclone PATH-fallback integrity warning (security backlog B3). When the pinned binary fails and `rclone_bin.ensure()` falls back to a PATH `rclone`, the message now states the binary is UNVERIFIED instead of a benign note (a download sha256 mismatch still hard-aborts). Message-only. |
| v1.11.1 | 2026-06-04 | Self-update supply-chain hardening (B1·B2) — `self_update` now hard-aborts when a downloaded tree lacks `system/MANIFEST.json`, and a new `_trusted_zipball()` pins the GitHub `zipball_url` to the `lamaBread/heron-press` origin. |
| v1.11.0 | 2026-06-04 | Security-audit round — fixed confirmed findings from a 4-domain review: search snippets stop double-encoding escaped code (`html.unescape`), Pond's visibility toggle strips one underscore not all, a PCRE-null 500 guard in `highlight_html`, and `admin_safe_rel` rejects NUL bytes. By-design and supply-chain items moved to a backlog. |
| v1.10.0 | 2026-06-04 | Deploy hotfix — `build_argv` emitted `--sftp-known-hosts`, a flag rclone never had, so every deploy died at flag parsing; corrected to `--sftp-known-hosts-file`. Added a guard that checks every emitted flag against the real rclone `help flags`. |
| v1.9.7 | 2026-06-03 | Default tool + site language flipped Korean→English (v1.9.x language-pack line) — i18n canonical/default/fallback changed `ko`→`en` (existing installs keep their choice, changeable in Pond ▸ Settings), and the visitor-site `lang` default flipped for fresh clones/the demo only. Added `--check-locale` (parity CI gate) and `--new-locale` (scaffold). |
| v1.9.6 | 2026-06-03 | Translated the comments in `user/site.yaml` (the last user yaml with Korean comments) to English; values, structure, key order and encoding unchanged, and the locale packs left untouched. |
| v1.9.5 | 2026-06-03 | Right-sized the `ko` locale pack — unified settled technical terms to English (cache hit/miss, raster, the Actions column) and the Pond 403 SAPI/loopback guard; writer-facing issue/warning bodies kept Korean. |
| v1.9.4 | 2026-06-02 | Localized the per-article content-defect `_issue`/`_warning` bodies (29 builder sites) and the image-encoding-failure message into `build.issue.*` / `build.label.*` / `build.warn.*`. |
| v1.9.3 | 2026-06-02 | Localized argparse `--help` (description, epilog, option help → `cli.help.*`) and the remaining `images.*` validation aborts plus parity-failure messages (→ `build.abort.*` / `build.parity.*`). |
| v1.9.2 | 2026-06-02 | Localized the build console and the entire `build-report.md` (16-step progress, milestones, live counters, `[search]` lines, report tables) — a tool=`en` build panel is fully English while `ko` stays byte-identical. |
| v1.9.1 | 2026-06-02 | Language-pack stabilization — unified the PHP/Python locale loaders on one byte-identical parser with escape handling (fixing two v1.9.0 rendering bugs), and added `cli.yaml` to bring deploy/update/migration/CLI operator output into the pack. |
| v1.9.0 | 2026-06-01 | Tool internationalization — the Korean-only tool became localizable via `system/locales/<locale>/*.yaml`, with two independent selectors: the build-time *site language* (`site.yaml: lang`) and the runtime *tool language* (`user/.heron/locale`, a Settings dropdown). `ko` is canonical and fallback, an `en` pack ships, and `ko` output is byte-identical to v1.8.0. |
| v1.8.0 | 2026-06-01 | Pond **Settings** (`?a=settings`) + system-overview **Home** (`?a=home`) — a structured `deploy.json` form and a raw `site.yaml` editor (saved only after passing `Heron.py --check-config`, with a backup), and the Pond brand now links to a pipeline-visualizing home (the article list moved to `?a=list`). dist byte-identical to v1.7.0. |
| v1.7.2 | 2026-06-01 | Hotfix — the "update available" banner persisted after self-update because the cache was not refreshed post-overlay; `self_update` now updates it to the new-version state, and `list.php` strips a doubled `vv` in the tag. |
| v1.7.1 | 2026-06-01 | Hotfix — the self-update backup copied the schema stamp without creating the destination's parent directory, dying with `FileNotFoundError`; fixed in `_backup_program`, with `test_full_flow` covering the branch. |
| v1.7.0 | 2026-06-01 | rclone one-click dist deploy (§ 13-1 · § 17-7) — Pond's **[Deploy]** incrementally syncs `dist/` to the server via rclone (SFTP) behind a two-stage dry-run gate guarding remote deletion. A pinned rclone (v1.74.2) is fetched on demand and SHA256-verified; auth is an SSH key file with forced `known_hosts` verification, configured in the gitignored `user/.heron/deploy.json`. dist byte-identical to v1.6.2. |
...
Heron is released under the **MIT License** — see [`LICENSE`](LICENSE) at the repository root. In short: anyone may use, copy, modify, and redistribute it (including in closed-source or commercial work) as long as the copyright notice and the license text are preserved; the software is provided as-is, without warranty.

Copyright (c) 2026 Siheon Lee.

**Third-party components** (all permissive, compatible with MIT):

| Component | Where | License |
|---|---|---|
| **Parsedown** 1.7.4 — ported to Python in `system/scripts/parsedown.py` | bundled (source) | MIT — © Emanuil Rusev ([parsedown.org](http://parsedown.org)) |
| **rclone** (pinned v1.74.2) | fetched on demand into `system/runtime/bin/`, **not committed** | MIT |
| **Pillow** | build-time dependency (`pip install Pillow`), not bundled | HPND (MIT-style) |

`LICENSE` is repository infrastructure (like `.gitignore`) — not part of the program surface tracked in `system/MANIFEST.json`, so it is not overlaid by self-update.

---

*Heron v1.14.12 — build with Python + Pillow, runtime PHP (OPcache recommended). Full release history in [§ 16](#16-changelog).*
