...
| **사이트 내 검색** — 클라이언트 JS 없이 한국어 친화 부분검색 | 빌드 시 `dist/search-index.json` (한글 bigram + 영문 토큰 역색인) + `dist/search.php` 동시 생성. 카테고리 페이지 검색은 자동으로 해당 카테고리 내부로 한정. v0.4.0 부터 1글자 한국어는 인덱싱/쿼리 대상에서 제외, Python↔PHP 토크나이저 패리티를 빌드마다 자동 검증. **v0.5.0 부터 Okapi BM25 + 필드 가중치 + phrase 부스트** — 흔한 토큰 vs 희귀 토큰, 짧은 글 vs 긴 글, 흩어진 매치 vs 정확한 phrase 매치를 점수가 합리적으로 반영합니다. |
...
│   └── search_bm25.php       ← (v0.5.0) BM25 점수 + 매치 밀도 스니펫 + 하이라이트
│
├── tests/                ← (v0.5.0) 단위 테스트
│   ├── __init__.py
│   └── test_bm25.py          ← BM25 알고리즘 회귀 차단 (25개)
│
...
| `slug 예약어` | `assets`, `search` 등 예약된 이름 (v0.5.2 부터 `src` 는 제외) | site.yaml 의 reserved_slugs 확인 후 변경 |
...
```
[빌드 시점 — Python]
  build.py 가 모든 글의 평문 본문을 토크나이즈 → BM25 인덱스 (포맷 v3) 빌드 →
  dist/search-index.json + dist/search.php
  + dist/search_tokenize.php (토크나이저 함수)
  + dist/search_bm25.php   (BM25 점수 함수, v0.5.0 신설)

[방문자 검색]
  사용자가 nav 우측 검색창에 입력 → form GET → /search.php?q=...
  PHP 가 search_tokenize.php / search_bm25.php 를 require_once 한 뒤
  search-index.json 을 메모리에 로드 → 쿼리를 같은 토크나이저로 쪼개
  필드별 (body / title) BM25 점수를 계산하고 합산 → 원본 쿼리가 substring
  으로 매치되면 phrase 부스트 → 매치 밀도 윈도우로 스니펫 추출 → 결과 렌더
```
...
```json
{
  "version": 3,
  "params": {
    "k1_body": 1.5, "b_body": 0.75,
    "k1_title": 1.2, "b_title": 0.5,
    "w_title": 3.0,
    "phrase_boost_body": 1.5, "phrase_boost_title": 2.0
  },
  "stats":   {"N": 6, "avgdl_body": 94.17, "avgdl_title": 4.17},
  "docs": [
    {"slug":"hello-world","title":"Hello, World!","date":"2026-05-07",
     "category":"Blog","category_slug":"blog",
     "body":"평문 본문 전체...","dl_body":42,"dl_title":2}
  ],
  "categories": {"blog": "Blog"},
  "df_body":    {"마스": 1, "프린": 2, ...},
  "df_title":   {"hello": 1, ...},
  "tf_body":    {"마스": [[0, 3]], "프린": [[0, 1]], ...},
  "tf_title":   {"hello": [[0, 1]], ...}
}
```
...
> **v0.4.x 의 v2 포맷과 비호환**: 키 이름이 `index/title_index` → `tf_body/tf_title` 로 변경, `df_*` / `stats` / `params` 가 추가되었습니다. `search.php` 는 `version < 3` 인 인덱스를 거부합니다.

### 점수 계산 — BM25 + 필드 가중치 + phrase 부스트 (v0.5.0)
...
- **k1 (TF 포화 강도)** — 본문 1.5 / 제목 1.2. 작을수록 두 번째 매치부터의 한계 이득이 작아짐.
- **b (길이 정규화 강도)** — 본문 0.75 / 제목 0.5. 제목은 길이가 짧고 토큰 하나의 의미 비중이 크므로 정규화 영향을 줄임.

#### 두 필드 합산

```
score(Q, D) = bm25_body(Q, D) + w_title · bm25_title(Q, D)        (w_title = 3.0)
```

#### Phrase 부스트 (곱셈)

원본 쿼리 문자열 (trim 후 길이 ≥ 2) 이 평문 본문/제목에 **연속 substring** 으로 매치되면:

```
if  q in body:   score *= phrase_boost_body   (1.5)
if  q in title:  score *= phrase_boost_title  (2.0)
```

곱셈이므로 score=0 (토큰 미매치) 은 그대로 0 — 의미 있는 점수만 부스팅됩니다.

#### v0.4.x → v0.5.0 결함 해소표

| v0.4.x 결함 | v0.5.0 해소 |
|---|---|
| 단순 TF 합산 (IDF 없음). 흔한 한글 bigram 과 희귀 영문 토큰 동등 가중 | IDF 도입 — `df` 큰 토큰은 IDF ≈ 0 |
| 본문 길이 정규화 없음. 긴 글이 부당하게 상위 | `b·dl/avgdl` 정규화 |
| 제목 부스트 `×5` 의 매직 넘버 | BM25 정규화 점수 위에 `w_title=3.0` — IDF/길이 정규화가 이미 척도 맞춤 |
| Phrase vs 흩어진 토큰 매치 동일 | 원본 쿼리 substring 매치 시 곱셈 부스트 (본문 ×1.5, 제목 ×2.0) |
| 스니펫이 첫 매치 위치 기준 | 매치 밀도 최대 80자 윈도우 |

#### Python ↔ PHP 점수 패리티

`scripts/search.py` 의 `bm25_score()` 는 Python 참조 구현입니다. 런타임 PHP (`templates/search_bm25.php`) 와 동일 공식. tests/test_bm25.py 가 Python 측 회귀를 차단하고, 양측 패리티는 빌드 산출물 (`dist/search-index.json`) 위에서 임시 CLI 스크립트로 손쉽게 확인할 수 있습니다 (개발 환경에서 PHP 8.3 로 소수점 6자리까지 일치 확인).
...
├── search_bm25.php        ← BM25 점수 + 스니펫 함수 (v0.5.0; search.php 가 require_once)
├── search-index.json      ← 검색 인덱스 v3 (Python 이 생성)
└── ...
```
...
*이 문서는 siheonlee.com v0.5.1 (PHP 기반 경량 웹 사이트 생성기 — 빌드는 Python + Pillow, 런타임은 PHP; 검색은 Okapi BM25; 이미지는 WebP 다중 해상도 자동 변환) 기준으로 작성되었습니다. (2026-05-14)*
