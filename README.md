...
> **v1.1.1 한 줄 요약:** *배포 사고 수정 (빌드 로직 릴리스).* v1.0.2.2 실배포에서 `/u-hof/`·`/automatic-grading/` 등 **imgBox PHP 를 쓰는 글이 그 지점부터 끝까지 잘려** 나갔다 (사용자 보고 문제 1·2 동일 원인). `scripts/markdown.py` 의 `simulate_php_in_html` 이 한 줄 단일 호출 `<?php imgBox(a) ?>` 만 펼치고 실제 본문 대부분인 **다중 구문 블록**(`<?php\n global $sig;\n imgBox(…);\n imgBox(…);\n?>`)은 시뮬레이트 실패 → 원본 PHP 가 `dist/*/index.php` 로 새고, 정적 siheonlee.com 엔 정본 lama.pe.kr 이 auto_prepend 하던 `imgBox()`/`$reference_*` 런타임이 없어 `Call to undefined function imgBox()` fatal → 그 뒤 HTML 전부 소실. 수정: (1) `simulate_php_in_html` 을 블록 토큰 스캐너로 재작성(주석·`global` 선언·`;` 무시, imgBox/imgSlideBox 만이면 정적 HTML 치환, 다른 살아있는 구문 있으면 원문 보존=동적 PHP 보호), (2) `site.yaml` `php_globals:` 신설 — 정본 `lama_website-main/PHP/GlobalVariables.php` 의 서명 변수(`$reference_hanbyeol`·`$reference_hanbyeol_webDesign` = 김한별 일러스트·웹디자인 크레딧)를 운영자 설정으로 옮겨 캡션 `{$name}` 을 빌드 시 치환, (3) `_simulate_imgbox` 캡션 이스케이프 해제(정본 `PHP/GlobalFunctions.php` imgBox 가 `$exp` raw echo — 작성자 의도 `<br>`·`<a>`·서명 보간 보존; `alt` 는 이스케이프 유지). admin 미리보기도 같은 `php_globals` 적용. 무결성 = **코드 릴리스 형** (불변 `siheonlee.com_v1.1.0` 미수정·4번째-숫자 검증 복사본 `siheonlee.com_v1.1.0.1` 의 v1.1.0 *코드* 클린 재빌드 vs v1.1.1): **786=786**, leak `.php` 10개 제거 → 동일 slug `index.html` 10개 추가, `search.php` 1개 변경(BM25 인덱스가 교정 본문 반영), 그 외 전부 byte-불변; 클린 2회 결정성 동일(combined sha256 `836fa333…`), `__version__` 1.1.0→1.1.1 dist 누수 0(B1). 단위 **337**(`test_markdown` `ParsePhpGlobals`+`SimulatePhp` 21 신설) · 진단 **6/6**([6] ld+json 47/46, violations 0). 직전 v1.1.0(로컬 글쓰기 `admin.php` 추가)·v1.0.x 내역은 [§ 16](#16-업데이트-로그) (각 버전 한 줄 요약·무결성 보존).
...
**현재 한계 (v1.1.1)** — 두 부류. ⓐ 현 능력의 *내재적* 한계, ⓑ *의도적으로 보류한* 확장. **둘 다 현 상황에서는 도입하지 않는다** (사용자 합의 2026-05-18 — 사유가 유효한 동안 유지, 필요해질 때 재검토).
