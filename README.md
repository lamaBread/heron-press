...
google_adsense:                       # Google AdSense (v1.1.3, v1.1.4 에 exclude_pages 추가)
  ...
  exclude_pages: [404, search]        # (v1.1.4) head_script 미주입할 페이지 타입 / [] 면 5 페이지 전체 주입
...
#   exclude_pages (v1.1.4): 식별자 5종(article·home·category·404·search)
#   중 매칭된 페이지 타입은 placeholder 라인이 제거되어 head 에 로더
#   스크립트가 들어가지 않음 = Google auto-ads JS 미로드 = 광고 원천 차단.
#   인-페이지 광고 위치는 여전히 Google auto-ads 가 결정. 모르는 식별자는
#   자연 무시. yaml 의 `404` 는 정수로 파싱되지만 파서가 str()/lower() 정규화.
```
