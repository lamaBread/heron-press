...
|---|---|---|
| **v1.11.1** | 2026-06-04 | **Self-update supply-chain hardening (security backlog B1·B2)** — two defense-in-depth fixes to `update.py`'s self-update path (both already first-lined by the pinned `REPO` + TLS, now structurally enforced). **(B1)** when the downloaded release tree carried no `system/MANIFEST.json`, `self_update` previously *logged a note and overlaid anyway*, silently skipping the sha256 integrity gate; it now **hard-aborts before any overlay** (`cli.update.no_manifest` reworded warning→refusal). **(B2)** the GitHub tags API's `zipball_url` was downloaded verbatim; a new `_trusted_zipball()` accepts it only when it begins with `https://api.github.com/repos/lamaBread/heron-press/`, else falls back to the constructed `ZIPBALL` URL — wired into both `check_update` and `self_update` (belt-and-suspenders against a redirected/forged download origin). New `test_update.py` guards (no-MANIFEST download leaves the install untouched; `_trusted_zipball` pass-through / foreign-host / other-repo / None). No schema change → no new migration step (stamp 1.11.0 → 1.11.1); MANIFEST 88; 536 → 542 unit tests. |
...
*Heron v1.11.1 — build with Python + Pillow, runtime PHP (OPcache recommended). Full release history in [§ 16](#16-changelog).*
