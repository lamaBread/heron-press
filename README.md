...
> **v1.4.0 한 줄 요약** — 글 푸터 이전/다음 글 + 글 끝 발행/수정 메타 + 다크 모드 + 내부 링크 검증 + site.yaml 슬림화(다섯 키 코드 상수로) + BuildReport "PHP 빌드 글" 별도 카테고리. 누적 기능은 [§ 16](#16-업데이트-로그) 표 참조.
...
*siheonlee.com v1.4.0 — 빌드 Python + Pillow, 런타임 PHP (OPcache 권장). 누적 릴리스 내역은 [§ 16](#16-업데이트-로그).*
