...
|---|---|---|
| **v1.14.0** | 2026-06-07 | **Pond admin list: the per-row category dropdown now shows each post's *actual* category (its current parent is `selected`), and changing the selection moves the post immediately — the separate *Move* button is gone.** Previously the dropdown *excluded* the post's own parent, so every sub-category post defaulted to "(top level)" as its first option — reading like a false "everything is top-level" status (and a top-level post oddly defaulted to the first real category). Now nothing is excluded and the current parent is `selected`, so the control doubles as an honest current-category indicator. Picking a different category fires `onchange` → one `confirm()` → submit (a folder rename; the URL slug is unchanged); **Cancel restores the prior selection** via `data-cur`, a wheel-scroll guard (`blur` on wheel) prevents accidental value changes, and a server-side **no-op guard** makes "re-pick the same category" do nothing. After a successful move the list reloads with the moved row's **path string highlighted once** (a ~1.8s green settle) and scrolled into view via `#moved` (sticky-header offset), with a `prefers-reduced-motion` fallback. Admin-UI only; the builder and dist output are byte-identical. No schema change (stamp 1.13.1 → 1.14.0); MANIFEST 88; 589 unit tests (unchanged — the PHP admin surface is not covered by the Python suite). |
...
*Heron v1.14.0 — build with Python + Pillow, runtime PHP (OPcache recommended). Full release history in [§ 16](#16-changelog).*
