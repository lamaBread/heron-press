...
> **v1.1.2 한 줄 요약:** *배포 사고 수정 + imgSlideBox 페이지네이션형 재디자인 (CSS+JS 자산 릴리스).* v1.1.1 dist 실배포에서 imgSlideBox 가 슬라이드 박스가 아니라 폴더 안 **모든 이미지가 세로로 나열**됐다 (사용자 보고, `/clear` 글). 원인: 정본 lama.pe.kr 의 `imgSlideBox` 는 단일 `<img>` 의 `src` 를 JS 로 갈아끼워 한 장만 보였으나, siheonlee 정적 빌드(`_simulate_imgslidebox`)는 폴더의 모든 이미지를 `<img class="slide">` 로 펼치고 첫 장에만 `.active` 를 줘 `imgslidebox.js` 가 `.active` 를 옮기는 설계 — "한 장만 보이기"가 전적으로 CSS 책임인데 `common_template.css` 에 `.imgSlideBox .slide{display:none}`+`.slide.active{display:block}` 가 **처음부터 없어** 모두 표시됐다 (JS 정상, 누락 CSS 가 본질; 그 두 줄 제거 금지=회귀). **같은 릴리스에서 디자인도 함께 개선** — 사용자 요청으로 좌우 큰 검정 오버레이 버튼을 폐지하고, 사이트 `.pagination-nav` 와 같은 절제된 톤의 **점(dot) 인디케이터를 이미지 하단 중앙**에 두는 모던·간결한 페이지네이션형으로 재디자인 (‹ prev · 점들 · next ›, 활성 점 흰색·확대, 점 클릭으로 이동, 평소 흐릿·hover/`focus-within` 시 또렷). 핵심 설계: 이 `.slide-nav` 는 `imgslidebox.js` 가 **런타임에 생성**(`_simulate_imgslidebox` 정적 마크업·빌드 로직 무변경) → **정적 HTML 산출물은 한 글자도 안 바뀌고** 변경이 `common_template.css`+`imgslidebox.js` 두 자산에만 갇힌다(JS 미동작 시 오류수정 CSS 로 첫 장만 보이는 안전 폴백). 무결성 = **코드 릴리스 형·dist 의도적 변경형** (불변 `siheonlee.com_v1.1.1` 미수정·4번째-숫자 검증 복사본 `siheonlee.com_v1.1.1.1` 의 v1.1.1 *코드* 클린 재빌드 vs v1.1.2): **786=786**, 0 added/0 removed, `assets/common_template.css`·`assets/imgslidebox.js` **2 파일만 변경**, 글·홈·카테고리·피드·사이트맵·`search.php`·이미지 전부 byte-불변; 클린 2회 결정성 동일(combined sha256 `748997de…`), `__version__` 1.1.1→1.1.2 dist 누수 0(B1). 단위 **337** 승계(순수 자산 — 신규 테스트 없음) · 진단 **6/6**([6] ld+json 47/46, violations 0 — HTML 무변경 실증). 직전 v1.1.1(다중 구문 imgBox PHP 블록 truncate 수정)·v1.1.0(로컬 글쓰기 `admin.php`)·v1.0.x 내역은 [§ 16](#16-업데이트-로그) (각 버전 한 줄 요약·무결성 보존).
...
```
빌드 시작 - siheonlee.com v1.0.2 (2026-05-19 03:00:00)
...
│   │
│   └── tests/                   ← 단위 테스트 (337) + run_diagnostics.py (6 항목)
│
...
**기본 og:image 자산 (v1.0.0)** — `site.default_og_image` 가 가리키는 자산은 raster 여도 빌드가 webp 변환·srcset 등록을 **건너뛰고 원본 그대로** `dist/` 에 낸다 (`_copy_site_assets` 예외). og:image 소비자는 `<img>` 후처리가 아니라 SNS 링크 언퍼ler — `og:image` 메타의 고정 URL 하나만 가져가므로 다중 해상도가 무의미하고, KakaoTalk·일부 Facebook 은 WebP og:image 를 렌더하지 못한다. `resolve_og_image` 도 이 값을 문자열 그대로 쓰므로(webp 재매핑 없음) 변환했다면 그 URL 이 dist 에서 404 다. 기본 자산 `src/assets/default-og.png`(1200×480, 도메인 워드마크) 동반 — 이전 릴리스는 `site.yaml` 의 `default_og_image` 경로에 실제 파일이 없어 본문 이미지가 없는 모든 글·홈·카테고리의 og:image 가 죽은 링크였다. `default_og_image` 가 외부 URL 이거나 `assets/` 밖이면 이 예외는 무동작 (그 경우 파일 배치는 작성자 책임).
...
| 위치 | 설정 |
|---|---|
| `site.yaml` | 도메인·name·copyright·lang·default_og_image / `category_per_page`·`category_preview_per_page` / robots.txt / reserved_slugs / warn_on_* / `description_truncate` / `images:` / `jsonld:` / `php_globals:` / `google_adsense:` / `error_404_title`·`search_title` |
...
php_globals:                          # PHP 서명 변수 (v1.1.1, 생략 시 보간 없음)
...
google_adsense:                       # Google AdSense (v1.1.3, v1.1.5 에 URL 기반 exclude_urls 로 교체)
  ...
  exclude_urls: ['/404.html', '/search.php', '/about/']   # (v1.1.5) head_script 미주입할 URL / [] 면 5 페이지 전체 주입
...
#   exclude_urls (v1.1.5): site-relative 절대 URL ('/' 로 시작) 리스트. 매칭된
...
#   build-report.md "살펴볼 사항" 으로 자동 보고. v1.1.4 의 page-type 식별자
#   5종(article/home/category/404/search) 방식은 v1.1.5 에서 URL 기반으로
#   통합 — 글 단위 차단이 자연스러움(예: /about/).
```
...
**현재 한계 (v1.1.2)** — 두 부류. ⓐ 현 능력의 *내재적* 한계, ⓑ *의도적으로 보류한* 확장. **둘 다 현 상황에서는 도입하지 않는다** (사용자 합의 2026-05-18 — 사유가 유효한 동안 유지, 필요해질 때 재검토).
...
> v1.1.1 은 **배포 사고 수정** 릴리스 — 빌드 *로직* 변경이라 dist 가 바뀐다(B1 유지 — `__version__` 1.1.0→1.1.1 자체의 dist 누수 0). v1.0.2.2 실배포에서 `/u-hof/`(전역 서명 변수 사용)·`/automatic-grading/` 등 imgBox PHP 를 쓰는 글이 본문 중간(imgBox `<?php` 블록)부터 끝까지 클라이언트에 전달되지 않고 잘려 나갔다. **사용자 보고 문제 1(서명 변수 페이지 truncate)과 문제 2(imgBox 블록부터 끝까지 누락)는 동일 원인**: `scripts/markdown.py` 의 `simulate_php_in_html` 이 한 블록당 호출 1개인 **한 줄** `<?php imgBox(a) ?>` 형태만 정적 HTML 로 펼치고(닫는 `?>` 가 호출 `)` 직후여야 하는 제약), 실제 본문 거의 전부인 **다중 구문 블록**(`<?php\n global $reference_hanbyeol_webDesign;\n imgBox("a","…{$reference_hanbyeol_webDesign}");\n imgBox("b","…");\n?>`)은 시뮬레이트에 실패해 **원본 PHP 가 그대로 `dist/*/index.php` 로 샜다**. 정적 siheonlee.com 에는 정본 lama.pe.kr 이 auto_prepend 하던 `PHP/GlobalFunctions.php`(imgBox/imgSlideBox)·`PHP/GlobalVariables.php`($reference_*) 런타임이 없어, 요청 시 첫 `imgBox()` 에서 `Call to undefined function imgBox()` (또는 미정의 `$reference_*`) fatal → PHP 가 그 지점까지만 출력하고 종료해 그 뒤 HTML(나머지 imgBox·`<hr>`·날짜·`<footer>`·`</body></html>`) 이 통째로 사라진 것. `global $var` 는 별개 원인이 아니라 같은 leak 의 부수 증상. 실측 영향: 생성된 `.php` 10개 전부가 leak(그중 `/automatic-grading/`·`/algorithm-dictionary/`·`/movie-go-watch/`·`/radish-pang-leaderboard/`·`/u-hof/` 는 실제 truncate, 나머지 5개는 `<?php // imgBox(…) ?>` 주석형이라 fatal 은 없지만 불필요하게 `.php`). 수정 세 가지: **(1)** `simulate_php_in_html` 을 블록 토큰 스캐너로 재작성 — `<?php`…`?>` 사이를 스캔해 `//`·`#`·`/* */` 주석과 `global …;` 선언과 `;` 을 무시하고, 남는 게 `imgBox`/`imgSlideBox` 호출뿐이면 블록을 통째로 정적 HTML 로 치환(라이브 PHP 소멸 → `index.html`), 살아있는 다른 구문이 하나라도 있으면 블록 원문을 그대로 보존(검색 페이지 등 진짜 동적 PHP 보호 — 보수적). 한 줄 단일 호출 형태는 동일 출력(회귀 가드 테스트). 전부 주석인 블록은 ''(소멸)이라 주석형 5개도 `.html` 로 정리. **(2)** `site.yaml` `php_globals:` 신설(`SiteConfig.php_globals`, `parse_php_globals`) — 정본 `lama_website-main/PHP/GlobalVariables.php` 의 `$reference_hanbyeol`("Character illustration by 김한별 (kakao ID: zzang767401)")·`$reference_hanbyeol_webDesign`("Illustration and Web Design by 김한별 (kakao ID: zzang767401)")를 운영자 설정으로 옮겨, 캡션의 `{$name}`/`$name`(큰따옴표 한정) 을 빌드 시점에 치환(`_interpolate_php`; 미정의 변수는 빈 문자열 = PHP 미정의 echo 동등). site.yaml 값은 운영자 입력이라 [§16 v0.8.x 정신의] **코드 무결성 항목이 아니다**. **(3)** `_simulate_imgbox` 가 캡션(`exp`)을 더 이상 이스케이프하지 않는다 — 정본 `PHP/GlobalFunctions.php` 의 `imgBox` 가 `{$exp}` 를 그대로 echo 했고 작성자가 캡션에 `<br>`·`&nbsp;`·`<a …>` 와 서명 변수 보간을 의도적으로 넣기 때문(정적 빌드가 원 사이트와 같은 결과를 내야 함). `alt` 는 속성값이라 이스케이프 유지. 이로써 `ron-mueck` 의 `<a …>[source link]</a>` 캡션도 원래 의도대로 진짜 링크가 된다(이전엔 escape 되어 글자 그대로 노출). admin 미리보기 `src/admin/render_one.py` 도 빌더와 같은 `<verdir>/site.yaml` `php_globals` 를 읽어 본문 충실도 유지(`test_render_one` 패리티 게이트는 양쪽이 같은 site.yaml 에서 읽도록 갱신 — 구성적 패리티 보존). 무결성 = **코드 릴리스 형** (정본 Articles 고정, 불변 `siheonlee.com_v1.1.0` 를 손대지 않는 4번째-숫자 검증 복사본 `siheonlee.com_v1.1.0.1` 의 v1.1.0 *코드* 클린 재빌드 vs v1.1.1 클린 재빌드의 *열거된* diff): **786=786**, leak `index.php` 10개(`algorithm-dictionary`·`automatic-grading`·`clear`·`cpp-template-meta-programming`·`mmca-korean-contemporary-art`·`movie-go-watch`·`radish-pang-leaderboard`·`revision-notes`·`ron-mueck`·`u-hof`) **제거** → 동일 slug `index.html` 10개 **추가**(올바른 정적 HTML 로 펼쳐짐), `search.php` 1개 **변경**(인라인 BM25 인덱스가 교정된 10개 글 본문/캡션을 반영 — [4] 점수 패리티·[5] 인덱스 형식 PASS), 그 외 홈·카테고리·피드·사이트맵·robots·assets·미영향 imgBox 12 페이지 전부 **byte-불변**. 클린 빌드 2회 결정성 동일(v1.1.1 dist combined sha256 `836fa333f28dc732244175323778834957ac0fe6ad5ca4beca664c72f7371745`). `__version__` 1.1.0→1.1.1 의 dist 누수 0(B1 유지 — 위 leak→html 전환은 빌드 로직 변경 때문이지 버전 문자열 때문이 아니다). 단위 **337**(`src/tests/test_markdown.py` 에 `ParsePhpGlobalsTests` 4 + `SimulatePhpTests` 17 = 21 신설; 단일 한 줄 회귀·다중 블록·`global` strip·`{$var}`/`$var` 보간·작은따옴표 비보간·주석/블록주석·동적 PHP 원문 보존·중첩 괄호·문자열 안 `?>`·미닫힘 블록·`process_html` 통합), 진단 **6/6**([6] JSON-LD 게이트가 article `index.html` 만 스캔하는데 이제 leak 10개가 `.php`→`.html` 이 되어 `ld+json 47 / BreadcrumbList 46`(옛 37/36 + 복구된 10), violations 0 — 게이트 스코프상 정확, 복구 페이지 JSON-LD 도 의미 정합 실증). 정본 lama.pe.kr 출처는 `lama_website-main/PHP/` (사용자 지시로 그 폴더에서 서명 변수 원값을 회수).
> v1.1.5 는 **AdSense URL 기반 광고 차단** 릴리스 — *기능* 릴리스이되 v1.1.4 의 `exclude_pages` (page-type 5종) 를 `exclude_urls` (사이트 내 임의 URL 목록) 로 교체. 빈 리스트로 두면 dist byte-불변(B1 유지 — `__version__` 1.1.4→1.1.5 자체의 dist 누수 0). 한 가지 변경: `site.yaml` `google_adsense:` 의 `exclude_pages` 필드를 **`exclude_urls`** 로 교체 — 사이트-relative 절대 URL 리스트(예 `['/', '/about/', '/404.html']`). 매칭은 정확 일치(case-sensitive · trailing-slash 포함). 페이지별 canonical URL: 홈=`/`, 글=`/<slug>/`, 카테고리=`/<slug_path>/`, 404=`/404.html`, 검색=`/search.php`. v1.1.4 의 page-type 식별자 enum(`article`/`home`/`category`/`404`/`search`) 은 폐기 — 페이지의 "타입" 단위 통제가 "URL" 단위로 일원화돼 **개별 글 차단**(예: `/about/`·`/clear/`)이 자연스럽게 가능해진 것이 핵심 동기. v1.1.3 의 `head_script` 빈 문자열 = 전체 차단 의미는 유지. 호환성: v1.1.4 의 `exclude_pages` 키는 v1.1.5 파서가 무시 → 옛 site.yaml 이 자동으로 "전체 주입" 으로 안전 폴백(compat shim 없이 깨끗한 교체). 코드 변경: `AdSenseConfig.exclude_pages: frozenset[str]` → `exclude_urls: frozenset[str]`, 파서가 leading `/` 보정(`about/` → `/about/`) + `str().strip()` 정규화 + frozenset 변환(대소문자 보존 — URL 표준). 헬퍼 `_apply_adsense_head_placeholder(tpl, page_url)` 시그니처에서 `page_type` → `page_url`; 5 호출 위치 모두 자기 페이지 URL 을 전달. 빌더 인스턴스 속성 `self._adsense_seen_urls: set[str]` 신설 — 매 헬퍼 호출 시 URL 적재, 빌드 종료 직후 새 `_check_exclude_urls()` 가 `exclude_urls - seen` 차집합을 BuildReport warning 으로 보고(오타·삭제된 글 감지). 무결성 = **코드 릴리스 형이되 기본값 byte-불변 형** — 정본 Articles 고정, 불변 `siheonlee.com_v1.1.4` 미수정·4번째-숫자 검증 복사본 `siheonlee.com_v1.1.4.2` 의 v1.1.4 *코드* 클린 재빌드 with `exclude_pages: []` vs v1.1.5 클린 재빌드 with `exclude_urls: []` 의 dist 가 **byte-완전 동일** 목표(두 빈 리스트는 동일 `frozenset()` 으로 정규화돼 5 호출 위치에서 v1.1.3·v1.1.4 와 동일 분기 경로). 운영자가 실제 URL 을 추가하면 매칭된 페이지들의 HTML 에서 자동광고 스크립트 한 줄이 사라지는 것만이 변화. `site.yaml` 의 `exclude_urls: []` 추가는 운영자 입력이라 코드 무결성 항목 아님. 클린 빌드 2회 결정성 동일. `__version__` 1.1.4→1.1.5 의 dist 누수 0(B1). 단위 v1.1.4 의 `test_adsense.py` 갱신(page_type → page_url 분기 + 신규: leading-`/` 보정, case-sensitive, trailing-slash 엄격, 개별 글 차단, seen-URL 적재) · 진단 6/6 승계.

> v1.1.4 는 **AdSense 페이지 타입 제외 리스트** 릴리스 — *기능* 릴리스이되 운영자가 새 필드를 사용하지 않으면 dist byte-불변(B1 유지 — `__version__` 1.1.3→1.1.4 자체의 dist 누수도 0). 한 가지 변경: `site.yaml` `google_adsense:` 블록에 `exclude_pages` 필드 신설 — 문자열·정수 리스트(예 `[404, search]`). 식별자 5종: `article` / `home` / `category` / `404` / `search`. 매칭된 페이지 타입은 그 페이지 head 의 `{{ADSENSE_HEAD}}` placeholder 가 line-eating 으로 제거되어 auto-ads 로더 스크립트가 들어가지 않는다 = Google auto-ads JS 자체가 로드되지 않아 광고 **원천 차단**. 이로써 v1.1.3 의 "전역 ON/OFF 단일 토글" 한계가 풀려 페이지 단위 제어가 가능해진다 — 단 인-페이지 광고 *위치*는 여전히 Google auto-ads 알고리즘이 결정하며, 이 필드는 "어느 페이지 타입에 로더를 둘지" 의 페이지 단위 on/off 만 제어한다. v1.1.3 의 `head_script` 빈 문자열 = 전체 차단 의미는 그대로 — 빈 문자열이면 `exclude_pages` 와 무관하게 5 페이지 모두 미주입. 모르는 식별자는 자연 무시(forward compat). 코드 변경: `AdSenseConfig.exclude_pages: frozenset[str]` 신설(`field(default_factory=frozenset)`), `_parse_adsense_config` 가 리스트(또는 단일 스칼라)를 받아 `str.strip().lower()` 정규화 후 `frozenset` 변환 — yaml 의 `404` 가 int 로 파싱되는 것도 `str()` 캐스팅으로 흡수. 헬퍼 `_apply_adsense_head_placeholder(tpl, page_type)` 시그니처에 `page_type` 추가; 5 호출 위치(`_render_articles`='article', `_build_category_page`='category', `_build_home`='home', `_build_404`='404', `_build_search`='search')가 자기 식별자를 넘긴다. 빌더 클래스 변수 `_ADSENSE_PAGE_TYPES = {'article','home','category','404','search'}` 는 forward-compat 참조용이며 매칭 자체는 `exclude_pages` set membership 으로 단순(미정의 식별자는 무매치 = no-op). 무결성 = **코드 릴리스 형이되 *기본값 byte-불변* 형** (정본 Articles 고정, 불변 `siheonlee.com_v1.1.3` 미수정·4번째-숫자 검증 복사본 `siheonlee.com_v1.1.3.1` 의 v1.1.3 *코드* 클린 재빌드 vs v1.1.4 클린 재빌드 with `exclude_pages: []`(=기본값)의 dist 가 **787=787, 0 added/0 removed/0 changed = byte-완전 동일**). exclude_pages 의 모든 부재 상태(키 부재·빈 리스트·None) 가 동일 `frozenset()` 으로 정규화돼 `head_script and not excluded` 분기에서 v1.1.3 와 동일 경로를 탄다. 운영자가 실제로 페이지 타입을 추가하면(예 `[404, search]`) 매칭된 페이지들의 HTML 에서 `<script async src=...adsbygoogle...>` 한 줄이 사라지는 것만이 변화 — 그 외 자산·feed·sitemap·검색 인덱스 등 본문 무관 산출물 byte-불변. 클린 빌드 2회 결정성 동일. `site.yaml` 의 `exclude_pages: []` 추가는 운영자 입력이라 [§16 v0.8.x 정신의] **코드 무결성 항목이 아니다** — config-driven dist 변화 평가는 코드-vs-코드 diff 에서 공통 인자라 상쇄. `__version__` 1.1.3→1.1.4 dist 누수 0 (B1 유지). 단위 v1.1.3 승계 + `_parse_adsense_config` 의 `exclude_pages` 파싱/정규화·`_apply_adsense_head_placeholder` page_type 분기 테스트 신규(실측치는 진단 리포트 참조) · 진단 6/6 승계.

> v1.1.3 은 **Google AdSense 통합 + 기본 og:image 자산 교체** 릴리스 — *기능* 릴리스라 dist 가 바뀐다 (B1 유지 — `__version__` 1.1.2→1.1.3 자체의 dist 누수 0). **(1) AdSense 통합**: `site.yaml` 에 `google_adsense:` 블록 신설 — `ads_txt`(/ads.txt 본문) + `head_script`(자동광고 스크립트) 두 문자열 필드(`AdSenseConfig` dataclass, `ImageConfig`·`JsonLdConfig` 와 같은 패턴). 두 필드는 빈 문자열/키 부재 시 자동 비활성 — SeoMeta 의 3-state None/''/'text' 원칙과 일관이며 별도 `enabled` 마스터 토글은 두지 않는다(필드 단위로 독립 토글 가능; 둘 다 비우면 v1.1.2 와 동일 동작=광고 미통합). `ads_txt` 는 새 빌드 단계 [11b] `_build_ads_txt` 가 `dist/ads.txt` 로 그대로 기록(robots.txt 와 같은 패턴) — 빈 값이면 미생성하고 이전 빌드의 잔존 `dist/ads.txt` 가 있으면 자동 삭제(stale 가드, v0.5.2 옛 `dist/src/` 정리와 같은 결). `head_script` 는 5 개 템플릿 — article·home·category·404·search.php — 의 `<head>` 에 새 `{{ADSENSE_HEAD}}` placeholder 로 raw 그대로 주입(escape 없음 — author 가 명시한 스크립트 문자열 신뢰); 빈 값이면 placeholder 가 자리한 라인 자체가 strip 되어 빈 줄도 남지 않는다(ROBOTS_META · JSONLD · COMMON_CSS 와 동일 line-eating 패턴, 공용 헬퍼 `_apply_adsense_head_placeholder`). 적용 대상은 사용자가 방문하는 5 개 dist 페이지 종류 전부 — `admin.php`/`src/admin/` 은 빌더가 `Articles/` 만 스캔하므로 자연 제외(설계 원칙 1 일관). **(2) 기본 og:image 자산 교체**: `src/assets/default-og.png` 를 표준 OG 규격으로 갱신(1200×480 → 1200×630, Pretendard SemiBold 폰트로 워드마크 재제작). v1.0.0 의 default_og_image 패스스루 예외 그대로 — 이 자산은 webp 변환·variant 등록 없이 원본 그대로 `dist/assets/` 로 복사돼 SNS 언퍼ler 가 고정 URL 로 가져간다. 무결성 = **코드 릴리스 형·dist 의도적 변경형** (정본 Articles 고정, 불변 `siheonlee.com_v1.1.2` 미수정·4번째-숫자 검증 복사본 `siheonlee.com_v1.1.2.1` 의 v1.1.2 *코드* 클린 재빌드 vs v1.1.3 클린 재빌드의 *열거된* diff): `+dist/ads.txt`, 모든 HTML/PHP head 에 자동광고 스크립트 한 줄 삽입, `dist/assets/default-og.png` byte 교체. 그 외 자산·feed·sitemap·검색 인덱스 등 본문 무관 산출물 byte-불변. 클린 빌드 2회 결정성 동일. `site.yaml` 값(`google_adsense.ads_txt`/`head_script`)은 운영자 입력이라 [§16 v0.8.x 정신의] **코드 무결성 항목이 아니다** — 비활성(블록 비움) 검증에서는 default-og.png 1줄 교체를 제외한 모든 산출물이 v1.1.2 와 byte-동일하다(placeholder line-eating 이 깨끗하게 작동해 빈 줄·잔재 없이 v1.1.2 의 head 와 동일 라인 구성으로 떨어진다). `__version__` 1.1.2→1.1.3 dist 누수 0(B1 유지). 단위 **337** 승계(필드 추가형 — 새 옵션은 옛 동작을 유지하는 디폴트라 기존 테스트 영향 없음, v1.0.1·v1.0.2·v1.1.2 자산성 릴리스 선례와 일관) · 진단 6/6 승계.

> v1.1.2 는 **배포 사고 수정 + imgSlideBox 페이지네이션형 재디자인** 릴리스 — CSS·JS 두 자산이 바뀌므로 dist 가 바뀐다(B1 유지 — `__version__` 1.1.1→1.1.2 자체의 dist 누수 0). **(1) 오류** : v1.1.1 dist 실배포에서 imgSlideBox 가 슬라이드 박스가 아니라 폴더 안 **모든 이미지가 세로로 나열**됐다(사용자 보고, `/clear` 글에서 관찰). 원인 — 정본 lama.pe.kr 의 `PHP/GlobalFunctions.php` 의 `imgSlideBox` 는 단일 `<img>` 의 `src` 를 JS 로 갈아끼워 한 장만 보였지만, siheonlee 정적 빌드(`scripts/markdown.py` 의 `_simulate_imgslidebox`)는 폴더의 모든 이미지를 `<img class="slide">` 로 미리 펼치고 첫 장에만 `.active` 를 준 뒤 `assets/imgslidebox.js` 가 `.active` 를 옮기는 설계다. 즉 "active 한 장만 보이기"는 전적으로 CSS 책임인데 `assets/common_template.css` 에 그 규칙(`.imgSlideBox .slide{display:none}` + `.imgSlideBox .slide.active{display:block}`)이 **처음부터 없어** 모든 슬라이드가 그대로 표시됐다(JS 의 `.active` 토글이 시각 효과가 없었다 — `imgslidebox.js` 의 토글 자체는 정상). 정본 CSS 에도 그 규칙이 없던 이유는 원본 PHP 가 애초에 한 장만 출력해 숨김 CSS 가 불필요했기 때문 — *정적 펼침* 방식으로 바꾼 siheonlee 가 그에 맞는 CSS 를 빠뜨린 것이 결함의 본질. 그 두 줄(`.slide` 숨김 / `.slide.active` 노출)이 핵심 수정이며 절대 제거 금지(제거 시 전체 나열로 회귀). **(2) 재디자인(같은 릴리스에서 함께)** : 사용자 요청으로 좌우 큰 반투명 검정 오버레이 버튼을 폐지하고, 사이트의 `.pagination-nav` 와 같은 절제된 톤의 **점(dot) 인디케이터를 이미지 하단 중앙**에 두는 모던·간결한 페이지네이션형으로 재디자인했다 — `‹ prev · ● ○ ○ … · next ›`, 현재 위치 점은 흰색·확대, 점 클릭으로 해당 슬라이드 이동, 평소 흐릿(opacity 0.6)·hover/`focus-within` 시 또렷(콘텐츠를 시각적으로 압도하지 않는 사이트 페이지네이션 철학 유지), 이미지 위 가독성용 흰색+그림자. **핵심 설계 결정**: 이 `.slide-nav`(prev·점들·next)는 `assets/imgslidebox.js` 가 **런타임에 생성**한다 — `_simulate_imgslidebox` 가 내는 정적 마크업(`<img class="slide">` × N + 정적 `<button class="prev/next">`)은 그대로 두고, JS 가 기존 prev/next 를 하단 바로 옮기고 슬라이드 수(`slides.length`)만큼 점을 만들어 끼운다(점·prev/next 에 aria-label 부여). 그래서 **빌드 산출물(정적 HTML)은 한 글자도 바뀌지 않고**, 디자인 변경이 `common_template.css`·`imgslidebox.js` 두 자산에만 갇힌다 — JS 미동작 환경에선 (오류 수정 CSS 덕에) 첫 장만 보이는 안전한 폴백이며, 슬라이드가 JS 의존이라는 기존 점진적 향상 자세와 일관. `_simulate_imgslidebox`·빌드 로직은 **무변경**. 무결성 = **코드 릴리스 형·dist 의도적 변경형** (정본 Articles 고정, 불변 `siheonlee.com_v1.1.1` 미수정·4번째-숫자 검증 복사본 `siheonlee.com_v1.1.1.1` 의 v1.1.1 *코드* 클린 재빌드 vs v1.1.2 클린 재빌드의 *열거된* diff): `assets/common_template.css` + `assets/imgslidebox.js` **딱 두 파일만 변경**, 786=786·0 added/0 removed, 그 외 글·홈·카테고리·`feed.rss`/`feed.atom`·`sitemap.xml`·`search.php`·이미지·기타 자산 전부 byte-불변(공통 CSS·JS 는 모든 페이지에 `<link>`/`<script src>` 로 *참조*될 뿐 HTML 에 인라인되지 않으며, raster 가 아니라 webp 변환 비대상이라 `_copy_site_assets` 가 그대로 복사 — HTML 산출물 무변경을 [6] ld+json 47/46 불변이 실증). 클린 빌드 2회 결정성 동일(v1.1.2 dist combined sha256 `748997de3e15860866c35732684c9ebf410023259b54a9292ad66a2e30a699fa`). 빌드 *로직* 모듈 `src/scripts/`·`src/templates/` 는 byte-불변; `build.py`·`src/scripts/__init__.py` 변경은 docstring/`__version__`뿐(주석·B1 — dist 미도달, 위 2-파일 diff 가 실증). Python 로직 무변경이라 단위 **337** 승계(신규 테스트 없음 — 순수 CSS/JS 자산 변경, 이 프로젝트는 자산용 테스트 패턴이 없고 같은 성격의 v1.0.1·v1.0.2 도 단위 불변이었던 선례와 일치), 진단 **6/6** 승계([6] JSON-LD 게이트는 글 `index.html` 스코프라 자산 변경 무관 — v1.1.1 의 ld+json 47/46 그대로). `__version__` 1.1.1→1.1.2 의 dist 누수 0 (B1 유지).
...
| v1.1.2 | 2026-05-20 | **배포 사고 수정 + imgSlideBox 페이지네이션형 재디자인** (CSS+JS 자산 릴리스) — v1.1.1 dist 실배포에서 imgSlideBox 가 슬라이드 박스가 아니라 폴더 안 **모든 이미지가 세로로 나열**됨(사용자 보고, `/clear` 글). 원인: 정본 `PHP/GlobalFunctions.php` 의 `imgSlideBox` 는 단일 `<img>` `src` 를 JS 로 교체해 한 장만 보였으나, siheonlee 정적 빌드(`_simulate_imgslidebox`)는 모든 이미지를 `<img class="slide">` 로 펼치고 첫 장에만 `.active` 를 줘 `imgslidebox.js` 가 `.active` 를 옮기는 설계 — "한 장만 보이기"가 CSS 책임인데 `common_template.css` 에 `.imgSlideBox .slide{display:none}`+`.slide.active{display:block}` 이 처음부터 없어 모두 표시됨(JS 정상, 누락 CSS 가 본질; 그 두 줄 제거 금지=회귀). **같은 릴리스에서 디자인도 함께**: 사용자 요청으로 좌우 큰 검정 오버레이 버튼 → 사이트 `.pagination-nav` 톤의 **점(dot) 인디케이터를 이미지 하단 중앙**에 두는 모던·간결한 페이지네이션형으로 재디자인(‹ prev·점들·next ›, 활성 점 흰색·확대, 점 클릭 이동, 평소 흐릿·hover/focus-within 또렷). 핵심: `.slide-nav`(prev·점·next)는 `imgslidebox.js` 가 **런타임 생성** — 정적 마크업(`_simulate_imgslidebox`)·빌드 로직 무변경이라 **정적 HTML 산출물 불변**, 변경이 `common_template.css`+`imgslidebox.js` 두 자산에만 갇힘(JS 미동작 시 오류수정 CSS 로 첫 장만 보이는 안전 폴백). 무결성 = **코드 릴리스 형·dist 의도적 변경형** (정본 Articles 고정, 불변 `siheonlee.com_v1.1.1` 미수정·4번째-숫자 검증 복사본 `siheonlee.com_v1.1.1.1` 의 v1.1.1 *코드* 클린 재빌드 vs v1.1.2): `assets/common_template.css`+`assets/imgslidebox.js` **2 파일만 변경**, 786=786·0 added/0 removed, 그 외(글·홈·카테고리·피드·사이트맵·검색·이미지) 전부 byte-불변, 클린 빌드 2회 결정성 동일(combined sha256 `748997de…`). `__version__` 1.1.1→1.1.2 dist 누수 0(B1). 단위 **337** 승계(순수 자산 — 신규 테스트 없음, v1.0.1·v1.0.2 선례 일치) · 진단 **6/6** 승계([6] ld+json 47/46 불변 — HTML 무변경 실증). |
| v1.1.1 | 2026-05-20 | **배포 사고 수정** (빌드 로직 릴리스) — v1.0.2.2 실배포에서 `/u-hof/`·`/automatic-grading/` 등 imgBox PHP 글이 본문 중간부터 잘림. 원인(사용자 보고 문제 1·2 동일): `scripts/markdown.py` 의 `simulate_php_in_html` 이 **한 호출 한 줄** `<?php f(a) ?>` 만 펼치고, 실제 본문 대부분인 **다중 구문 블록**(`<?php\n global $sig;\n imgBox(…);\n imgBox(…);\n?>`)은 시뮬레이트 실패 → 원본 PHP 가 그대로 dist 로 새고 정적 siheonlee.com 엔 `imgBox()`/`$reference_*` 런타임이 없어 `Call to undefined function` fatal → 그 지점부터 응답 truncate. 수정: (1) `simulate_php_in_html` 을 블록 토큰 스캐너로 재작성(주석 `//`·`#`·`/* */`·`global` 선언·`;` 무시, imgBox/imgSlideBox 만이면 블록 통째 정적 HTML, 다른 살아있는 구문 있으면 원문 보존=동적 PHP 보호); (2) `site.yaml` `php_globals:` 신설 — 정본 `lama_website-main/PHP/GlobalVariables.php` 의 서명 변수(`$reference_hanbyeol`/`$reference_hanbyeol_webDesign` = 김한별 일러스트·웹디자인 크레딧)를 운영자 설정으로 옮겨 캡션 `{$name}` 을 빌드 시 치환(미정의=빈 문자열=PHP 동등); (3) `_simulate_imgbox` 캡션(`exp`) 이스케이프 해제 — 정본 `PHP/GlobalFunctions.php` 의 imgBox 가 `$exp` 를 그대로 echo 했고 작성자가 `<br>`·`&nbsp;`·`<a>`·서명 보간을 캡션에 의도(원 사이트와 동일 결과), `alt` 는 속성값이라 이스케이프 유지. admin 미리보기(`render_one.py`)도 같은 `php_globals` 적용(`test_render_one` 패리티 게이트 갱신). 무결성 = **코드 릴리스 형** (정본 Articles 고정, 불변 `siheonlee.com_v1.1.0` 미수정·4번째-숫자 검증 복사본 `siheonlee.com_v1.1.0.1` 의 v1.1.0 *코드* 클린 재빌드 vs v1.1.1 클린 재빌드 열거 diff): **786=786**, leak `.php` 10개 제거 → 동일 slug `index.html` 10개 추가, `search.php` 1개 변경(BM25 인덱스가 교정된 본문 반영 — [4]/[5] PASS), 그 외 홈·카테고리·피드·사이트맵·미영향 imgBox 12 페이지 전부 byte-불변. 클린 빌드 2회 결정성 동일(combined sha256 `836fa333…`). `site.yaml` 값은 운영자 입력이라 코드 무결성 항목 아님(`__version__` 1.1.0→1.1.1 dist 누수 0, B1). 단위 **337**(`test_markdown` 에 `ParsePhpGlobalsTests`·`SimulatePhpTests` 21 신설) · 진단 **6/6**([6] ld+json 47/46 — 옛 37/36 + 이제 정적 HTML 이 된 글 10, violations 0). |
...
*siheonlee.com v1.1.2 — 빌드 Python + Pillow, 런타임 PHP. 검색 Okapi BM25 메타데이터 3-필드 인라인 인덱스, 이미지 WebP 다중 해상도 자동, 글/홈/카테고리 통일 SEO 메타 + 외부 CSS·`template:` 지원, schema.org JSON-LD (글 페이지 `Article`+`BreadcrumbList`, 기존 메타 태그 보강·off 스위치 동반). 빵부스러기는 정확한 의미로 출시 — 중간 조상=자기 중첩 URL, 글 말단=글 제목 (nav-tracker/JSON-LD 단일 공유 소스); JSON-LD 의미 게이트 [6] 동반. v1.0.0 첫 정식 릴리스(기본 og:image 자산 패스스루 + `About` 검색 비허용). v1.0.1 소분류 헤더 링크화 — 우측 → 화살표 폐지, 소분류명 글씨 자체가 그 소분류 페이지로 가는 스타일 없는 깔끔한 링크(`.gap .subcat-link`: color inherit·밑줄·호버 없음); 정본 Articles 고정 v1.0.0 코드 클린 재빌드(불변 v1.0.0 미수정·4번째-숫자 검증 복사본) 대비 실측 5파일 열거 diff(`assets/common_template.css` + `blog`·`project`·`research`·`study`/`index.html`, 786=786·0 add/rm·781 byte-동일)·그 외 byte-불변, 클린 2회 결정성 동일, 단위 313·진단 6/6 승계. v1.0.2 메인페이지 Recent posts 기본 출력 개수 코드 디폴트 5→10 (사용자 정책; `HOME_PER_PAGE_DEFAULT` + 문서 정합) — 정본 `Articles/meta.yaml` 이 `per_page:10` 명시라 상수 dormant, v1.0.1 코드 클린 재빌드 대비 dist 786=786·0 changed = byte-완전 동일. v1.1.0 로컬 글쓰기 `admin.php` 추가 (§ 17) — `build.py` 패턴 미러(얇은 진입점 + `src/admin/`), 로컬 전용 단일 사용자, 미리보기는 `scripts.markdown` 재사용으로 본문 byte-충실(`test_render_one` 게이트), 빌드 로직 0 변경이라 dist 는 v1.0.2 와 byte-동일. v1.1.1 배포 사고 수정 — 다중 구문 imgBox PHP 블록(`<?php global …; imgBox(…); imgBox(…); ?>`)이 시뮬레이트 실패로 원본 PHP 가 dist 로 새 정적 서버에서 본문이 그 지점부터 잘리던 결함을 `simulate_php_in_html` 블록 스캐너 재작성으로 해소; 정본 서명 변수는 `site.yaml` `php_globals:` 로, 캡션은 raw(원 사이트 동등). v1.1.0 코드 클린 재빌드 대비 leak `.php` 10→`.html` 10·`search.php` 1 변경·그 외 byte-불변(786=786), 클린 2회 결정성 동일, 단위 337·진단 6/6. v1.1.2 배포 사고 수정 + imgSlideBox 페이지네이션형 재디자인 — imgSlideBox 가 슬라이드가 아니라 모든 이미지를 세로로 나열하던 결함(`/clear`)을 해소(정적 빌드는 모든 이미지를 `<img class="slide">` 로 펼치고 `imgslidebox.js` 가 `.active` 를 옮기는데 `common_template.css` 에 `.slide{display:none}`/`.slide.active{display:block}` 가 없어 모두 표시됐다 — JS 정상·CSS 누락이 본질, 원본 PHP 는 한 장만 출력해 불필요했던 규칙; 그 두 줄 제거 금지), 같은 릴리스에서 좌우 검정 오버레이 버튼을 사이트 `.pagination-nav` 톤의 점(dot) 인디케이터 이미지 하단 중앙 배치(모던·간결한 페이지네이션형, 활성 점 흰색·확대·클릭 이동, `imgslidebox.js` 가 런타임 생성이라 정적 HTML 불변)로 재디자인. `assets/common_template.css`+`assets/imgslidebox.js` 두 자산만 변경(마크업 `_simulate_imgslidebox`·빌드 로직 무변경); v1.1.1 코드 클린 재빌드 대비 786=786·0 add/rm·2 파일만 변경·그 외 byte-불변, 클린 2회 결정성 동일(combined sha256 `748997de…`), 단위 337·진단 6/6([6] ld+json 47/46 불변). (2026-05-20)*
