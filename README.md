...
|---|---|---|
| **v1.14.6** | 2026-06-10 | **The Update button now streams its log live too — the same fix the Build button got in v1.14.5; previously the page sat "loading" through the whole download → integrity → backup → overlay → migration chain and the log appeared at once at the end.** `?a=update` switches from the blocking `admin_run_heron` to the build/deploy streaming path: output buffering off, a new `admin_run_update_stream` in `proc.php` (stderr merged via the opt-in `2 => ['redirect', 1]`, `PYTHONUNBUFFERED=1`), the shared `admin_log_widget_head()` log widget, and the result banner below the log since the exit code is only known at the end (the restart notice is unchanged). On the Python side `_action_update` now logs with `print(..., flush=True)` — unlike the builder it had no per-line flush, so nothing would stream through a pipe without it. Side benefit: the view/layout/proc/i18n table are all loaded *before* the overlay replaces them, so the page renders from consistent old-version code — previously `update.php` was require'd *after* the overlay, letting a new-version view be parsed by old-version Pond. Admin-UI plus a one-line CLI flush change; dist output byte-identical. No schema change (stamp 1.14.5 → 1.14.6); MANIFEST 89; 606 unit tests. |
...
*Heron v1.14.6 — build with Python + Pillow, runtime PHP (OPcache recommended). Full release history in [§ 16](#16-changelog).*
