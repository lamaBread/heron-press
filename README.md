...
|---|---|---|
| **v1.9.5** | 2026-06-03 | **Locale-pack translation right-sizing + low-level guards unified to English (v1.9.x language-pack line).** Following Korean dev convention, settled technical terms in the `ko` canonical pack are right-sized to English — inconsistencies where the same term was English in one place and transliterated in another are unified to English (cache `hit/miss` ↔ `히트/미스`: `build.console.cache`·`build.report.md_cache_hitmiss`; `raster` ↔ `래스터`: admin home), and the article-list Actions column header `동작`→`작업`. The Pond security guards are English-consistent too — the mixed Korean/English line in the 403 SAPI/loopback guard (which runs before the i18n bootstrap, so `t()` is unavailable) is now fully English (405/CSRF were already English). The `보완 필요`/`살펴볼 사항` issue/warning bodies are kept deliberately as writer-facing UX. No `user/` schema change → no new migration step (stamp 1.9.4 → 1.9.5); MANIFEST 86 (count unchanged); dist byte-identical; 519 unit tests + 6/6 diagnostics. Only the tool=`ko` console cache-count label changes (`히트/미스`→`hit/miss`); all other ko output is unchanged. |
...
*Heron v1.9.5 — build with Python + Pillow, runtime PHP (OPcache recommended). Full release history in [§ 16](#16-changelog).*
