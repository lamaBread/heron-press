...
| 14 | `dist/search.php` 단일 파일 생성 — 메타데이터 3-필드 (title / description / tags) BM25 인덱스 + 토크나이저 + 점수 계산기를 한 파일에 PHP 정적 배열 / 함수로 인라인 (v0.6.0) |
...
**폴백 결과가 빈 문자열이면 해당 태그 자체를 출력하지 않습니다.** `<meta name="description" content="">` 같은 빈 태그는 절대 생성되지 않습니다. `''` (빈 문자열) 과 `None` (키 부재) 은 산출물에서는 동일하게 태그 누락으로 처리되지만, `seo.description` 의 경우에 한해 `''` 는 작성자 실수로 간주하여 BuildReport 에도 기록됩니다 (자세한 의미는 [§ 17 의 설계 원칙 10](#17-설계-원칙과-한계) 참조).
...
>
> **v0.5.5 변경:** SEO description / og_image / 갤러리 썸네일 / 피드 summary 의 **본문 폴백 폐기** ([§ 17 의 설계 원칙 10](#17-설계-원칙과-한계) 참조). author 가 `meta.yaml` 의 `seo:` 블록에 명시한 값만 사용. `seo.description` 누락 시 빌드는 통과하지만 BuildReport 의 issue 에 기록되어 작성자가 보완할 수 있도록 안내.
>
...
**검증:** 빌드 6 글 / 2 카테고리 / 0 abort (description 누락 issue 4 건 = about / home / blog / blog/tutorials — 콘텐츠 측 잔존, v0.6.1 까지는 글 1 건만 잡혔던 게 v0.6.2 부터 홈/카테고리도 잡혀 정확한 미완성 글 목록이 됨). 단위 테스트 188/188. 진단 5/5 PASS (단위 테스트 + sha256 결정성 22 파일 일치 + PHP -l + Python↔PHP BM25 점수 패리티 8 쿼리 × 12 매치 + 인덱스 v4 형식). `diff -rq` v0.6.1/dist vs v0.6.2/dist: 5 파일 차이 (`index.html` / `blog/index.html` / `blog/tutorials/index.html` = 새 메타 태그 추가, `feed.atom` / `feed.rss` = generator v0.6.1→v0.6.2 자동 갱신). 글 페이지 6 개 (`about/index.html`, `hello-world/index.html`, `section-markers-demo/index.html`, `pagination-demo-{one,two,three}/index.html`) 모두 v0.6.1 과 *바이트 동일* — 회귀 가드 통과.

**적용 메모:** 옛 글/카테고리/홈 meta.yaml 변경 의무 없음. 변경 후 새 description 누락 리포트를 보고 `Articles/meta.yaml` (홈) / `Articles/<카테고리>/meta.yaml` 에 `seo.description` 을 채우면 됨. 글의 description 정책과 정확히 동일. 옛 빌드의 dist 위에 v0.6.2 빌드를 돌리면 5 개 파일이 갱신되고 나머지는 그대로 보존.

**미수용 (차기 의제):** 태그별 색인 페이지 (`/tag/foo/`) / 검색 결과의 카테고리·태그 필터 UI / Parsedown PHP↔Python 동등성 비교 트리 동봉 (의도된 한계).

### v0.6.1 (2026-05-15) — 문서·주석·산출물 가독성 안정화 3회차 (코드 동작 변경 0)

v0.5.0 → v0.6.0 의 누적 변경 이후 진행한 안정화 패치의 3 회차. 1 회차 (A 그룹, 코드 버그 4 건) 와 2 회차 (B/C 그룹, 문서·주석·dist 가독성 11 건) 가 같은 날 처리됐고, 그 직후의 면밀 재감사에서 드러난 잔존 drift 9 건을 한 라운드로 마무리. **빌더 로직·산출물 동작 변경 0** — dist 의 모든 파일은 v0.6.0 과 바이트 동등 (단, dist/search.php 의 헤더 주석 텍스트만 placeholder 누수 정정 차원에서 달라짐).

| 그룹 | 위치 | 내용 |
|---|---|---|
| D | [Articles/Blog/Hello World/meta.yaml](Articles/Blog/Hello%20World/meta.yaml) | SEO 본문 폴백 안내가 v0.5.5 의 본문 ↔ 메타데이터 분리 원칙 (§ 17 의 설계 원칙 10) 으로 거짓이 됐던 부분 + `tags` 안내가 v0.6.0 의 BM25 색인 합류 (w_tags=2.0, phrase boost ×2.5) 후에도 "미사용" 으로 굳어 있던 부분 동시 갱신. |
| E | [Articles/Blog/meta.yaml](Articles/Blog/meta.yaml) | layout 안내 "'gallery' / 향후 확장 예정" 이 v0.5.3 의 gallery 구현 후에도 굳어 있던 부분 정정 (2 회차 패치 때 형제 카테고리 메타 한 곳을 빠뜨렸던 누락분). |
| F | [scripts/models.py](scripts/models.py) | `ArticleMeta.tags` 의 v0.5.3 변경 절 + 필드 옆 인라인 주석 두 곳이 "현재 빌드 산출물에서 직접 쓰이지는 않지만" 으로 굳어 있던 부분을 feed `<category>` + BM25 색인 합류 사실로 갱신. |
| G | [templates/search_tokenize.php](templates/search_tokenize.php), [templates/search.php](templates/search.php) | (G-1) 토크나이저 헤더 주석의 require_once 흐름이 v0.6.0 의 인라인 동작과 모순되던 부분 갱신. (G-2) search.php 헤더 주석의 placeholder 누수 결함 해소 — 안내 줄 `(d) {{LANG}} / {{PAGE_TITLE}} / ...` 의 placeholder 토큰이 빌더의 `_render_template` 단순 치환에 의해 dist 의 *주석 안* 에서도 치환되어 dist/search.php 헤더가 자기 결과를 인용하던 메타-광경 정리. 빌더 코드는 그대로, 템플릿 텍스트만 plain 변수명 (LANG, PAGE_TITLE, …) 으로 정정. |
| H | [README.md](README.md) §3 / §12 / §13 | (H-1) 파일 트리에서 `search_tokenize.php` 의 v0.6.0 인라인 사실 누락 정정. (H-2) §12 마크다운 파이프라인 도식이 v0.5.5 폐기된 `first_paragraph` / `first_image` 를 여전히 그리고 있던 부분 정정 (test_markdown.py 의 회귀 가드와의 부조화 해소). (H-3) §13 토크나이저 PHP 측 안내의 require_once 흐름이 같은 절의 v0.6.0 인라인 안내와 자기모순이던 부분 정리. |

**검증:** 빌드 6 글 / 2 카테고리 / 0 abort, sha256 결정성 유지. 단위 테스트 179/179. 진단 5/5 PASS (Python↔PHP BM25 패리티 8 쿼리 × 12 매치 모두 1e-6 이하). dist 의 모든 파일이 v0.6.0 과 바이트 동등 (search.php 헤더 주석만 placeholder 누수 정정으로 변경).

**미수용 (차기 의제) — v0.6.1 시점:** 홈/카테고리 페이지의 SEO 메타 태그 출력 (§ 17 한계 표). 태그별 색인 페이지 / 검색 결과의 카테고리·태그 필터 UI. Parsedown PHP↔Python 동등성 비교 트리 동봉 (의도된 한계).

> v0.6.2 에서 첫 항목 (홈/카테고리 SEO 메타) 해소. v0.6.2 의 release notes 참조.

### v0.6.0 (2026-05-15) — 검색 메타데이터 3-필드 색인 + 정적 PHP 인라인 인덱스 + 단위 테스트 본격 확장

v0.5.5 시점 한계 표의 두 항목 ("정적 검색 인덱스 본문 포함" + "테스트 부분적") 을 동시에 해소. 사용자 결정 4 건의 종합 — (1) 색인 대상 본문 → (title / description / tags) 3-필드, (2) 인덱스는 search.php 안에 PHP 정적 배열로 인라인 (OPcache 활용), (3) BM25 가중치는 기본 책정 (title 3.0 / description 1.5 / tags 2.0, phrase boost title 2.0 / desc 1.5 / tags 정확매치 2.5), (4) description 누락 글은 빈 문자열로 색인 (자연스럽게 점수 0 으로 처리, BuildReport 가 별도 기록). 단위 테스트는 BM25 외 모든 빌드 모듈을 커버하도록 확장 + 진단 스크립트 신설.

| 변경 | 내용 |
|---|---|
| 본문 색인 폐기 + 메타데이터 3-필드 색인 | [scripts/search.py](scripts/search.py) 의 `build_search_index()` 가 인덱스 v4 로 전환. 색인 대상이 `body` 단일 → `(title, description, tags)`. 본문 평문은 docs[].body_snippet 으로 *앞 1500 자만* 보존되며 BM25 점수에 들어가지 않음 (검색 결과 미리보기 스니펫 추출 전용). `BM25_PARAMS` 가 6 → 12 키로 확장 (필드별 k1·b·w + 필드별 phrase_boost). `noindex` 글은 인덱스에서 제외 (sitemap / feed 와 일관). |
| 정적 PHP 인라인 인덱스 | dist/search-index.json 폐기. 빌더의 `_build_search` 가 [templates/search.php](templates/search.php) 의 sentinel 코멘트 (`/* INLINE: SEARCH_TOKENIZE */`, `/* INLINE: SEARCH_BM25 */`, `/* INLINE: SEARCH_INDEX */ []`) 자리에 토크나이저 본문 + BM25 함수 본문 + PHP 정적 배열 리터럴을 인라인. 결정적 직렬화 (`scripts/search.py` 의 `php_array_literal`) 가 같은 입력에 대해 같은 PHP 텍스트 보장. OPcache 가 search.php 의 바이트코드를 캐시 → 매 요청마다 JSON 파싱 / 디스크 IO 0. v0.5.x 의 search_tokenize.php / search_bm25.php / search-index.json 세 파일은 v0.6.0 빌더가 명시적으로 unlink. |
| BM25 가중치 + phrase boost | title `k1=1.2 / b=0.5 / w=3.0 / phrase=2.0` (v0.5.0 그대로) + description `k1=1.5 / b=0.75 / w=1.5 / phrase=1.5` (v0.5.0 body 자리) + tags `k1=1.2 / b=0.3 / w=2.0 / phrase=2.5` (정확매치만 부스트 — substring 노이즈 방지). title > tags > description 의 가중치 순서는 author 가 직접 적은 신뢰도 (title = 글의 정체성, tags = 명시 주제어, description = 압축된 한 줄 설명) 의 직관에 따른 책정. |
| 단위 테스트 본격 확장 | [tests/test_bm25.py](tests/test_bm25.py) 31 개 (v0.5.0 의 25 → v0.6.0 의 31; v4 포맷 적응 + body 미색인 / tags 정확매치 부스트 / 결정성 가드 등 신규 6 케이스). 신규 9 개 파일: [test_builder.py](tests/test_builder.py) (10), [test_markdown.py](tests/test_markdown.py) (24), [test_seo.py](tests/test_seo.py) (14), [test_sitemap.py](tests/test_sitemap.py) (6), [test_feed.py](tests/test_feed.py) (13), [test_yaml_parser.py](tests/test_yaml_parser.py) (15), [test_slugs.py](tests/test_slugs.py) (14), [test_report.py](tests/test_report.py) (8), [test_parsedown.py](tests/test_parsedown.py) (18), [test_php_literal.py](tests/test_php_literal.py) (26 — PHP CLI 가용 시 round-trip 까지 포함). 합 **179 개**. Parsedown PHP↔Python 동등성 비교는 본 트리에 동봉하지 않음 (사용자 포팅 시 별도 검증). |
| 진단 스크립트 신설 | [tests/run_diagnostics.py](tests/run_diagnostics.py) — 한 번 실행으로 5 개 항목 점검: (1) 단위 테스트 전체, (2) 두 번 빌드한 dist 의 sha256 일치 (결정성), (3) `php -l dist/search.php` (PHP 구문), (4) Python↔PHP BM25 점수 패리티 (8 쿼리, 모든 매치의 점수가 1e-6 이하), (5) dist/search.php 의 인덱스 v4 형식 검증 (top fields / doc keys / df_body 부재 등). 리포트가 `tests/diagnostics_report.txt` (또는 `--report-path` 로 override) 에 저장되어 사용자가 열람 가능. `--no-php` / `--no-build` 옵션으로 PHP 없는 환경 / 테스트만 돌리는 환경 지원. |

**검증 (6 글 / 2 카테고리):** 빌드 0 abort, sha256 결정성 유지. 단위 테스트 179/179 (BM25 31 + 비-BM25 148). 진단 5/5 PASS. Python↔PHP BM25 점수 8 쿼리 × 12 매치 모두 1e-6 이하. dist 크기 v0.5.5 의 4 파일 ~33 KB → v0.6.0 의 search.php 단일 ~28 KB (-15%). BM25 시맨틱 회귀 0 (긴 글 압도 / 흔한 한글 bigram 의 희귀 영문 압도 / tags substring 부스트 / 본문 단어 점수 / 결정적 키 순서 / 0 점수 phrase boost 6 개 가드).

**적용 메모:** 옛 글 meta.yaml 변경 의무 없음 — 단, 본문에만 있던 핵심 단어는 author 가 `seo.description` 이나 `tags` 에 명시해야 검색됨 (v0.5.5 의 `seo.description` 필수화와 자연 결합). v0.5.5 dist 위에 그대로 빌드 가능 — `_build_search` 가 옛 `search-index.json` / `search_tokenize.php` / `search_bm25.php` 자동 정리. **PHP OPcache 활성화 권장** — 꺼져 있어도 동작하지만 매 요청마다 search.php (인덱스 포함) 재파싱. `opcache.enable=1` 한 줄이면 바이트코드 캐시에 상주. 외부 의존성은 v0.5.5 와 동일 (Pillow + `images.enabled=false` 옵션).

**미수용 (차기 의제):** 홈/카테고리 페이지 SEO 메타 태그 (§ 17). 태그별 색인 페이지 / 검색 결과 카테고리·태그 필터 UI. Parsedown PHP↔Python 동등성 비교 트리 동봉.
...
**검증 (6 글 / 2 카테고리):** 0 abort, 결정성 유지. 리포트에 `seo.description` 미작성 글이 "보완이 필요한 항목" 으로 묶여 표시. 산출물의 `<meta name="description">` / `og:description` / `twitter:description` 모두 author 가 쓴 값만 출력 (부재 시 세 태그 모두 누락). 피드도 description 있는 글만 `<summary>` / `<description>` 출력. BM25 단위 테스트 25개 그대로 통과.

**미수용 (차기 의제):** 홈/카테고리 페이지 SEO 메타 태그 (§ 17). 태그별 색인 페이지 / 태그 검색 가중치 / 관련 글 / 갤러리 타일 추가 표시 옵션 (v0.5.3 이월). 빌드 모듈 단위 테스트 디렉터리 확장 (v0.4.2 로드맵 B-1).
...
**검증 (6 글 / 2 카테고리):** 0 경고, 결정성 보장. BM25 단위 테스트 25개 그대로 통과 (title / truncate / nav 변경 모두 검색·인덱스 모듈과 독립). `truncate_description` 11/11 케이스 통과 (영문 단어 한가운데 backup, 한국어 글자 단위 절단, 단일 긴 영문 단어 fallback). 기본 빌드의 `<title>`: 홈=`Lama` / Blog=`Blog` / 404=`Not Found` / Search=`Search` 등 폴백 체인 정상. nav 정렬은 기본 (`nav_priority=0`) 알파벳 순; 오버라이드 빌드에서 큰 값 먼저 동작 확인.

**적용 메모:** meta.yaml / 카테고리 meta.yaml / site.yaml 변경 의무 없음. 모든 신설 필드 (`nav_priority` / `title` / `seo` / `error_404_title` / `search_title`) 는 선택. v0.5.3 dist 위에 그대로 빌드 가능 — 사이트 디폴트가 빈 문자열이면 출력 차이도 없음.

**미수용 (차기 의제):** 홈/카테고리 페이지 SEO meta 태그 (description / og_* / twitter_*) — `seo:` 블록 파싱은 되지만 `<title>` 외 출력은 글에만 적용. description 폴백 추출 범위 (v0.4.2 로드맵 B-5). 단위 테스트 디렉터리 확장 (v0.4.2 로드맵 B-1).
...
**검증 (6 글 / 2 카테고리):** 0 경고. BM25 단위 테스트 25개 그대로 통과 (feed/gallery/tags 모두 검색·인덱스 모듈과 독립). `dist/feed.atom` / `dist/feed.rss` 가 well-formed XML 검증 통과. Tutorials 카테고리의 `layout: gallery` 가 자기 페이지 + 부모 Blog 페이지 양쪽에서 `<section class='paginated listup-gallery'>` 로 출력. 갤러리 썸네일이 webp 변종 srcset 으로 자동 부착. 글 / 홈 / 카테고리 `<head>` 에 두 피드 `<link rel='alternate'>` 정상 삽입. 검색·sitemap·robots·redirect 산출물은 v0.5.2 와 동등.

**적용 메모:** 기존 글 meta.yaml 변경 의무 없음 — `tags` 없어도 정상 빌드 (빈 리스트). 갤러리는 카테고리 meta.yaml 의 `layout: gallery` + 글의 `seo.og_image` (또는 본문 첫 이미지) 로 활성화. v0.5.2 dist 위에 그대로 빌드 가능 — `feed.atom` / `feed.rss` 두 파일 추가 + 페이지 `<head>` 에 `<link rel='alternate'>` 두 줄 추가. 외부 의존성은 v0.5.2 와 동일 (Pillow); 피드 모듈은 stdlib 만.

**미수용 (차기 의제):** 태그별 색인 페이지 (`/tag/foo/`). 태그를 검색 가중치에 반영. 피드 비활성화 site.yaml 토글. 피드 entry 본문 전체 (`<content>`). 갤러리 타일 추가 표시 옵션. v0.4.2 로드맵 B-5 / B-6 이월.

---

### v0.5.2 (2026-05-14) — 자산 경로 일원화 (글 자산은 글 폴더 안으로)

v0.1 ~ v0.5.1 까지 글 자산은 `dist/src/{slug}/` 하위 트리에 떨어졌습니다. 글 폴더 안에서는 `face_img.png` 가 `content.html` 옆에 있는데, 빌드 후에는 `dist/about/index.html` (본체) 과 `dist/src/about/face_img-778.webp` (자산) 처럼 두 갈래로 갈라졌습니다. "글마다 폴더 하나에서 본문과 첨부를 관리한다" 라는 시스템 자기 정체성과 dist 구조 사이의 비일관성이 있던 셈인데, v0.5.2 는 이 비일관성을 정리합니다.

| 변경 | 내용 |
|---|---|
| dist 출력 트리 | 글 자산이 옛 `dist/src/{slug}/...` 대신 `dist/{slug}/...` (글의 index.html 과 같은 폴더) 에 떨어집니다. raster 이미지의 webp 변종, 단일 첨부 파일, `imgs/` / `files/` 등 서브 폴더 모두 글 폴더 안에 동거. dist 트리에서 글 단위의 모든 산출물이 한 폴더 안에 모입니다. |
| URL 스킴 | `/src/{slug}/foo.jpg` → `/{slug}/foo.jpg`. `rewrite_asset_path` ([scripts/markdown.py](scripts/markdown.py)), `_simulate_imgbox` / `_simulate_imgslidebox`, `_sync_assets` 의 `url_prefix` 모두 새 스킴으로 출력. 글 본문의 상대 경로 (`./imgs/foo.jpg` 등) 는 빌드가 자동 재작성하므로 글 측에서 손볼 것 없음. |
| `reserved_slugs` 정리 | `src` 항목 제거 — 더 이상 충돌 가능 디렉터리가 아닙니다. `assets`, `search` 두 항목만 남음 ([site.yaml](site.yaml)). |
| `assets/` 의 역할 명시화 | README §3·§4-5 에서 `assets/` 를 **"사이트 어디서든 로드 가능한 전역 자산 보관 장소"** 로 명시. 글 단위 자산은 글 폴더 안 (= dist 의 `/{slug}/...`), 사이트 공통 자산은 `assets/` (= dist 의 `/assets/...`) 라는 이분법 — `dist/src/` 라는 제3의 위치가 사라졌습니다. |
| `_prune_article_assets` 가드 | 글 자산이 글 본체 산출물 (`index.html` / `index.php`) 과 같은 폴더에 동거하게 되어, asset 정리 단계에서 index 파일을 잘못 삭제하지 않도록 `expected` set 에 명시적으로 포함. asset sync 가 article render 보다 먼저 돌지만, 두 번째 빌드부터는 이전 빌드의 index 파일이 같은 폴더에 이미 존재한다. |
| `_prune_orphans` 의 옛 src 트리 정리 | 빌드 시작 시점에 `dist/src/` 가 남아 있으면 통째로 제거. v0.5.1 에서 v0.5.2 로 점진 빌드를 돌릴 때 자동으로 옛 자산 위치가 청소됩니다. 첫 v0.5.2 빌드는 그래도 `--clean` 권장 — 옛 자산이 새 위치로 올바르게 이전됐는지 확인하기 쉽습니다. |

**검증:** About 의 face_img.png → `dist/about/face_img-{400,778}.webp` 가 index.html 과 같은 폴더에 동거. `<img>` src / srcset 도 새 URL 스킴 출력. 옛 `dist/src/` 트리가 존재하지 않음. BM25 단위 테스트 25개 그대로 통과. 비-이미지 산출물 (검색, sitemap, robots, redirect, 404, 페이지 HTML 구조) 은 v0.5.1 과 동등.

**적용 메모:** 글마다 별도 작업 없음 — 본문의 상대 경로 그대로 둬도 빌드가 새 URL 스킴으로 출력. meta.yaml / site.yaml 변경 의무 없음. v0.5.1 dist 위에 그대로 빌드해도 `_prune_orphans` 가 옛 `dist/src/` 트리 자동 정리 (재현성 확보가 필요하면 `--clean` 권장). 외부 의존성은 v0.5.1 과 동일.

---

### v0.5.1 (2026-05-14) — 이미지 자동 최적화 + lazy loading

v0.5.0 까지의 빌더는 글 폴더 / `assets/` 의 raster 이미지를 그대로 dist 에 복사했습니다. v0.4.0 에서 전역 noindex 가 풀려 글이 검색엔진에 색인되기 시작한 뒤로 SEO 가 실제 트래픽에 영향을 미치는 단계가 되었는데, Google PageSpeed Insights / Lighthouse 가 ① "Serve images in next-gen formats (WebP)" ② "Properly size images" ③ "Defer offscreen images (loading=lazy)" 세 항목을 명시적으로 감점하면서, 빌드 산출물의 이미지 정책이 Core Web Vitals (LCP) 와 모바일 검색 랭킹에 직접 영향을 미치는 구조로 굳어졌습니다.

v0.5.1 은 이 세 항목을 빌드 단계에서 한꺼번에 해소합니다. 글마다 별도 마크업을 손볼 필요 없이, `![alt](photo.jpg)` 한 줄이나 `<img src="photo.png" alt="">` HTML 그대로 둬도 빌드 후 dist 에는 WebP 다중 해상도 변종 + responsive `<img srcset>` + `loading="lazy"` 가 자동으로 떨어집니다.

| 변경 | 내용 |
|---|---|
| 외부 의존성 도입 — Pillow ([scripts/images.py](scripts/images.py)) | v0.4.1 의 "빌드 PHP 의존 제거" 와 같은 보수적 의존성 정책에도 불구하고 도입. WebP 인코딩 / 정확한 리샘플링은 stdlib 만으로 현실적으로 구현 불가 (수천 줄의 디코더/리샘플러). Pillow 는 PSF 라이선스 (BSD 호환), `pip install Pillow` 한 줄. 두 가지 안전망: ① `images.enabled=false` 로 끄면 raster 이미지를 v0.5.0 처럼 그대로 복사하고 HTML 후처리도 안 함 → Pillow 없이 빌드 통과. ② `images.lazy_loading=true` 는 별도 토글이라 enabled=false 여도 `loading="lazy"` 부착은 동작 — Pillow 없는 환경에서도 SEO 의 lazy loading 부분은 챙길 수 있음. |
| WebP 다중 해상도 변종 자동 생성 | raster 이미지 (.jpg .jpeg .png .gif) 가 `_sync_assets` / `_copy_site_assets` 단계에서 Pillow 로 디코드 → site.yaml 의 `images.widths` (기본 `[400, 800, 1600]`) 중 원본 width 이하인 너비별로 LANCZOS 리샘플 → WebP 인코딩 (quality 85, method 6). 결과: `{stem}-{width}.webp` 파일들이 dist 의 원본 자리에 떨어지고 **원본 raster 파일은 dist 에 복사되지 않음**. SVG / 이미 webp 인 파일은 그대로 복사 (벡터 / 이미 최적화). 캐시 — variant mtime ≥ 원본 mtime 이면 재인코딩 건너뜀. |
| HTML 후처리 — `transform_img_tags` ([scripts/images.py](scripts/images.py)) | 글 본문의 모든 `<img>` 를 정규식으로 잡아 속성을 파싱 + 재조립. 변환 결과: ① src 가 webp primary variant (800px 부근, 디스플레이용) 로 치환, ② srcset 에 변종별 `{stem}-{w}.webp {w}w` 등록, ③ sizes 디폴트 `(max-width: 800px) 100vw, 800px` 부착 (siheonlee.com main column 너비 기준 — `images.default_sizes` 로 사이트마다 조정), ④ `loading="lazy"` 자동 부착 (이미 loading 속성이 있으면 보존 — eager 명시 의도 존중). 외부 URL (https://, //, data:) 의 `<img>` 와 SVG `<img>` 는 src 그대로 두고 `loading="lazy"` 만 부착. |
| 빌드 파이프라인 단계 순서 재배치 ([scripts/builder.py](scripts/builder.py)) | v0.5.0 까지는 `[5] _render_articles → [6] _sync_assets → ... → [9] _copy_site_assets` 순. v0.5.1 에서 asset 단계가 article render 보다 먼저 와야 (asset 단계가 `self.image_variants` 를 채워야 article render 의 `<img>` 후처리가 그 정보를 참조할 수 있어) `[5] _sync_assets → [6] _copy_site_assets → [7] _render_articles → ...` 로 재배치. 이 순서 변경은 v0.5.0 의 빌드 산출물에 영향을 주지 않는다 — 두 asset 단계는 _render_articles 의 입력에 의존하지 않으므로 (`articles` / `categories` 만 사용) 결정성 유지. |
| 새 site.yaml 블록 `images:` | 기본값 보유 — 옛 site.yaml 그대로 두면 자동으로 v0.5.1 기본 정책으로 빌드. 키: `enabled` (기본 true) / `widths` (기본 [400, 800, 1600]) / `max_width` (기본 widths 의 max) / `quality` (기본 85) / `lazy_loading` (기본 true) / `default_sizes` (기본 `"(max-width: 800px) 100vw, 800px"`). [SiteConfig.images](scripts/models.py) 가 [ImageConfig](scripts/images.py) 를 보유. |
| `_prune_article_assets` 의 webp 자매 파일 인식 | raster 원본 자리에 떨어지는 `{stem}-*.webp` 변종 파일을 자매로 인식. 원본 파일명이 dist 에 없는 상태에서도 다음 빌드에서 변종이 stale 로 잘못 정리되지 않도록. 옛 변종 (예전 widths 설정으로 만들어진 잔재) 은 같은 stem 의 새 widths 와 충돌하지 않는 한 보존. |

**검증 (출시 시점):** About 의 face_img.png (1,090,494 bytes) → face_img-400.webp (8,948) + face_img-778.webp (33,414). 원본 대비 30× ~ 120× 절감. `<img>` 후처리 결과 src / srcset / sizes / loading="lazy" 모두 정상 부착. 두 번째 빌드 (캐시) — 변종 mtime 동일 (재인코딩 건너뜀). 외부 URL / SVG — src 그대로 + loading="lazy" 만. BM25 단위 테스트 25개 그대로 통과.

**호환성 노트:** v0.5.0 까지의 dist 와 v0.5.1 의 dist 는 검색 / sitemap / robots / redirect / 404 / 페이지 HTML 구조 모두 동일. 차이는 본문 `<img>` 마크업이 webp + srcset + lazy 로 확장 + raster 자리에 webp 변종 파일. meta.yaml / 카테고리 meta.yaml 변경 불필요. site.yaml 의 `images:` 블록 미명시 시 v0.5.1 기본값으로 동작. 외부 의존성은 `pip install Pillow` 한 줄 — 회피는 `images: { enabled: false }`.

#### v0.5.0 → v0.5.1 마이그레이션 시 주의

- `pip install Pillow` (의존성 1개 — requirements.txt 미도입, 다음 의존성 추가 시 검토).
- 첫 빌드는 `--clean` 권장 (옛 dist 와 새 webp 공존 방지).
- Apache `mod_mime` 가 `.webp` 인식하는지 확인. 미인식 시 `AddType image/webp .webp` 한 줄.
- `images.widths` 조정 후 재빌드 시 `_prune_article_assets` 가 옛 변종 정리.
- v0.4.2 로드맵 B-5 / B-6 는 별도 의제로 이월.

---

### v0.5.0 (2026-05-14) — BM25 기반 검색 시스템

v0.4.7 까지의 검색 랭킹은 토큰별 TF 를 단순 합산하고 제목 매치에 매직 ×5 가중치를 더하는 방식이었습니다. IDF / 문서 길이 정규화 / phrase 부스트가 모두 없어, 흔한 한글 bigram 이 희귀 영문 토큰과 동등 가중치였고, 본문이 긴 글이 단순 TF 누적으로 부당하게 상위를 차지했으며, 토큰 단위 분리 매치와 정확한 phrase 매치가 동일 점수로 취급되었습니다. v0.5.0 은 이 결함을 일괄 해소하는 **Okapi BM25 기반 랭킹** 으로 전환합니다.

| 변경 | 내용 |
|---|---|
| 인덱스 포맷 v3 ([scripts/search.py](scripts/search.py)) | v0.4.x 의 v2 와 비호환. 키 이름 변경 (`index/title_index` → `tf_body/tf_title`), 필드별 `df_*` (문서 빈도), 문서별 `dl_body`/`dl_title` (필드 길이), 전역 `stats.avgdl_*` (평균 길이), `params` (BM25 하이퍼파라미터) 가 인덱스에 박힘. `search.php` 가 `version < 3` 인 인덱스를 거부 — `Search index malformed or version mismatch (expected v3)`. |
| Per-field BM25 점수 | `score = bm25_body(Q,D) + w_title · bm25_title(Q,D)`. 본문은 `k1=1.5, b=0.75` (BM25 표준), 제목은 `k1=1.2, b=0.5` (짧고 의미 비중 큰 필드에 맞춤). `w_title = 3.0` — v0.4.x 의 매직 ×5 는 길이 정규화 없는 합산 위에 곱해졌던 임의값이었으나, v0.5.0 의 w_title 은 이미 IDF·길이 정규화로 척도가 맞춰진 점수 위에 곱하므로 의미가 명확. |
| Phrase 부스트 (곱셈) | 원본 쿼리 (trim 후 길이 ≥ 2) 가 평문 본문/제목에 **연속 substring** 으로 매치되면 score 에 곱셈 부스트 — 본문 ×1.5, 제목 ×2.0. 곱셈이므로 score=0 (토큰 미매치) 은 그대로 0 — 의미 있는 점수만 강화. |
| 매치 밀도 스니펫 | v0.4.x 까지의 "첫 매치 위치 ±40 글자" 폴백 대신, 본문에서 토큰 매치 위치를 모두 모은 뒤 **80 글자 슬라이딩 윈도우** 의 매치 수가 최대인 구간을 선정. 매치가 한 곳에 몰려 있는 글의 본문 맥락이 잘 드러남. 매치가 전혀 없는 코너 케이스 (phrase 매치만 됐을 때 등) 는 본문 처음 폴백. |
| 점수 모듈 분리 ([templates/search_bm25.php](templates/search_bm25.php)) | 점수 함수 (`bm25_idf`, `bm25_field_score`, `bm25_search`, `snippet_density`, `highlight_html`) 를 별도 파일로 분리. search.php 는 `require_once` 만 하고 라우팅·필터·HTML 렌더에 집중. search_tokenize.php 와 동일한 모듈화 패턴. 빌더가 두 lib 를 dist/ 로 함께 복사. |
| 단위 테스트 디렉터리 신설 ([tests/test_bm25.py](tests/test_bm25.py)) | v0.4.2 로드맵의 미수용 B-1 항목 일부 수용. BM25 알고리즘 핵심 회귀 차단: 토크나이저, IDF (희귀/흔한, 단조성), 필드 점수 (TF 증가/포화, 길이 정규화, b=0 무효화), 전체 점수 (제목 vs 본문 매치, 희귀 vs 흔한 쿼리, 빈/미매치 쿼리), phrase 부스트 (body/title, score=0 가드), 인덱스 빌더 (통계·정렬·톱레벨 카테고리), v0.4.x 회귀 가드 (긴 글 TF 폭주, 흔한 한글 bigram 의 희귀 영문 압도 방지). 25개 테스트. `python -m unittest discover -s tests` 로 실행. |
| Python ↔ PHP 점수 패리티 (출시 시점 검증) | `scripts/search.py` 의 `bm25_score()` 는 Python 참조 구현. `templates/search_bm25.php` 의 `bm25_search()` 는 런타임 구현. 출시 시점에 임시 CLI 스크립트로 5개 한국어/영문 쿼리에 대해 두 구현의 점수가 소수점 6자리까지 일치함을 확인. |

**호환성 노트:** 검색 인덱스 포맷이 v2 → v3 으로 비호환 — 반드시 v0.5.0 의 build.py 로 새 dist/ 생성 필요 (v2 인덱스를 그대로 두면 search.php 가 500 반환). meta.yaml / site.yaml 등 콘텐츠 스키마는 v0.4.7 과 동일. 검색 외 산출물도 v0.4.7 과 동등. BM25 하이퍼파라미터는 인덱스의 `params` 블록에 박혀 빌드마다 결정적 — 튜닝하려면 `scripts/search.py` 의 `BM25_PARAMS` 수정 후 재빌드.
...
**호환성 노트:** v0.4.6 dist 와 v0.4.7 dist 는 바이트 단위 동일 (회귀 0). meta.yaml / site.yaml 변경 불필요. v0.4.2 로드맵 B-1 / B-5 / B-6 은 v0.5.0 으로 이월.
...
**호환성 노트:** v0.4.5 → v0.4.6 dist 는 거의 동일 — 페이지네이션이 활성화된 section 의 2 페이지 이후 `<div class='listup_module_div'>` 에 `style='display:none'` 인라인 부착 + `.pagination-nav` 의 padding/margin 변경 (마크업 동일). 기존 글 meta.yaml 변경 불필요. 카테고리 meta.yaml 의 `priority` 미명시 시 모두 0 (= v0.4.5 알파벳 순과 동일). JS 비활성 환경에선 페이지 2 이후 미표시 (시스템 정책 — 의도된 동작).

#### v0.4.5 → v0.4.6 마이그레이션 시 주의

- 옛 `home_per_page` / `home_excludes_categories` / `home_sort` 키는 site.yaml 에서 삭제 (잔존 시 워닝). 같은 값은 `Articles/meta.yaml` 의 `per_page` / `excludes_categories` 로 이전 (`home_sort` 는 dead field 였으므로 그대로 폐기).
- `Articles/meta.yaml` 은 선택 — 미존재 시 코드 디폴트 (`per_page=5`, `excludes_categories=[]`).
- `priority` 는 정수만, `excludes_categories` 는 리스트만 (그 외 형식 오류 빌드 실패). 음수 priority 도 허용 (0 보다 뒤).
...
**호환성 노트:** v0.4.4 → v0.4.5 dist 는 의도된 차이 — `<html lang>` 동적값, 홈/카테고리에 페이지네이션 마크업 + pagination.js 추가, `{{CATEGORY_STYLES}}` 자리 추가, 서브카테고리 인덱스 신규, sitemap 에 서브카테고리 URL 추가. 기존 글 meta.yaml 변경 불필요 (`lang:` 미명시 시 site.lang). 기존 카테고리 폴더도 meta.yaml 없이 정상 동작. 빌더는 폴더에 `content.md` / `content.html` 이 있으면 글, 없으면 카테고리로 구분.

#### v0.4.4 → v0.4.5 마이그레이션 시 주의

- 카테고리 meta.yaml 에 `slug:` / `title:` / `date:` 절대 금지 (안전 가드 — 셋 중 하나라도 있으면 무시).
- `per_page` = 자기 인덱스 페이지 / `preview_per_page` = 상위에 임베드 section. 톱레벨 카테고리는 후자 거의 미사용.
- 다국어 SEO — 영어 글에 `lang: en` 명시 권장 (혼재 사이트).
- pagination.js 실패 시 모든 항목이 한 페이지에 펼쳐짐 (graceful degradation).
...
| 포함 URL | (1) 홈 (`/`), (2) 톱레벨 카테고리 인덱스 (`/{cat-slug}/`), (3) 모든 글 (`/{slug}/`). 서브카테고리는 인덱스 페이지가 없으므로 (원본 quirk — § 5 참조) 제외. `search.php` (noindex,follow) 와 `404.html`, `/assets/` 도 제외. v0.5.2 부터 글 자산이 `/{slug}/` 안에 있지만 sitemap 은 글 인덱스 URL 만 등록하고 자산 파일은 등록 대상이 아님. |
...
**호환성 노트:** 글 산출물 바이트 동일 (회귀 0). 추가는 `dist/sitemap.xml` + `dist/robots.txt` 한 줄.

#### v0.4.3 → v0.4.4 마이그레이션 시 주의

- 배포 전 `site.yaml` 의 `base_url` 정확성 확인 (sitemap 의 모든 URL 접두).
- `robots_txt_main:` 을 직접 커스텀했다면 `Sitemap: ...` 라인 보존 확인.
- 검색엔진 색인 가속하려면 Google Search Console / Naver Search Advisor 에 sitemap URL 직접 등록 권장.
- noindex 글은 sitemap 에서도 자동 제외 — 추가 작업 불필요.
...
- 글 폴더 안의 파일은 v0.5.2 부터 `/{slug}/` (글 index.html 과 같은 폴더) 로 복사됨 (v0.5.1 까지는 `/src/{slug}/`). 글 본문 안의 상대 경로 `./imgs/x.jpg` 는 자동 변환.
