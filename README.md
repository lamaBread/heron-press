...
|---|---|---|
| **v1.13.1** | 2026-06-06 | **Deploy preview: a fifth *Unchanged* card — the files that stay as-is (not transferred), so a build/deploy decision no longer requires reverse-engineering "what's untouched" from Added/Modified/Deleted in your head.** A dry-run emits a line only for files it *changes*; skipped files produce nothing, so "unchanged" can't be parsed from rclone's output — it is computed as a **local `dist/` walk minus the (added ∪ modified) set** (deletes live only on the server and never appear in the walk), purely local with no extra network. The new card sits between **Deleted** and **Warnings**; its panel lists each unchanged file (path + size) with the note *"these already match the server, so they will not be transferred"* and no diff button (nothing to compare against). The full changed-path set is tracked **uncapped** for the subtraction even though the display list keeps the same 500-file cap + `+N more` rollup as the other panels; the card grid switches to `auto-fit` to hold five cards and wrap gracefully. The earlier "미변경" wording was tightened to **변경 없음 / Unchanged**, aligned EN↔KO. Summary JSON stays **additive** (added `unchanged`/`unchanged_files`/`unchanged_more`); the CLI one-liner and every prior test are unchanged. Dry-run only; the apply path is byte-identical. No schema change (stamp 1.13.0 → 1.13.1); MANIFEST 88; 583 → 589 unit tests. |
| v1.13.0 | 2026-06-06 | Deploy preview: Added / Modified / Deleted / Warnings cards — each click-to-expand, with a per-file code diff for modified files. v1.12.0 merged copy+update into one "Upload" card; the preview now splits them into four **clickable accordion cards** — **Added** (rclone `copy`), **Modified** (`update`), **Deleted** (`delete`, flagged red when non-zero), **Warnings** — and clicking a card reveals its file list below. The **Deleted** panel prefixes each entry with the full server path (`remote_path` + relative) so it is unambiguous *what at which path* is removed. The **Modified** panel gives each file a *View diff* button: because a dry-run carries no file contents and the "before" lives only on the server, clicking fetches **just that one file** live via a new `Heron.py --deploy-diff` (relpath on stdin → `rclone cat` reusing the same connection flags) and renders a VSC-style **unified line diff** (Python `difflib`, single-sourced in `deploy.py`'s `compute_diff`/`cat_remote`). Safeguards: text-only (binary detected by null byte → "binary, no diff"), a 512 KiB cap (oversize → "too large"), modtime-only updates surface as "identical content", a `gone`/`error` kind for TOCTOU / connection failures, per-line 2000-char and per-diff 4000-line clamps, and a first-deploy 500-file list cap with a `+N more` rollup (the by-directory table is retained for the bird's-eye view). The new `?a=deploy_diff` AJAX endpoint is POST + CSRF, validates the path with `admin_safe_rel` + a realpath-under-`dist/` bound (mirroring the `asset` proxy) and runs argv-list (shell-free); an authenticated local admin can only read remote files that *also exist under local `dist/`* — a strict subset of what the existing apply-sync already permits. The summary JSON is **additive** (kept `upload`; added `added`/`modified`/`*_files`/`*_more`/`remote_path`), so the v1.12.0 CLI one-liner and every prior test stay unchanged; `build_argv`'s connection flags were extracted to a shared `_connection_flags` (guarded byte-identical by the existing argv tests). Dry-run only; the apply path is byte-identical. No schema change (stamp 1.12.0 → 1.13.0); MANIFEST 88; 568 → 583 unit tests. |
| v1.12.0 | 2026-06-06 | Deploy preview: collapsible 10-line live log + parsed dry-run summary. The admin deploy preview poured rclone's entire per-object `--dry-run` NOTICE stream (hundreds of lines) into a single `<pre>`, making it unreadable. The live log is now a ~10-row peek (`.log-peek`, ≈212px) that auto-follows the tail while streaming, with an Expand/Collapse toggle; beneath it Pond renders a parsed summary — Upload / Delete / Directory-ops / Warnings cards (a non-zero Delete is flagged red), a by-top-level-directory table (largest first, with a `(root)` bucket and a `+N more` rollup past 30 rows), and a warnings list (host-key-validation-off notices + junk `.DS_Store`/`Thumbs.db` upload flags). Parsing is single-sourced in `deploy.py` (`parse_size`/`human_size`/`DryRunSummary`/`build_dry_run_summary`): it tallies the rclone stream and **always** prints a one-line human summary (the CLI benefits too), and — only when Pond sets `HERON_DEPLOY_SUMMARY=json` — emits one machine-readable JSON line behind a control-char (RS) sentinel that `deploy_run.php` intercepts (hidden from the log) and renders into the cards/table; `admin_proc_stream` gained an optional `$env` arg (full `getenv()` merged so `proc_open` keeps PATH) to carry the flag. Dry-run only; the apply path is byte-identical. No schema change (stamp 1.11.4 → 1.12.0); MANIFEST 88; 555 → 568 unit tests. |
| v1.11.4 | 2026-06-06 | rclone deploy `ssh_alias` delegation mode (opt-in) — fixes the passphrase-key + host-key-mismatch deploy failures — `deploy.py`'s key-file path handed rclone only `--sftp-key-file ~/.ssh/id_ed25519`, so a **passphrase-protected** key died at `ssh: this private key is passphrase protected`; even past that, rclone's own host-key check hit `knownhosts: key mismatch` (Go's `x/crypto` prefers a non-ed25519 host-key algorithm, but `known_hosts` held only ed25519 for that hostname). New **optional** `ssh_alias` field: when set, `build_argv` drops the host/user/port/key/known_hosts flags and emits `--sftp-ssh "ssh <alias>"` + `--sftp-disable-hashcheck`, delegating auth **and** host-key verification to system ssh (`~/.ssh/config`, Keychain, ssh-agent, `known_hosts`) — the exact path that already works for `ssh <alias>`; `--sftp-disable-hashcheck` prevents the per-file `ssh md5sum` connection storm. `load_config` then requires only `remote_path` and skips the key/known_hosts file-existence checks (those paths go unused); the alias is restricted to `^[A-Za-z0-9._-]+$` (single argv token — no spaces/shell metachars). Pond ▸ Settings gains an `ssh_alias` input that makes host/user/key optional. **Fully backward-compatible** — with no `ssh_alias` the key-file path is byte-identical (Windows default unchanged; alias mode is a macOS/Linux opt-in). `EXAMPLE_CONFIG`/`deploy.example.json` left untouched (opt-in, not a default). Regression guards: `load_config` alias accept/reject + `remote_path`-only, `build_argv` alias-flag shape + dry-run toggle, and the real-rclone flag guard (v1.10.0) extended to the alias path. Also (release tooling): `make_manifest` now excludes OS file-explorer artifacts (`.DS_Store`/`Thumbs.db`) at any depth — regenerating the manifest on macOS/Windows no longer injects a phantom entry that would make `--check` report `missing` on every other platform (guards in `test_update`). No schema change (stamp 1.11.3 → 1.11.4); MANIFEST 88; 546 → 555 unit tests. |
...
*Heron v1.13.1 — build with Python + Pillow, runtime PHP (OPcache recommended). Full release history in [§ 16](#16-changelog).*
