...
|---|---|---|
| **v1.14.5** | 2026-06-10 | **The Build button now streams its log live — previously the page sat "loading" until the build finished and the whole log appeared at once.** The `?a=build` action used the blocking `admin_proc` (waits for `Heron.py` to exit, then renders everything); it now mirrors the deploy streaming path: output buffering off, and the builder's `[ n/16]` step headers (already `flush=True` on the Python side) flow line by line into a collapsible auto-follow `<pre id="log">`, with the success/failure banner moved below the log since the exit code is only known at the end. The logbar + follow script is extracted from `deploy_run.php` into a shared `admin_log_widget_head()` in `layout.php`, and its strings generalized from `admin.deploy_run.log.*`/`banner.tail` to `admin.log.*`. The build child's stderr (end-of-build issue report, `[ABORT]`, tracebacks) is merged into stdout via a new opt-in `2 => ['redirect', 1]` descriptor in `admin_proc_stream` — removing the pipe-buffer deadlock risk of the drain-after-EOF design and keeping output in true order — plus `PYTHONUNBUFFERED=1` as a seatbelt for the few unflushed prints; deploy's sentinel path is untouched (merge defaults off). Admin-UI only; builder and dist output byte-identical. No schema change (stamp 1.14.4 → 1.14.5); MANIFEST 89; 606 unit tests. |
...
*Heron v1.14.5 — build with Python + Pillow, runtime PHP (OPcache recommended). Full release history in [§ 16](#16-changelog).*
