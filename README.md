...
|---|---|---|
| **v1.9.3** | 2026-06-02 | **argparse `--help` + remaining-abort localization (v1.9.x language-pack line).** Closes the last *always-on* operator surfaces still hardcoded in Korean — (1) `Heron.py --help`'s description, epilog, and 11 option `help=` strings move to a new `cli.help.*` (because argparse fixes its text at parser-build time, `main()` now loads `i18n.init_from_base` **before** `parse_args` — a pure reordering, no new function); (2) the six `images.{widths,max_width,quality}` validation aborts become `build.abort.images_*` (the builder's `tool_tr`; `{received!r}` preserves the original repr); (3) the tokenizer-parity-failure header + the sibling line v1.9.2 missed (the missing/timeout warning that was English-only) become `build.parity.fail_header` / `php_unavailable`. Now admin, CLI, deploy, update, migration, build console/report, `--help`, and **every** abort are **fully consistent in the tool language** (only the conditionally-shown per-article `_issue`/`_warning` bodies remain, as the next scope). `ko` values are byte-identical, so Korean output is unchanged. No `user/` schema change → no new migration step (stamp 1.9.2 → 1.9.3); MANIFEST hashes refresh (86 files, count unchanged); dist byte-identical (the build console / `--help` / report live outside `dist/`); 514 → 517 unit tests + 6/6 diagnostics. |
