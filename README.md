# siheonlee.com v0.6.1 — 사용설명서 & 시스템 문서
...
> **v0.6.1 한 줄 요약:** v0.6.0 의 안정화 패치 (문서·주석·산출물 가독성 정리, **코드 동작 변경 0** — dist 산출물은 v0.6.0 과 사실상 바이트 동일). v0.6.0 의 큰 변경은 그대로 — (1) **검색 인덱스가 메타데이터 3-필드 (title / description / tags) BM25 색인** 으로 전환 + 인덱스가 `dist/search.php` 안에 PHP 정적 배열 리터럴로 인라인되어 OPcache 가 바이트코드와 함께 캐시 (매 요청 시 JSON 파싱 / 디스크 IO 0). 본문 색인 폐기, 스니펫 추출용으로 앞 1500 자만 보존. (2) **단위 테스트 179 개** + 진단 스크립트 (`tests/run_diagnostics.py`) 가 단위 테스트 + 빌드 결정성 + PHP 구문 + Python↔PHP BM25 점수 패리티 + 인덱스 형식을 한 번에 검사. 자세한 내역과 이전 버전 변경 사항은 [§ 18 업데이트 로그](#18-업데이트-로그) 참조.
...
이 폴더(`siheonlee.com_v0.6.1/`) 에서 터미널을 열고:
...
> 아래 표는 v0.6.1 시점에 여전히 유효한 한계만 모았습니다. v0.6.1 은 v0.6.0 의 안정화 패치라 한계 표는 v0.6.0 과 동일합니다 (v0.6.0 에서 해소된 두 항목 — 정적 검색 인덱스 본문 포함 / 테스트 부분적 — 은 표에서 제거된 상태 그대로).
...
**미수용 (차기 의제):** 홈/카테고리 페이지의 SEO 메타 태그 출력 (§ 17 한계 표). 태그별 색인 페이지 / 검색 결과의 카테고리·태그 필터 UI. Parsedown PHP↔Python 동등성 비교 트리 동봉 (의도된 한계).
...
*이 문서는 siheonlee.com v0.6.1 (PHP 기반 경량 웹 사이트 생성기 — 빌드는 Python + Pillow, 런타임은 PHP; 검색은 Okapi BM25 메타데이터 3-필드 색인 + PHP 정적 배열 인라인; 이미지는 WebP 다중 해상도 자동 변환) 기준으로 작성되었습니다. (2026-05-15)*
